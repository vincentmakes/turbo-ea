...
![Панель портфеля PPM](../assets/img/ru/40_ppm_portfel.png)
...
![Вкладка обзора инициативы](../assets/img/ru/41_ppm_obzor.png)
...
![Вкладка отчётов о статусе](../assets/img/ru/42_ppm_otchety_statusa.png)
...
![Вкладка бюджета и затрат](../assets/img/ru/43_ppm_byudzhet_zatraty.png)

Отслеживание финансовых данных с двумя типами строк:

- **Строки бюджета** — Запланированный бюджет по финансовому году и категории (CapEx / OpEx). Бюджетные строки группируются по **месяцу начала финансового года**, настроенному в [Настройках](../admin/settings.md#начало-финансового-года). Например, если финансовый год начинается в апреле, бюджетная строка за июнь 2026 относится к ФГ 2026–2027
...
![Вкладка управления рисками](../assets/img/ru/44_ppm_upravlenie_riskami.png)
...
Когда модуль GRC включён, вкладка показывает два блока: **Риски проекта** (таблица выше) и **Риски ландшафта** — список только для чтения записей [Реестра рисков](risks.md), связанных с этой карточкой инициативы, чтобы полная картина рисков проекта жила на одной вкладке. Риски проекта и риски ландшафта намеренно остаются отдельными реестрами: один живёт и умирает вместе с проектом, другой несёт жизненный цикл управления TOGAF.

Риск проекта, оказавшийся вопросом ландшафта, можно эскалировать действием **Перенести в реестр рисков** в его строке (требуется `risks.manage`). Это создаёт риск реестра из риска проекта — название, описание, владелец, преобразованные оценки, привязанная карточка инициативы, а текст мер смягчения превращается в задачу по смягчению. Действие идемпотентно; после переноса строка показывает чип **Открыть риск R-000123**, а сам риск проекта остаётся полностью редактируемым.

### Вкладка «Задачи»

![Доска задач PPM](../assets/img/ru/45_ppm_doska_zadach.png)
...
![Диаграмма Ганта PPM](../assets/img/ru/46_ppm_gantt.png)
...
- **Полосы прогресса** — Визуальный процент завершения. Щёлкните по плашке с процентом задачи или конечного рабочего пакета, чтобы открыть ползунок, который привязывается к значениям **0%, 50% или 100%** — соответствующим трём состояниям задачи (К выполнению, В работе, Выполнено). Родительские рабочие пакеты с дочерними элементами отображают плашку только для чтения, значение которой автоматически вычисляется из поддерева.
...
- **Авто-завершение** — Процент завершения автоматически рассчитывается из соотношения выполненных/общих задач, затем рекурсивно суммируется вверх по иерархии WBS до родительских элементов. Процент завершения верхнего уровня отражает общий прогресс инициативы
