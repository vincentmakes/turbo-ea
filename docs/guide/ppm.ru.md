...
- **Экспорт в Excel (.xlsx)** — также из меню **⋮**. По одной строке на инициативу с её группой, подтипом, руководителем проекта, датами начала и окончания, тремя оценками состояния, плановыми и фактическими CapEx/OpEx и датой последнего отчёта о статусе.
