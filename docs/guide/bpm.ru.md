...
### Привязка организаций (дорожки и шаги)

Организационная ответственность фиксируется на уровне потока двумя взаимодополняющими способами:

- **Назначение дорожек** — Свяжите дорожку BPMN с карточкой организации один раз: через панель *Назначение дорожек* над таблицей шагов или выбрав дорожку в редакторе. Каждый шаг дорожки автоматически наследует эту организацию.
- **Переопределение на уровне шага** — Столбец *Организация* в таблице шагов позволяет привязать отдельный шаг к другой организации. Унаследованная организация отображается как контурная метка; явное переопределение отображается как заполненная метка и имеет приоритет.

Названия дорожек остаются свободным текстом на диаграмме — привязка дорожки к карточке организации необязательна. Эффективная организация шага всегда вычисляется (собственное переопределение, если задано, иначе привязка дорожки), поэтому ничего не нужно поддерживать дважды, а привязки дорожек и ссылки шагов никогда не могут противоречить друг другу. Привязанные организации создают связь «Бизнес-процесс → Организация», видимую на обеих карточках и в отчётах о зависимостях.
