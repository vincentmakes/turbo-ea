...
- **打印 / 另存为 PDF** — 打印机图标会按屏幕上的样子打印投资组合。标签栏、筛选工具栏和悬停弹出框会被隐藏，当前的分组、子类型和搜索条件会打印为一行紧凑的参数摘要，时间轴网格不再被裁剪，因此所有列都能完整地放进页面，而不会在滚动边缘被截断。
- **导出为 PowerPoint (.pptx)** — 位于 **⋮** 菜单中。第一张幻灯片包含标题、生成时间戳和当前筛选条件，并以演示质量呈现投资组合；较长的投资组合会延续到后续幻灯片，且只在计划之间分页 — 绝不会从某一行、分组标题或合计行中间截断。
- **导出为 Excel (.xlsx)** — 同样位于 **⋮** 菜单中。每个计划一行，包含其分组、子类型、项目经理、开始与结束日期、三项健康度评级、资本支出/运营支出的计划值与实际值，以及最新状态报告的日期。
