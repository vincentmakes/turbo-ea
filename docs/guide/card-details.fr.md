...
- **Ajouter une relation** -- Cliquez sur **+** pour créer une nouvelle relation ; le sélecteur liste les cartes correspondantes dès son ouverture (les meilleures correspondances de votre saisie apparaissent en premier, d'autres se chargent au défilement). Les cartes déjà liées sont masquées, avec une légende indiquant leur nombre. Le sélecteur reste ouvert après chaque choix, ce qui permet d'ajouter plusieurs relations à la suite -- un compteur s'affiche en dessous et **Terminé** ferme la ligne
