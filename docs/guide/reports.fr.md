...
- **Regrouper les relations** — Sur un diagramme chargé, chaque ligne tracée séparément peut masquer la forme du paysage. Activez cette option dans le menu **Options d'affichage** pour regrouper les fiches liées dans des cadres et tracer une seule connexion entre deux cadres, portant le nombre de relations qu'elle représente. Choisissez ce que sont les cadres : **Par couche**, **Par type de fiche** (un cadre par type, comme *Applications (8)*) ou **Par sous-type**. Les relations entre deux fiches d'un même cadre y restent tracées, et la fiche centrée reste isolée : vous voyez d'un coup d'œil comment elle se rapporte à chaque partie du paysage. Survolez une connexion pour lister les relations regroupées, ou une fiche pour n'éclairer que les siennes. **Créer un diagramme** est indisponible tant que les relations sont regroupées — les cadres et les connexions regroupées n'ont pas d'équivalent dans l'inventaire auquel rattacher une forme.
