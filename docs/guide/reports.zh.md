...
当卡片的关系带有「类型」值时——例如组织→应用关系上的**使用类型**（所有者 / 使用者 / 干系人），或应用→业务能力关系上的**支持类型**——您可以按该值为卡片着色并进行筛选。关系子类型会显示在*着色依据*下拉菜单的**关系子类型**分组中，并作为独立的筛选行出现；只有当所选卡片类型确实存在带有此类值的关系时才会显示。通过多条关系连接且取值不同的卡片，会以中性的**多个**颜色显示。
