...
- **显示父级 / 显示子级** —— 展开模式的两种定向替代方式。在控制面板中开启**显示父级**（上箭头）或**显示子级**（下箭头），然后点击某张卡片，即可仅将其层级父级或直接子级添加到图中。已显示的卡片会保留在图中——便于将父级与子级叠加查看——并在重新居中视图时清除。
...
- **重新排列** —— 拖动某张卡片可在其所在层内移动它，或拖动整个**层框**可连同其所有卡片一起移动。**重置布局**可恢复自动排列。
