...
- **Панель фильтров** — полнотекстовый поиск по id, имени, описанию и алиасам, плюс чипы уровней (Macro → L1 → L4), множественный выбор отрасли и переключатель «Показывать устаревшие». При прокрутке остаётся закреплённой непосредственно под верхней навигацией.
...
## Макро-возможности (Уровень 0)

Над уровнями L1 / L2 / L3 / L4 каталог поставляет дополнительный слой **Macro** — небольшой набор группировок бизнес-уровня, которые охватывают целые семейства L1. Примеры включают *Customer Engagement* (охватывает L1 продаж, маркетинга, сервиса) или *Talent & Workforce* (охватывает L1 HR).

Макросы — это полноценные элементы каталога:

- Они попадают в ваш реестр как карточки `BusinessCapability` с `attributes.capabilityLevel = "Macro"` и `catalogueId` с префиксом `MC-` (например, `MC-10`).
- Они располагаются **над** своими дочерними L1 — ограничение глубины иерархии ослабляется с 5 до 6, чтобы вместить дополнительный слой (`Macro → L1 → L2 → L3 → L4 → L5`).
- Когда вы импортируете Макрос, любой существующий L1-потомок, помеченный как принадлежащий этому Макросу, автоматически переподчиняется новой карточке — то же двунаправленное связывание, что и между L1 и более низкими уровнями.
- **Макросы никогда не сопоставляются с существующими карточками по имени** — только по `catalogueId`. Это предотвращает случайные коллизии с клиентски именованными группами возможностей, у которых случайно совпадает метка с Макросом каталога.

Макросы можно выбирать со страницы каталога так же, как L1 — поставьте флажок, и поддерево выбирается соответствующим образом.
