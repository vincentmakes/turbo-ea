...
- **Show relationship labels** — Each relation's verb (*supports*, *uses*, …) is drawn on its line. On by default; turn it off in the **Card display** menu for a cleaner canvas on a dense landscape. The lines and their arrowheads still show what connects to what, and in which direction.
