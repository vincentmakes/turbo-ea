...
- **Card types** — The funnel button lists every card type on the diagram with how many cards of it are there. Untick one to take its cards, and every relation touching them, off the view — useful when you want to read how applications depend on IT components without the capabilities that come with them. A badge on the button counts what is currently hidden, a hidden type stays in the list so you can bring it back, and **Show all** restores everything at once. The card you are centered on is always shown, whatever its type. Choices are remembered between visits.
...
- **Aggregate relations** — On a busy diagram, every line drawn separately can hide the shape of the landscape. Turn this on in the **View options** menu to group the related cards into boxes and draw a single connector between any two boxes, labelled with how many relations it stands for: the count sits in a pill on the line, and the line is drawn heavier the more relations it carries, so the weight of a dependency reads at a glance. Choose what the boxes are: **By layer**, **By card type** (one box per type, as in *Applications (8)*) or **By subtype**. Relations between two cards of the same box stay drawn inside it, and the card you are centered on stays on its own, so you can see at a glance how it relates to each part of the landscape. While relations are aggregated the four layer lanes are set aside and the boxes are placed by what they connect to, which keeps the connectors short instead of running them the height of the diagram; **By layer** is there when you want the layers back. Hover a connector to list the relations it merged, or a card to light up just its own. The boxes can be dragged like the layer lanes, and their cards move with them. **Create diagram** works on a grouped view too, with a caveat it states first — see below.
...
- **Create diagram** — Turn the current view into a new, editable diagram in the [Diagram module](diagrams.md). It recreates the cards, relationships, and the four architecture layer lanes, and every shape stays linked to its inventory card. The connection routing travels too: lines keep the bends and the attachment points the report worked out, so the diagram opens looking like the report instead of being re-routed from scratch. You're asked for a name, then taken straight to the new diagram. Available to users who can create diagrams. From a view with **Aggregate relations** on, each box becomes a container in the diagram with its cards inside — still linked to the inventory — while the connectors between boxes are drawn as decoration carrying today's counts: they do not follow the inventory, cannot be synced and are not flagged when relations change. A dialog says so before you are asked for the name.

![Relations aggregated by card type](../assets/img/en/13c_dependencies_aggregate.png)
...
Costs are annual, so the report always shows them for **one fiscal year** — the current one when you open it. A card's annual cost counts **in full** in every fiscal year from the one it goes **Active** in through the one its **End of Life** falls in, both included. Nothing is pro-rated: an application retiring in November still carries its whole annual cost for that year, and none in the next. A card whose End of Life fell in an earlier fiscal year therefore no longer adds to the total.

Missing lifecycle dates are read as in the other time-travel reports:

- **No Active date, but a Plan or Phase In date** — the card is still planned and counts in no year.
- **No End of Life date** — the card counts in every year from the one it goes Active in.
- **No lifecycle dates at all** — the card counts in every year.

To look at another year, drag the **Time Travel** slider in the toolbar. It stops on fiscal years only — there is nothing in between to land on — and the arrow keys step one year at a time. The read-out names the selected year (for example *FY 2026*, or *FY 2025–2026* when your fiscal year does not start in January), and **Current fiscal year** brings you back. The slider spans the years your cards' lifecycles cover, plus one on either side.

The selected year applies to everything on the page: the totals, the treemap, the table, **Cost Source** roll-ups (a retired IT Component stops adding to its Application's total) and drill-downs. It is printed in the report header and saved with the report: a report saved on the current year re-opens on whichever year is current then, and one saved on another year re-opens on that year. An administrator sets the month the fiscal year starts in under [Settings → General](../admin/settings.md#fiscal-year-start).
