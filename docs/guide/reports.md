...
- **Aggregate relations** — On a busy diagram, every line drawn separately can hide the shape of the landscape. Turn this on in the **View options** menu to group the related cards into boxes and draw a single connector between any two boxes, labelled with how many relations it stands for. Choose what the boxes are: **By layer**, **By card type** (one box per type, as in *Applications (8)*) or **By subtype**. Relations between two cards of the same box stay drawn inside it, and the card you are centered on stays on its own, so you can see at a glance how it relates to each part of the landscape. Hover a connector to list the relations it merged, or a card to light up just its own. **Create diagram** is unavailable while relations are aggregated — boxes and merged connectors have no inventory counterpart to link a shape to.
