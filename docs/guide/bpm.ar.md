...
- انقر خلية **التطبيق** أو **كائن البيانات** أو **مكوّن تقنية المعلومات** لخطوة ما واختر البطاقة — يتصفّح المنتقي المخزون، فلا يُكتب شيء يدويًا
- يُخزَّن الربط على الخطوة وينشئ علاقة بين العملية والبطاقة، فيظهر في تدفّق العملية وفي تبويب «العلاقات» للبطاقة على حدّ سواء
- يربط عمود **عملية الأعمال** الخطوة بالعملية التي تُسلِّم إليها — انظر أدناه
- وتتوفّر الروابط الخمسة نفسها **داخل المُحرِّر** ما دامت مسودة مفتوحة: مجموعة **البطاقات المرتبطة** في لوحة الخصائص، ومدخل **ربط بطاقات** في قائمة السياق يفتح قائمةً بها
- على المخطط نفسه، تحمل الخطوة المرتبطة **نقطة صغيرة لكل نوع بطاقة مرتبط** أسفل اسمها، بلون نوع البطاقة ذاك، في المُحرِّر وفي العارض للقراءة فقط على حدٍّ سواء. لا تُرسَم الأسماء على اللوحة: انقر على الخطوة لرؤيتها، أو مرِّر المؤشر فوق نقطة

### ربط خطوة بعملية

كثيرًا ما تُسلِّم الخطوة إلى عملية قائمة بذاتها — لها مخططها ومالكها ودورة حياتها الخاصة، وتُعاد عادةً من عدة مواضع. وأي خطوة تستطيع التعبير عن ذلك: مهمة أو عملية فرعية أو حدث أو بوابة تشير إلى بطاقة **عملية أعمال**، ولا تُدخل أبدًا معرّف عملية يدويًا:

- تعرض **لوحة الخصائص** مجموعة **البطاقات المرتبطة** على كل خطوة، بصفٍّ لكل رابط — عملية الأعمال، التطبيق، كائن البيانات، مكوّن تقنية المعلومات، المؤسّسات — ولكلٍّ منها **اختر** و**فتح** (للتعمّق في البطاقة المرتبطة) و**مسح**
- تحمل **قائمة السياق** للخطوة المحدَّدة مدخل **ربط بطاقات** يسرد الخمسة نفسها، لحالة طيّ اللوحة. أما كائن البيانات أو مخزنها فيتيح رابط كائن البيانات وحده، كما في الجدول
- يحتوي **جدول الخطوات** للتدفّق المنشور (وجدول الربط المسبق للمسودة) على الربط نفسه في عمود **عملية الأعمال**، والشريحة هناك تتعمّق في تبويب «تدفّق العملية» للعملية المرتبطة. أما كائنات البيانات ومخازنها فليست خطوات، ولذلك تعرض صفوفها شَرطة

![البطاقات المرتبطة](../assets/img/ar/92_bpm_called_process.png)

في BPMN بنية واحدة *هي بذاتها* عملية أخرى: **نشاط الاستدعاء** (call activity)، وهو مهمة بحدّ سميك تستدعي عملية معرَّفة بشكل مستقل. أما **العملية الفرعية** المضمَّنة فتجمع الخطوات أيضًا لكنها تخصّ المخطط الذي رُسمت فيه؛ وقاعدة Method & Style بسيطة: إذا كانت العملية موجودة بشكل مستقل فاستخدم نشاط استدعاء. ويعامله Turbo EA بوصفه الحالة الأصلية — **فعند وضع نشاط استدعاء يُسأل عن العملية التي يستدعيها**، ويُحفظ الربط في *العنصر المُستدعى* الخاص بـ BPMN، فتقرأه الأدوات الأخرى. أما كل خطوة أخرى فتحفظ الربط سمةً من سمات Turbo EA داخل المخطط.

يؤدي نشر تدفّق يحتوي خطوة مربوطة إلى إنشاء علاقة **يستدعي** بين العمليتين — يعرض تبويب «العلاقات» للعملية المرتبطة *يُستدعى بواسطة*، وترسم عرض التبعيات مخطط الاستدعاءات. يحتفظ المخطط المستورد من أداة أخرى بمرجع العملية الخاص بتلك الأداة؛ ويعرضه جدول الخطوات كتلميح (*يشير إلى Process_X*) حتى تختار العملية المطابقة في Turbo EA.

### مجموعة واحدة من الروابط

يعرض المُحرِّر والجداول الروابط نفسها، فتُقرأ الخطوة بالطريقة ذاتها أينما نظرت إليها:

- داخل **المسودة** يسود ما تضبطه فيها — في المُحرِّر أو في جدول **الربط المسبق للعناصر**، وكلاهما المخزن نفسه — ويبقى مرجع العملية في المخطط بديلًا لخطوة لم تقل عنها شيئًا. ومسح الرابط يمسحه فعلًا، وعند النشر أيضًا.
- ويظل التدفّق **المنشور** معتمدًا بينما تُحرَّر روابطه في جدول عناصره: فتلك بيانات وصفية فوق مخطط موقَّع، لا سببًا لاعتماده من جديد.
- وتبدأ المسودة **المُنشأة من النسخة المنشورة** من الروابط التي تحملها العملية في تلك اللحظة. أما مسودة مفتوحة بالفعل فتحتفظ بروابطها، حتى لا يغيّر تحرير شخصٍ آخر المخطط الذي تعمل عليه.
