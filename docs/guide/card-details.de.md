...
- **Beziehung hinzufügen** — Klicken Sie auf **+**, um eine neue Beziehung zu erstellen; die Auswahlliste zeigt passende Karten bereits beim Öffnen an (die besten Treffer zu Ihrer Eingabe stehen oben, weitere werden beim Scrollen geladen). Bereits verknüpfte Karten werden ausgeblendet; eine Beschriftung nennt ihre Anzahl. Die Auswahlliste bleibt nach jeder Auswahl geöffnet, sodass Sie mehrere Beziehungen nacheinander hinzufügen können, und jede hinzugefügte Karte erscheint als Chip direkt über dem Suchfeld — ein Klick auf das **×** eines Chips macht diese Zuordnung rückgängig. **Fertig** schließt die Zeile
