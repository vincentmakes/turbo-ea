...
- **A `Relations` sheet** listing **every relation** of the exported cards, one row each. This is where a relation's own values (`attr_<field>`) and description are read and filled in.
...
A relation can carry values of its own — a *Usage Type* on an `Organization` → `Application` link, an annual cost on an `Application` → `IT Component` link, or a free-text description. A `rel:` cell on a card sheet is already a list of names and has nowhere to put them, so they live on the `Relations` sheet:
...
The sheet lists **every relation of the exported cards**, whether or not its type currently carries values — so a relation type you have just given a value to has a row waiting for it. Relations pointing *at* an exported card are listed too; `source_ref` and `target_ref` say which way round each one goes. The `attr_<field>` columns are those of the relation types in this workbook, not every one defined in the instance.
...
!!! note "A relation type that carries values has no `rel:` column"
    A card sheet's `rel:` cell is a list of card names with no room for a value, so a relation type that carries one is edited on this sheet instead. If you add a `rel:` column for such a type by hand, the import warns and ignores it rather than silently dropping the values.

If a card sheet and the `Relations` sheet contradict each other — one adds a relation the other removes — the **removal wins**, and the import preview says so.
