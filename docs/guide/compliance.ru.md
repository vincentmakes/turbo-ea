...
|----------|-------------|--------------------|
| **Ручной** | Пользователь с `compliance.manage` нажимает **+ Новая находка** в сетке Соответствия | Обязательства из аудита, внешне отрапортованные пробелы, аттестации третьих сторон — всё, что нужно отслеживать, но что AI-сканирование не выявит |
| **AI-сканирование** (TurboLens) | Пользователь с `compliance.manage` запускает сканирование с панели инструментов Соответствия | Периодический анализ пробелов ландшафта по включённым регламентам |
...
| **Охват** | Обязательно. Выберите **Конкретную карточку** — привязать находку к одному Приложению или ИТ-Компоненту — или **Весь ландшафт** для системного, охватывающего весь парк пробела, которым не владеет ни одна карточка (например, отсутствующая процедура уровня организации). Находки уровня ландшафта отображаются зелёным в столбце **Карточка**. |
| **Связанный риск** | Опционально — заранее связать с существующим Риском, если он уже отслеживает этот пробел. |

Каждая находка должна быть привязана либо к конкретной карточке, либо ко всему ландшафту — неоднозначного состояния «без карточки» не существует. Ландшафтный охват — это полноценный выбор, а не пустое поле: сканирования ИИ также создают находки уровня ландшафта для системных мер контроля, охватывающих весь парк.

`compliance.manage` необходимо для создания, редактирования, удаления или пакетных действий с находками. `compliance.view` достаточно для чтения реестра и сортировки из вкладки Соответствие на уровне карточки.

### Редактирование находки

Каждая строка таблицы Соответствия содержит встроенную иконку **Изменить** (карандаш), которая открывает находку для редактирования в один клик. Вы также можете открыть любую находку — из таблицы Соответствия или из вкладки **Соответствие** карточки — и нажать **Изменить** в панели, чтобы после создания изменить её **статус** соответствия (например, Соответствует → Частично), серьёзность, требование, пробел, свидетельство, исправление, статью или связанную карточку. Редактирование содержимого не затрагивает решение жизненного цикла находки; для этого используйте временную шкалу жизненного цикла. Требует `compliance.manage`.
...
Переходы ограничены пользователями с `compliance.manage`. Движок принудительно выполняет переходы на сервере и отклоняет незаконные движения с понятной ошибкой.
...
При предоставлении `compliance.manage` сетка показывает фильтр-осведомлённый мультивыбор. Отметьте флажок заголовка, чтобы выбрать каждую строку, соответствующую активным фильтрам, затем используйте липкую панель инструментов:
...
![Деталь карточки — вкладка Соответствие](../assets/img/ru/56_kartochka_sootvetstvie_tab.png)

Карточки в области любой находки также показывают вкладку **Соответствие** на своей странице деталей (управляется `compliance.view`). Она перечисляет каждую находку, связанную с карточкой, с теми же действиями Подтвердить / Принять / **Создать риск** / **Открыть риск**, что и вид GRC — так что владелец Приложения может сортировать свои собственные находки, не покидая карточку. Тот же правило авто-скрытия применяется к вкладке **Риски** в деталях карточки: обе вкладки появляются только тогда, когда карточка действительно имеет связанные элементы, чтобы карточки без активности GRC не тянули за собой пустые вкладки.
...
`compliance.view` управляет доступом для чтения к реестру, вкладке Соответствие по карточке и KPI обзора. `compliance.manage` необходимо для создания или редактирования находок, изменения их статуса, запуска сканирований, пакетных действий, продвижения в Риск или удаления находки.
