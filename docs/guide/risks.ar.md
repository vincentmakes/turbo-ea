...
يمكن **تجميع** جدول السجل: زر *تجميع حسب* في شريط أدوات الجدول يرتب المخاطر تحت عناوين قابلة للطي حسب الحالة أو الفئة أو المستوى الأولي أو المتبقي أو المالك، مع مجموعة «غير محدد» في الأعلى — وهو أمر عملي لمراجعة السجل مجموعةً تلو الأخرى في اجتماع المراجعة. وعند التمرير داخل مجموعة طويلة يظل عنوانها مثبتًا أسفل عناوين الأعمدة، فلا تفقد أبدًا معرفة المجموعة التي أنت فيها.
...
- من صفحة تفاصيل المخاطرة: لوحة **Affected cards** ← اختر بطاقة وأضفها. انقر على `×` لإلغاء الربط. يعرض المُنتقي قائمة قابلة للتصفح فور فتحه (دون الحاجة إلى الكتابة أولًا)، ويحمّل مزيدًا من البطاقات عند التمرير، ويخفي البطاقات المرتبطة بالفعل.
...
تُجمَّع البطاقات المرتبطة **حسب نوع البطاقة**، وتُرتَّب أبجديًا داخل كل مجموعة، وتُلوَّن بلون نوع البطاقة نفسه، فتبقى مخاطرة تمس عشرات البطاقات عبر عدة أنواع مقروءة بنظرة واحدة. ويسري الترتيب واللون ذاتهما على عمود **Cards** في شبكة السجل، حيث تظهر كل بطاقة كشريحة بلون نوعها — ويعرض تلميح الخلية القائمة الكاملة تحت عناوين الأنواع.
...
![مهام التخفيف في صفحة تفاصيل الخطر](../assets/img/ar/97_risk_mitigation_tasks.png)
...
![مصفوفة المخاطر 4×4 في سجل المخاطر](../assets/img/ar/78_risk_matrix.png)
...
### الترشيح حسب البطاقات المتأثرة

يضم شريط الترشيح الأيسر قسم **Affected cards** الذي يجيب عن أسئلة المشهد التي يعجز مرشِّح العمود عنها:

- **Cards** — اختر بطاقة أو أكثر لعرض كل مخاطرة تؤثر في *أي* منها («اعرض كل المخاطر المؤثرة في التطبيقات A وB وC»). تعرض القائمة كل بطاقة مرتبطة بمخاطرة واحدة على الأقل، مع إمكانية البحث بالاسم.
- **Card type** — حدِّد نوعًا أو أكثر لعرض كل مخاطرة تمس *أي* بطاقة من تلك الأنواع («اعرض كل المخاطر المؤثرة في أي تطبيق»)، دون تسمية بطاقات بعينها.

تُطابق كل قائمة **أيًّا** من اختياراتها الخاصة، وتُدمج القائمتان بعلاقة **و**. لذا فإن تحديد `Application` واختيار البطاقة *Payments Gateway* يعيد المخاطر التي تؤثر في Payments Gateway **وَ**تمس تطبيقًا واحدًا على الأقل.

وكغيرها من مرشِّحات الشريط الجانبي، تُضيّق هذه المرشِّحات الصفحة بأكملها — فبطاقات مؤشرات الأداء ومصفوفة 4×4 والشبكة وورقتا تصدير `.xlsx` تتبع الاختيار النشط.
