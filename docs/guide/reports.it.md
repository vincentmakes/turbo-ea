...
La scheda che mettete al centro determina quanto vedete: per questo il selettore elenca per primi, in ogni tipo, le schede meglio collegate. Una capability è di solito la scelta più rivelatrice, perché è l'unico tipo di scheda che raggiunge in un solo passo gli obiettivi sopra e le applicazioni sotto. Il selettore elenca **ogni scheda dell'inventario** — escluse quelle archiviate — qualunque sia la data su cui si trova la linea temporale: è qui che scegliete cosa guardare, e il cursore è nascosto in questa fase, quindi una scheda già dismessa, o non ancora in produzione, resta comunque centrabile. Le schede che hanno raggiunto il **fine vita a oggi** (non alla data della linea temporale) portano l'etichetta *DISMESSA* con la relativa data di fine vita; l'interruttore **Nascondi le schede a fine vita**, accanto ai chip di tipo, le filtra. Una volta centrata una scheda, l'elenco **Centra su** nella barra degli strumenti permette di passare a un'altra: quell'elenco è **alfabetico**, perché a quel punto sapete quale scheda cercate — i due ordinamenti differiscono di proposito.
...
- **Mostra sulla scheda** — Un pulsante dedicato della barra degli strumenti (l'icona a forma di occhio) elenca come **caselle di spunta** tutto ciò che una scheda può mostrare: l'etichetta del **tipo**, il **sottotipo**, un **indicatore di stato del ciclo di vita** e ogni **campo attributo** disponibile, raccolto sotto il tipo di scheda a cui appartiene. Le prime due righe compaiono sulla scheda stessa e l'insieme completo nel suggerimento. Un contrassegno sul pulsante conta ciò che è attualmente mostrato. Le scelte vengono ricordate tra una visita e l'altra e seguono **Crea diagramma**: un diagramma DrawIO generato da questo report si apre con le stesse righe, scelte dallo stesso menu — lì tutte, perché una forma del diagramma cresce per contenerle mentre un nodo del report no. Su un telefono l'elenco si apre a schermo intero. **Cancella tutto** toglie tutte le spunte in una volta.
- **Tipi di scheda** — Il pulsante a imbuto elenca ogni tipo di scheda presente nel diagramma con quante schede di quel tipo ci sono. Deselezionane uno per togliere dalla vista le sue schede e ogni relazione che le tocca: utile per leggere come le applicazioni dipendono dai componenti IT senza le capacità che le accompagnano. Un contrassegno sul pulsante conta ciò che è nascosto, un tipo nascosto resta nell'elenco per poterlo ripristinare, e **Mostra tutto** riporta tutto in una volta. La scheda al centro è sempre mostrata, qualunque sia il suo tipo. Le scelte vengono ricordate tra una visita e l'altra.
- **Mostra i logo delle schede** — Una scheda che possiede un proprio logo lo mostra nell'angolo in alto a sinistra, con l'icona del tipo di scheda come piccolo badge sopra di esso, così restano leggibili sia il prodotto sia il tipo di scheda. Attivo per impostazione predefinita; disattivalo dal menu **Opzioni di visualizzazione** per un diagramma essenziale. Le schede senza logo — e tutte le schede di un tipo per cui un amministratore ha disattivato i logo — restano invariate in entrambi i casi. I logo sono inclusi nelle esportazioni di immagini.
...
- **Raggruppa le relazioni** — In un diagramma affollato, ogni linea tracciata singolarmente può nascondere la forma del panorama. Attiva questa opzione nel menu **Opzioni di visualizzazione** per raggruppare le schede collegate in riquadri e tracciare una sola connessione fra due riquadri, etichettata con quante relazioni rappresenta: il numero sta in una pillola sulla linea, e la linea è tracciata tanto più spessa quante più relazioni porta, così il peso di una dipendenza si legge a colpo d'occhio. Scegli cosa sono i riquadri: **Per livello**, **Per tipo di scheda** (un riquadro per tipo, come *Applicazioni (8)*) o **Per sottotipo**. Le relazioni fra due schede dello stesso riquadro restano disegnate al suo interno, e la scheda al centro resta a sé: così si vede a colpo d'occhio come si rapporta a ciascuna parte del panorama. Finché le relazioni sono raggruppate, le quattro corsie dei livelli vengono messe da parte e i riquadri sono collocati in base a ciò che collegano, il che tiene corte le connessioni invece di farle percorrere tutta l'altezza del diagramma; **Per livello** ti restituisce i livelli. Passa sopra una connessione per elencare le relazioni raggruppate, o sopra una scheda per illuminare solo le sue. I riquadri si possono trascinare come le corsie dei livelli, e le loro schede si spostano con essi. **Crea diagramma** funziona anche su una vista raggruppata, con un'avvertenza mostrata prima: vedi sotto.
...
- **Stile della linea** — Scegliete come vengono disegnate le linee di connessione a riposo: **continua**, **punteggiata**, **tratteggiata** (predefinita) o **tratto lungo**, dal menu **Opzioni di visualizzazione**. Al passaggio del mouse la linea è sempre continua e una dipendenza interrotta mantiene i propri trattini.
...
- **Crea diagramma** — Trasforma la vista corrente in un nuovo diagramma modificabile nel [modulo Diagrammi](diagrams.md). Ricrea le schede, le relazioni e le quattro corsie dei livelli di architettura, e ogni forma rimane collegata alla sua scheda di inventario. Ti viene chiesto un nome e poi vieni portato direttamente al nuovo diagramma. Disponibile per gli utenti che possono creare diagrammi. Da una vista con **Raggruppa le relazioni** attivo, ogni riquadro diventa nel diagramma un contenitore con le sue schede all'interno — ancora collegate all'inventario — mentre le connessioni fra riquadri sono disegnate come decorazione con i conteggi di oggi: non seguono l'inventario, non possono essere sincronizzate e non vengono segnalate quando le relazioni cambiano. Una finestra lo dice prima che ti venga chiesto il nome.

![Relazioni raggruppate per tipo di scheda](../assets/img/it/13c_dipendenze_aggregate.png)
