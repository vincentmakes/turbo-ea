...
Welche Karte Sie ins Zentrum stellen, entscheidet, wie viel Sie sehen — die Auswahl listet deshalb je Typ die am besten vernetzten Karten zuerst. Eine Fähigkeit ist meist die aufschlussreichste Wahl: Sie ist der einzige Kartentyp, der in einem Schritt sowohl die darüberliegenden Ziele als auch die darunterliegenden Anwendungen erreicht. Die Auswahl listet **jede Karte des Bestands** — archivierte ausgenommen — unabhängig davon, auf welchem Datum die Zeitleiste steht: hier wählen Sie aus, was Sie ansehen möchten, und der Regler ist in diesem Schritt ausgeblendet. Eine bereits ausgeschiedene Karte oder eine noch nicht gestartete lässt sich also weiterhin ins Zentrum stellen. Karten, die **zum heutigen Tag** (nicht zum Datum der Zeitleiste) ihr End-of-Life erreicht haben, tragen die Markierung *AUSGESCHIEDEN* samt End-of-Life-Datum; der Schalter **End-of-Life ausblenden** neben den Typ-Chips blendet sie aus. Sobald Sie eine Karte ins Zentrum gestellt haben, wechseln Sie über das Auswahlfeld **Zentrieren auf** in der Werkzeugleiste zu einer anderen: diese Liste ist **alphabetisch**, denn dann kennen Sie die gesuchte Karte bereits — die beiden Sortierungen unterscheiden sich mit Absicht.
...
- **Auf Karte anzeigen** — Eine eigene Schaltfläche in der Symbolleiste (das Augensymbol) listet als **Kontrollkästchen** alles auf, was eine Karte anzeigen kann: das **Typ**-Label, den **Untertyp**, einen **Lebenszyklus-Statuspunkt** und jedes verfügbare **Attributfeld**, jeweils unter dem Kartentyp eingeordnet, zu dem es gehört. Die ersten beiden Zeilen erscheinen auf der Karte selbst, der vollständige Satz im Tooltip. Ein Abzeichen an der Schaltfläche zählt, was gerade angezeigt wird. Die Auswahl bleibt zwischen Besuchen erhalten und wird von **Diagramm erstellen** übernommen: Ein aus diesem Bericht erzeugtes DrawIO-Diagramm öffnet sich mit denselben Zeilen, ausgewählt über dasselbe Menü — dort mit allen, da eine Diagrammform mitwächst, ein Berichtsknoten aber nicht. Auf dem Smartphone öffnet sich die Liste im Vollbild. **Alle löschen** entfernt sämtliche Häkchen auf einmal.
- **Kartentypen** — Die Trichter-Schaltfläche listet jeden Kartentyp im Diagramm mit der Anzahl seiner Karten auf. Entfernen Sie das Häkchen bei einem Typ, um dessen Karten und alle daran hängenden Beziehungen aus der Ansicht zu nehmen — nützlich, wenn Sie lesen möchten, wie Anwendungen von IT-Komponenten abhängen, ohne die Fähigkeiten, die daran hängen. Ein Abzeichen auf der Schaltfläche zählt, was gerade ausgeblendet ist, ein ausgeblendeter Typ bleibt in der Liste, damit Sie ihn zurückholen können, und **Alle anzeigen** stellt alles auf einmal wieder her. Die zentrierte Karte wird immer angezeigt, unabhängig von ihrem Typ. Die Auswahl wird zwischen Besuchen gespeichert.
- **Kartenlogos anzeigen** — Eine Karte mit eigenem Logo zeigt es in der oberen linken Ecke, das Kartentyp-Symbol als kleines Abzeichen darüber — so bleibt sowohl erkennbar, um welches Produkt es sich handelt, als auch um welchen Kartentyp. Standardmäßig aktiv; im Menü **Ansichtsoptionen** ausschalten für ein schmuckloses Diagramm. Karten ohne Logo — und alle Karten eines Typs, für den ein Administrator Logos deaktiviert hat — sehen so oder so unverändert aus. Logos werden in Bildexporte übernommen.
...
- **Beziehungen bündeln** — In einem dichten Diagramm kann jede einzeln gezeichnete Linie die Form der Landschaft verdecken. Aktivieren Sie dies im Menü **Ansichtsoptionen**, um die verbundenen Karten in Kästen zusammenzufassen und zwischen zwei Kästen eine einzige Verbindung zu zeichnen, beschriftet mit der Anzahl der Beziehungen, für die sie steht: Die Zahl sitzt in einer Plakette auf der Linie, und die Linie wird umso kräftiger gezeichnet, je mehr Beziehungen sie trägt — so ist das Gewicht einer Abhängigkeit auf einen Blick erkennbar. Wählen Sie, was die Kästen sind: **Nach Ebene**, **Nach Kartentyp** (ein Kasten je Typ, etwa *Anwendungen (8)*) oder **Nach Untertyp**. Beziehungen zwischen zwei Karten desselben Kastens bleiben darin gezeichnet, und die zentrierte Karte steht weiterhin für sich — so sehen Sie auf einen Blick, wie sie sich zu jedem Teil der Landschaft verhält. Solange Beziehungen gebündelt sind, werden die vier Ebenen-Spuren beiseitegelassen und die Kästen danach platziert, was sie verbindet — das hält die Verbindungen kurz, statt sie über die gesamte Diagrammhöhe laufen zu lassen; **Nach Ebene** holt die Ebenen zurück. Fahren Sie über eine Verbindung, um die gebündelten Beziehungen aufzulisten, oder über eine Karte, um nur deren eigene hervorzuheben. Die Kästen lassen sich wie die Ebenen-Spuren verschieben, ihre Karten wandern mit. **Diagramm erstellen** funktioniert auch in einer gebündelten Ansicht — mit einem Hinweis, den es zuerst gibt, siehe unten.
...
- **Linienstil** — Wählen Sie, wie Verbindungslinien im Ruhezustand gezeichnet werden: **durchgezogen**, **gepunktet**, **gestrichelt** (Standard) oder **lang gestrichelt**, über das Menü **Ansichtsoptionen**. Beim Überfahren wird eine Linie immer durchgezogen dargestellt, und eine getrennte Abhängigkeit behält ihre eigenen Striche.
...
- **Diagramm erstellen** — Wandeln Sie die aktuelle Ansicht in ein neues, bearbeitbares Diagramm im [Diagramm-Modul](diagrams.md) um. Karten, Beziehungen und die vier Architektur-Ebenen werden nachgebildet, und jede Form bleibt mit ihrer Inventarkarte verknüpft. Sie werden nach einem Namen gefragt und gelangen anschließend direkt zum neuen Diagramm. Verfügbar für Benutzer, die Diagramme erstellen dürfen. Aus einer Ansicht mit eingeschaltetem **Beziehungen bündeln** wird jeder Kasten im Diagramm zu einem Container mit seinen Karten darin — weiterhin mit dem Inventar verknüpft —, während die Verbindungen zwischen Kästen als Dekoration mit den heutigen Zahlen gezeichnet werden: Sie folgen dem Inventar nicht, lassen sich nicht synchronisieren und werden bei geänderten Beziehungen nicht markiert. Ein Dialog sagt das, bevor Sie nach dem Namen gefragt werden.

![Nach Kartentyp gebündelte Beziehungen](../assets/img/de/13c_abhaengigkeiten_aggregiert.png)
