...
- **Ссылки** — введённый или вставленный URL становится ссылкой по мере ввода, и каждая ссылка в документе открывается в новой вкладке. Адреса в документах, написанных до появления поддержки ссылок, превращаются в ссылки при отображении
...
- **Архитектурные решения** — записи ADR здесь больше не перечисляются. Теперь у каждой карточки есть собственная вкладка **ADR**; о том, что она показывает и как привязать, создать или отвязать решение с карточки, см. [Детали карточки](card-details.md).
