...
画布上的每张卡片都会在左上角显示其**卡片类型图标**（一个白色小字形），紧邻类型颜色——因此卡片的类型同时通过图标和颜色来传达。这与整个应用中使用的图标一致，并改善了色盲用户的可读性。该图标会出现在此后插入的卡片上。若要为已在较早图表中的卡片添加图标，请在编辑器工具栏中点击**应用卡片类型图标**。如果卡片拥有自己的**徽标**，则改为显示徽标，并将卡片类型图标保留为角落的小角标——这样图形既说明了这是哪个产品，也说明了它是哪类卡片。徽标在打开图表时出现，并在徽标更换后刷新；没有徽标的卡片，以及管理员已关闭徽标的类型下的所有卡片，绘制方式与以往完全相同。
