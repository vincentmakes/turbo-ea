...
同一个下拉菜单还决定**每个图形显示什么内容**。在**在卡片上显示**中勾选**卡片类型**、**子类型**或画布上现有卡片类型的任意属性，每个图形便会在名称下方增加小号明细行。 该下拉菜单分为两部分：**着色依据**选择为图形着色的那一个属性，**在卡片上显示**则可选择任意多个要打印在图形上的字段。字段按其所属的卡片类型分组列出；若某字段为其中多个类型共有，则归入**共用**。
