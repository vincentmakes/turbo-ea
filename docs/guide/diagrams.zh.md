...
工具栏中的第二个按钮**在卡片上显示**决定**每个图形显示什么内容**。勾选**卡片类型**、**子类型**或画布上现有卡片类型的任意属性，每个图形便会在名称下方增加小号明细行。字段按其所属的卡片类型分组列出；若某字段为其中多个类型共有，则归入**共用**。它与**着色依据**是彼此独立的按钮，这样查看任一列表都无需滚动越过另一个。
