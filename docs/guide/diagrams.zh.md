...
前两项勾选会绘制在图形上：卡片尺寸有限，放不下的文字会溢出边框。您勾选的全部内容都会被记住，因此之后扩大选择只会改变显示哪两项，无需重新勾选。

这些明细行随图表一起保存，因此所有读者——包括打开已发布链接的人——看到的图形完全一致。展开的子卡片只保留名称：它们太小，放不下更多内容。

[依赖关系报表](reports.md)中的**创建图表**按钮会带上它自己的卡片显示设置，因此由报表生成的图表打开时显示的正是报表所显示的那些行。
