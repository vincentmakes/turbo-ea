...
![按业务关键性着色卡片的图表编辑器](../assets/img/zh/99_diagram_editor.png)
...
- 在右侧栏按名称搜索；每行都有一个复选框。将筛选缩小到单个具有层级的类型后，列表会变成缩进的树，便于按分支查找卡片。
- 已勾选的卡片会以徽章形式显示在列表上方，并在你更改筛选或搜索时保持选中——点击其 × 可移除。
- **选择显示的全部**会勾选当前筛选留在屏幕上的所有内容；**插入所选项**将已选卡片以网格形式添加到画布。

同一对话框在单选模式下用于**更改链接的卡片**和**链接到现有卡片**。

画布上的每张卡片都会在左上角显示其**卡片类型图标**（一个白色小字形），紧邻类型颜色——因此卡片的类型同时通过图标和颜色来传达。这与整个应用中使用的图标一致，并改善了色盲用户的可读性。该图标会出现在此后插入的卡片上。若要为已在较早图表中的卡片添加图标，请在编辑器工具栏中点击**应用卡片类型图标**。如果卡片拥有自己的**徽标**，则改为显示徽标，并将卡片类型图标保留为角落的小角标——这样图形既说明了这是哪个产品，也说明了它是哪类卡片。徽标在打开图表时出现，并在徽标更换后刷新；没有徽标的卡片，以及管理员已关闭徽标的类型下的所有卡片，绘制方式与以往完全相同。同一菜单中的**卡片徽标**勾选项可将其关闭，以获得简洁的图示；默认开启。
...
工具栏中的**着色依据**下拉菜单为画布上的卡片重新着色：

- **卡片颜色**（默认）——每张卡片使用其卡片类型的颜色。
- **审批状态**——按「已批准」/「待审」/「已破损」重新着色。
- **字段值**——在画布上任意卡片类型下勾选一个单选字段。**多个卡片类型可以同时各带一条规则**——应用按关键性着色，*同时* IT 组件按托管模式着色。未设置规则的卡片类型保留其原有颜色，包括手动设置的填充；只有自身规则找不到取值的卡片才会变灰。在同一卡片类型内勾选第二个字段会替换第一个，因为一张卡片只有一种填充。

画布左下角的浮动图例为每条生效规则显示一组色阶。字段规则与**审批状态**是二选一而非叠加：选择其一会清除另一个。取消所有规则后，画布回到卡片颜色。该选择会随图表一起保存。在查看模式下打开图表以及通过其发布或嵌入链接查看时，也会显示同一图例，方便读者理解颜色的含义。

![查看模式下带颜色图例的图表](../assets/img/zh/99b_diagram_viewer_legend.png)

#### 在卡片上显示

工具栏中的第二个按钮**在卡片上显示**决定**每个图形显示什么内容**。勾选**卡片类型**、**子类型**或画布上现有卡片类型的任意属性，每个图形便会在名称下方增加小号明细行。字段按其所属的卡片类型分组列出；若某字段为其中多个类型共有，则归入**共用**。它与**着色依据**是彼此独立的按钮，这样查看任一列表都无需滚动越过另一个。**全部清除**可一次性取消所有勾选。

所有勾选项都会绘制在图形上，图形也会**随之变大**以容纳它们。卡片本身就容得下两行，因此在勾选第三项之前不会有任何变化；从第三项起，每多勾选一项卡片就略高一些，取消勾选则相应缩回。您手动调整过尺寸的卡片会保留您设定的高度：它只增减一行所需的空间。

这些明细行随图表一起保存，因此所有读者——包括打开已发布链接的人——看到的图形完全一致。通过**展开**引入画布的卡片与其他卡片一样显示相同的明细行，并同样会变大。而排布在**下钻**或**上卷**容器中的卡片只显示格位里放得下的内容：放大它们会压到下面一行。不过容器自身的标题栏会随其明细行一起变高，容器内的卡片也会随之下移，因此把一张卡片变成容器绝不会让它原本显示的内容消失。

[依赖关系报表](reports.md)中的**创建图表**按钮会带上它自己的卡片显示设置，因此由报表生成的图表打开时显示的正是报表所选择的那些行——全部显示，包括报表本身放不下的那些。
...
- **库存已变更**——自图表保存以来在库存中发生的变更，准备拉回画布。每一行提供对应的操作，「全部接受」可一次性解决所有行：
    - **重命名**的卡片——「接受更新」重写单元格标签；
    - **已删除**或**已归档**的卡片——「从图表中移除」将单元格（及其连线）从画布上移除；
    - **已删除的关系**——「从图表中移除连线」将过时的连线从画布上移除；
    - **流向**发生变更的关系——「接受更新」使箭头与资产清单保持一致。

![图表编辑器中的同步抽屉](../assets/img/zh/99a_diagram_sync_drawer.png)

每次打开图表时，Turbo EA 都会**自动检查库存变更**——工具栏 Sync 按钮上的蓝色徽章会统计待审查的变更数量。未经您确认不会应用任何更改；徽章只是提示您打开侧边栏。侧边栏中的「检查更新」按钮可随时重新运行同样的检查。
...
- **仅限登录的人员**——访问者通过你的身份提供商进行验证，可选择限制为指定的邮箱域。系统不会为他们创建 Turbo EA 账户。 在嵌入页面中，登录会在一个小弹出窗口中打开，且 Turbo EA 必须通过 HTTPS 提供服务，会话才能在框架内保持；访问者在嵌入该图表的每个站点上各登录一次。
...
!!! note "在嵌入页面中登录"
    以「仅限登录的人员」方式发布的图表会在嵌入页面中显示**登录**按钮。它会在一个小弹出窗口中打开你的身份提供商，窗口关闭后图表随即显示。有三件事可能造成阻碍：Turbo EA 必须通过 **HTTPS** 提供服务（浏览器只在安全来源上保留嵌入所需的跨站会话）；嵌入方页面必须允许其框架打开弹出窗口（没有 `allow-popups` 的沙盒框架会同时阻止弹出窗口和备用链接）；在 Safari 中，请使用登录页面提供的**在新标签页中打开图表**链接。另外，Chrome 会拒绝把来自私有网络地址（内部 IP 或主机名）的嵌入内容加载到 Confluence Cloud 等公共页面中——请将实例发布在公共 HTTPS 主机名上，或从同一网络内的 wiki 进行嵌入。
