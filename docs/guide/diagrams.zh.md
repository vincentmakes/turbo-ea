...
图表库将每个图表显示为一张紧凑的卡片，包含缩略图、名称、作者以及它引用的卡片数量。可在**卡片**视图和**列表**视图之间切换，并对任意图表进行**创建**、**打开**、**编辑详情**、整理或**删除**。

### 查找图表

- **筛选侧边栏** — 左侧栏可将图表库限定为**所有图表**、**我创建的**或您的**收藏**，并按图表**类型**（Data Flow / Free Draw）筛选。使用**筛选**按钮可将其折叠；在小屏幕上它会以滑出面板的形式打开。
...
将相关图表归入**分区**——工作区范围内共享的标签。一个图表可以同时属于多个分区。在卡片视图中，图表库将每个分区显示为可折叠的标题；未分配的图表显示在**未分组**下。

- 使用侧边栏中的**管理分区**来创建、重命名、改色或删除分区。
- 使用图表菜单中的**添加到分区…**将其放入一个或多个分区（可以即时创建新分区）。
- 在侧边栏中选择某个分区会将图表库筛选为仅该分区。
