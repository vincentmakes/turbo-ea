...
- **Путешествие во времени** — перетащите ползунок шкалы времени, чтобы увидеть ландшафт на любую дату. Карточки, которые ещё не начались, скрываются. При взгляде в будущее карточки, которые **появляются** за этот период, обводятся пунктиром и помечаются *ПЛАН*, а **выводимые** из эксплуатации остаются на диаграмме приглушёнными с меткой *ВЫВОД* — так запланированная трансформация показывает и то, что убирает, и то, что оставляет. Переключатель **Показывать выводимые карточки** на панели инструментов отключает их, оставляя только конечное состояние. На шкале отмечена каждая дата появления или вывода карточек отображаемой диаграммы; щелчок по метке переводит ползунок прямо к этому изменению. Ползунок действует во всех представлениях, а дата сохраняется вместе с отчётом.
