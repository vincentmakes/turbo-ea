...
- **Меню отображения карточки** — Включите метку **типа** и **точку статуса жизненного цикла**, включите **родительскую иерархию** (добавляет родительскую карточку каждой карточки и рисует связь принадлежности *содержит / часть*) и выберите **дополнительные поля атрибутов** для отображения на каждой карточке — первые два отображаются на карточке, а полный набор появляется во всплывающей подсказке при наведении. Выбранные настройки запоминаются между посещениями.
...
- **Перестановка** — Перетащите карточку, чтобы переместить её внутри своего уровня, или перетащите целый **блок уровня**, чтобы переместить его со всеми карточками. **Сбросить расположение** восстанавливает автоматическую компоновку.
