...
Риск также можно перенести из **риска проекта PPM**: на вкладке **Риски** инициативы PPM действие **Перенести в реестр рисков** создаёт риск ландшафта непосредственно из риска проекта — название, описание и владелец переносятся, оценки по шкале 1–5 преобразуются в словарь реестра, а карточка инициативы привязывается как затронутая. Текст мер смягчения превращается в разовую задачу по смягчению. Этот перенос также идемпотентен, а риск проекта остаётся полностью редактируемым в PPM — обе записи намеренно сохраняют независимые жизненные циклы.
