...
Таблицу реестра можно **группировать**: кнопка *Группировать по* на панели инструментов таблицы упорядочивает риски под сворачиваемыми заголовками по статусу, категории, начальному или остаточному уровню либо владельцу, с группой «Не задано» в самом верху — удобно проходить реестр группа за группой на встрече по обзору рисков. При прокрутке длинной группы её заголовок остаётся закреплённым под заголовками столбцов, поэтому вы не теряете из виду, в какой группе находитесь.
...
- Со страницы деталей риска: панель **Затронутые карточки** → выберите карточку и добавьте её. Щёлкните `×`, чтобы снять связь. Список открывается сразу для просмотра (вводить текст не нужно), подгружает следующие карточки при прокрутке и скрывает уже связанные.
...
Связанные карточки **группируются по типу карточки**, внутри каждой группы сортируются по алфавиту и окрашиваются в собственный цвет типа, поэтому риск с десятком карточек разных типов остаётся читаемым с первого взгляда. Тот же порядок и те же цвета применяются к столбцу **Карточки** в сетке реестра, где каждая карточка отображается чипом в цвете своего типа — подсказка ячейки перечисляет полный список под заголовками типов.
...
![Меры по снижению риска на странице риска](../assets/img/ru/97_risk_zadachi_snizheniya.png)
...
![Матрица рисков 4×4 в реестре рисков](../assets/img/ru/78_matritsa_riskov.png)
...
### Фильтрация по затронутым карточкам

В левой панели фильтров есть раздел **Затронутые карточки**, отвечающий на вопросы о ландшафте, которые не покрывает фильтр столбца:

- **Карточки** — выберите одну или несколько конкретных карточек, чтобы увидеть все риски, затрагивающие *любую* из них («показать все риски, затрагивающие приложения A, B и C»). Список предлагает каждую карточку, связанную хотя бы с одним риском, с поиском по названию.
- **Тип карточки** — отметьте один или несколько типов, чтобы увидеть все риски, затрагивающие *любую* карточку этих типов («показать все риски, затрагивающие приложение»), не называя отдельные карточки.

Каждый список срабатывает по **любому** из собственных значений; два списка объединяются через **и**. Отметив `Application` и выбрав карточку *Payments Gateway*, вы получите риски, которые затрагивают Payments Gateway **и** касаются хотя бы одного приложения.

Как и любой другой фильтр панели, они сужают всю страницу — плитки KPI, матрица 4×4, сетка и оба листа экспорта `.xlsx` следуют активному выбору.
