...
- **关联到子项的卡片** —— 当卡片存在子项时，每个关系分组会显示 **子项中另有 N 个** 标签，用于统计层级更下方所关联的卡片——例如挂在某能力的子能力下的应用。点击可打开只读列表，每一行都会标明持有该关联的子项（通过多个子项可达的卡片只显示一次，并列出全部来源）。该计数仅包含上方分组中尚未列出的卡片。如需修改关联，请打开持有该关联的子项。
