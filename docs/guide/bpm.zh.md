...
组织职责可在流程层面以两种互补的方式记录：

- **泳道分配** — 将 BPMN 泳道一次性绑定到组织卡片，可通过步骤表上方的*泳道分配*面板，或在建模器中选择泳道来完成。泳道中的每个步骤都会自动继承该组织。
- **按步骤覆盖** — 步骤表中的*组织*列可将单个步骤链接到不同的组织。继承的组织显示为描边样式的标签；显式覆盖显示为实心标签并具有优先权。

泳道名称在图表中仍是自由文本 —— 将泳道绑定到组织卡片是可选的。步骤的有效组织始终是计算得出的（若设置了覆盖则使用覆盖，否则使用泳道绑定），因此无需重复维护，泳道绑定与步骤链接也绝不会相互矛盾。链接的组织会创建业务流程 → 组织关系，在两张卡片和依赖关系报告中均可见。
