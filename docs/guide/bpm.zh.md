...
误批的审批可以撤销，无需删除整个流程。该功能**默认关闭**；管理员可在[设置](../admin/settings.md)的「受控流程发布」中启用。

启用并授予权限后，已发布的版本会出现「撤回」按钮。撤回时需要填写书面理由，然后：

- 版本状态变为**已撤回**——绝不会被删除，也不会退回草稿；
- 原有审批记录保留：审批人和审批时间依然可见；
- 记录撤回人、撤回时间和理由——同时出现在版本历史和卡片的**历史**选项卡中；
- 在新版本提交并获批之前，该流程将**没有已审批的流程图**；
- 已提取的流程步骤及其卡片关联保持不变。

已归档的版本不会自动恢复。重新发布较旧的版本本身就是一次审批：请基于它创建新草稿，提交并获得审批。
