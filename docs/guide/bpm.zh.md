...
- 点击步骤的**应用**、**数据对象**或 **IT 组件**单元格并选择卡片 —— 选择器会浏览清单，无需手动输入
- 链接保存在步骤上，并在流程与卡片之间创建关系，因此在流程图和卡片的「关系」标签页中都可见
- **业务流程**列将步骤链接到它交接至的流程 —— 见下文
- 草稿打开期间，**编辑器中**同样提供这五个关联：属性面板中的**关联卡片**分组，以及上下文菜单中打开其列表的**关联卡片**项
- 在流程图本身上，已关联的步骤会在名称下方显示**每种已关联卡片类型一个小圆点**，颜色即该卡片类型的颜色，编辑器与只读查看器均如此。画布上不显示名称：点击步骤即可查看，或将鼠标悬停在圆点上

### 将步骤链接到流程

步骤常常交接给一个独立存在的流程 —— 该流程有自己的图表、负责人和生命周期，通常在多处复用。任何步骤都可以表达这一点：任务、子流程、事件或网关都能指向一张**业务流程**卡片，您永远不必手动输入流程 ID：

- **属性面板**在每个步骤上显示**关联卡片**分组，每个关联一行 —— 业务流程、应用、数据对象、IT 组件、组织 —— 每行都有**选择**、**打开**（下钻到关联卡片）和**清除**
- 选中步骤的**上下文菜单**带有列出这五项的**关联卡片**项，便于在面板折叠时使用。数据对象和数据存储与表格中一样，只提供数据对象关联
- 已发布流程的**步骤表格**（以及草稿的预链接表格）在**业务流程**列中有同样的链接，点击那里的标签会下钻到关联流程的「流程图」标签页。数据对象和数据存储不是步骤，因此其行显示为短横线

![关联卡片](../assets/img/zh/92_bpm_called_process.png)

BPMN 中有一种本身*就是*另一个流程的构造：**调用活动**（call activity），一个带粗边框的任务，用于调用独立定义的流程。内嵌**子流程**也能对步骤分组，但它属于所在的图表；Method & Style 的规则很简单：若流程独立存在，就使用调用活动。Turbo EA 将其视为原生情形 —— **放置调用活动时会询问它调用哪个流程**，链接保存在 BPMN 自带的*被调用元素*中，其他工具也能读取。其他步骤则将链接保存为图中的 Turbo EA 属性。

发布包含已链接步骤的流程会在两个流程之间创建「调用」关系 —— 关联流程的「关系」标签页显示*被调用*，依赖视图会绘制调用图。从其他工具导入的图表会保留该工具自身的流程引用；步骤表格将其显示为提示（*引用 Process_X*），直到您在 Turbo EA 中选择对应的流程。

### 同一套关联

编辑器与表格显示同一套关联，因此同一步骤在任何位置读到的内容都一致：

- 在**草稿**内，您在其中所设的内容优先 —— 编辑器与**预链接元素**表格是同一存储 —— 而图中自带的流程引用只作为您未作说明的步骤的回退。清除关联即为清除，发布时同样生效。
- **已发布**的流程在其元素表格中编辑关联时仍保持已审批状态：这是在已签署图表之上的元数据，而不是重新审批的理由。
- **从已发布版本创建**的草稿以流程当时持有的关联为起点。已经打开的草稿保留自己的关联，因此他人的编辑不会改变您正在处理的图表。
