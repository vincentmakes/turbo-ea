...
步骤表中的*组织*列可将步骤链接到组织卡片，与应用程序 / 数据对象 / IT 组件并列：

- **泳道中的步骤** — BPMN 泳道中的所有步骤共享同一个组织：在任意步骤上链接、更改或移除组织都会应用于整个泳道。也可以在建模器中选择泳道来设置同一绑定。
- **无泳道的步骤** — 组织将链接到该单个步骤。

泳道名称在图表中仍是自由文本 —— 将泳道链接到组织卡片是可选的，现有图表不受影响。链接的组织会创建业务流程 → 组织关系，在两张卡片和依赖关系报告中均可见。若要让多个组织参与同一流程，请为每个组织使用一条泳道 —— 这正是 BPMN 泳道的用途。
