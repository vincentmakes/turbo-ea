...
| **范围** | 必填。选择**特定卡片**——将发现限定到单个应用或 IT 组件——或选择**整个全景**，用于没有任何单个卡片归属的、覆盖整个资产的系统性差距（例如缺失的组织级流程）。全景发现在**卡片**列中以绿色显示。 |
| **关联风险** | 可选 — 如果已有一个风险在跟踪该差距，预先关联。 |

每个发现都必须限定到特定卡片或整个全景——不存在含糊的「无卡片」状态。全景范围是一个正式选项，而不是空白：AI 扫描同样会为跨越整个资产的系统性控制生成全景级发现。

创建、编辑、撤回或批量操作发现需要 `compliance.manage`。读取登记册以及在卡片级合规标签页中分诊只需 `compliance.view`。

### 编辑发现

合规网格中的每一行都带有一个内联**编辑**（铅笔）图标，一键即可打开发现进行编辑。你也可以从合规网格或卡片的**合规**标签页打开任意发现，在抽屉中点击**编辑**，即可在创建之后更改其合规**状态**（例如「合规」→「部分合规」）、严重程度、要求、差距、证据、整改、条款或关联卡片。编辑内容不会改变发现的生命周期决策；如需更改请使用生命周期时间线。需要 `compliance.manage`。
