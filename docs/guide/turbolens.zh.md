...
| **依赖关系图** | 交互式 [Layered Dependency View](reports.md#layered-dependency-view)（分层依赖视图），显示现有节点与建议节点（虚线边框，带绿色「NEW」徽章）。可平移、缩放并以可视化方式探索架构 |
...
合规扫描器是一项 TurboLens 分析，针对已启用的法规生成合规发现。发现、生命周期、手工撰写路径、提升到风险的流程和批量操作都已记录在专用的[**合规指南**](compliance.md)中 —— 只有扫描触发按钮本身位于 TurboLens 开关之后。
...
- 分析类型（供应商分析、供应商解析、重复项检测、现代化、架构师、compliance）
- 状态（运行中、已完成、失败）
- 开始和完成时间戳
- 错误信息（如有）
