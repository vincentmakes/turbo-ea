...
- **Adicionar relacionamento** — Clique em **+** para criar um novo relacionamento; o seletor lista os cartões correspondentes assim que abre (as melhores correspondências para o que você digita aparecem primeiro, e mais são carregados ao rolar). Os cartões já ligados ficam ocultos da lista, com uma legenda indicando quantos são. O seletor permanece aberto após cada escolha, permitindo adicionar vários relacionamentos seguidos, e cada cartão adicionado aparece como uma etiqueta logo acima do campo de busca — clique no **×** de uma etiqueta para desfazer essa adição. **Concluído** fecha a linha
