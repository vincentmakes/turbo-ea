...
- **搜索** —— 按卡片名称进行自由文本搜索。当管理员启用[语义搜索](../admin/ai.md#semantic-search-embeddings)后，搜索框上方会出现一个**文本 / 语义**切换开关：**语义**模式按含义查找卡片（例如「面向客户的支付系统」会匹配名为「NexaPay Gateway」的卡片），而非精确措辞。语义搜索在你输入完成后执行；如果嵌入提供程序不可用，则回退到文本匹配。
