...
- **筛选栏** —— 对 id、名称、描述和别名的全文搜索，外加层级标签（Macro → L1 → L4）、行业多选器，以及「显示已弃用」开关。页面滚动时会固定在顶部导航栏正下方。
...
在 L1 / L2 / L3 / L4 层级之上，目录还附带一层 **Macro** —— 一组业务层面的归类，将整个 L1 家族框定在内。例如 *Customer Engagement*（框定销售、营销、服务等 L1）或 *Talent & Workforce*（框定人力资源相关 L1）。

宏是一等的目录条目：

- 它们以 `BusinessCapability` 卡片的形式落入你的清单，`attributes.capabilityLevel = "Macro"`，`catalogueId` 以 `MC-` 为前缀（例如 `MC-10`）。
- 它们位于其 L1 子项**之上** —— 为容纳额外的一层，层级深度上限从 5 放宽到 6（`Macro → L1 → L2 → L3 → L4 → L5`）。
- 导入宏时，任何被标记为属于该宏的现有 L1 子项会自动被重新挂载到新卡片下 —— 与 L1 和下层之间的双向链接相同。
- **宏从不按名称与现有卡片匹配** —— 仅按 `catalogueId`。这避免了与恰好与某个目录宏共享标签的客户自命名能力分组发生意外冲突。

宏在目录页面上与 L1 一样可选择 —— 勾选复选框，子树相应被选中。
