...
Каждая строка также несёт цветную иконку происхождения и акцентную полосу, поэтому смешанные списки читаются с первого взгляда. Задача, которую расширение-коннектор зеркалирует во внешний трекер (Jira, GitLab, …), сохраняет своё настоящее происхождение и показывает внешний идентификатор (например, *KAN-6*) в виде небольшой ссылки — зеркало служит только для справки, а задача всегда завершается в Turbo EA. О коннекторе Jira см. [Jira Todo Sync](../extensions/jira-todos.md).
