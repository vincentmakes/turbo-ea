...
- **Aggiungi relazione** — Cliccate su **+** per creare una nuova relazione; il selettore elenca le carte corrispondenti già all'apertura (le corrispondenze migliori con quanto digitate compaiono per prime, altre si caricano scorrendo). Le carte già collegate sono nascoste dall'elenco, con una didascalia che ne indica il numero. Il selettore resta aperto dopo ogni scelta, così potete aggiungere più relazioni di seguito, e ogni carta aggiunta compare come chip appena sopra il campo di ricerca: cliccate la **×** di un chip per annullare quell'aggiunta. **Fatto** chiude la riga
