...
- **Aggiungi relazione** — Cliccate su **+** per aprire la finestra **Aggiungi relazioni**. Elenca le carte corrispondenti mentre digitate (le corrispondenze migliori per prime, altre si caricano scorrendo) e nasconde quelle già collegate, con una didascalia che ne indica il numero. Cliccando una carta la collegate subito e compare come chip in alto: cliccate la **×** di un chip per annullare quell'aggiunta. La finestra resta aperta per aggiungerne quante volete, e il selettore **Tipo di relazione** in alto consente di cambiare tipo senza chiuderla. Su smartphone si apre a schermo intero
