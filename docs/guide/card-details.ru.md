...
- **Псевдоним** — второе имя карточки; отображается под заголовком, если задано. Карандаш редактирования у заголовка позволяет изменить название и псевдоним вместе. Псевдонимы учитываются в каждом поле поиска, поэтому карточку можно найти по имени, которым её действительно называют в вашей организации.
...
Карточки типа, для которого это разрешено, могут иметь собственный
**логотип** вместо общего значка типа — тогда приложение для SAP, Kafka или
Jira показывает знак самого продукта. Узнаваемые логотипы делают просмотр
реестра значительно быстрее, особенно для тех, кто его читает, а не ведёт.

Наведите указатель на значок в левом верхнем углу карточки и щёлкните по нему,
чтобы **загрузить**, **заменить** или **удалить** изображение. Значок типа при
этом не исчезает: он превращается в небольшую отметку в углу логотипа, так что
по-прежнему сразу видно, карточка какого типа перед вами.

- **Допустимые форматы** — PNG, JPEG, WebP или GIF размером до 1 МБ. Формат SVG
  не принимается, поскольку может содержать сценарии.
- **Где отображается** — в заголовке карточки, в необязательном столбце
  **Логотип** в [Инвентаре](inventory.md) и в любом опубликованном
  веб-портале, построенном на этом типе карточек.
- **Если логотипа нет** — карточка возвращается к значку своего типа, как и
  раньше.

Логотипы доступны для тех типов карточек, которые включил администратор; по
умолчанию это «Приложение» и «ИТ-компонент». См.
[Метамодель](../admin/metamodel.md).

Щёлкните логотип и выберите **Выбрать значок бренда…**, чтобы взять его из
встроенного набора в несколько тысяч марок: найдите продукт по имени и выберите
его — файл изображения не нужен. **Загрузить** позволяет использовать
собственный файл. ИИ-ассистент, подключённый по [MCP](../admin/mcp.md), может
задавать логотипы тем же способом массово, а если продукта в наборе нет —
находит марку сам.

То же меню доступно из столбца **Логотип** в [Инвентаре](inventory.md) —
наведите указатель на ячейку логотипа и щёлкните, — так что знаки можно
проставить на только что импортированном ландшафте, не открывая каждую
карточку. Это намеренно делается по одной карточке: логотип не предлагается ни
для заполнения перетаскиванием, ни для массового редактирования.
...
Когда утверждённая карточка редактируется, её статус автоматически меняется на **Нарушено**, чтобы указать на необходимость повторной проверки. Архивирование родительской карточки даёт тот же результат для утверждённых дочерних карточек, которые при этом перемещаются в иерархии. Уведомление получают все, у кого есть роль заинтересованной стороны на карточке, — кроме того, кто внёс изменение, — а сам переход фиксируется на вкладке **История** карточки. Одно действие, нарушающее сразу несколько карточек, например массовое редактирование, отправляет каждому одну сводку, а не по уведомлению на каждую карточку.
...
- **Дополнительные поля описания** — администратор может отнести к секции описания любое собственное поле типа карточки, поэтому здесь могут быть поля помимо самого описания
- **Ссылки** — веб-адрес (`http://` или `https://`), введённый в описание, любое текстовое поле, комментарий или задачу, становится ссылкой, которая открывается в новой вкладке
...
Если дата фазы позже даты фазы, которая должна идти после неё, — например, **Вывод** позже **Конца жизни**, — рядом с этой датой появляется значок предупреждения, а при редактировании то же предупреждение показывается под полем. Это лишь подсказка: даты всё равно можно сохранить в том виде, в каком они введены.
...
- **Тип иерархической связи** — если администратор настроил типы связей для этого типа карточек, каждую связь «родитель — потомок» можно обозначить: например, одну дочернюю компанию как *коммерческую*, а другую как отвечающую за *продажи*. Обозначение принадлежит дочерней карточке: родительская карточка показывает его рядом с каждым потомком, а дочерняя — рядом со своим родителем. Если карточку убрать или перенести на верхний уровень, обозначение снимается, а изменение фиксируется на вкладке **История**.
...
- **Добавить связь** — нажмите **+**, чтобы открыть диалог этой связи. По мере ввода он показывает подходящие карточки (наиболее точные совпадения идут первыми, остальные подгружаются при прокрутке) и скрывает уже связанные, а подпись показывает их количество. Щелчок по карточке сразу создаёт связь, и карточка появляется меткой сверху — нажмите **×** на метке, чтобы отменить это добавление. Диалог остаётся открытым, поэтому можно добавить сколько угодно, а на телефоне он открывается на весь экран. Связи без собственного раздела доступны через кнопку **Добавить связь** внизу раздела. Если связываемый тип иерархический (бизнес-способность, организация и подобные), диалог показывает его в виде дерева с отступами, а не алфавитного списка, поэтому подспособность можно выбрать по её ветке. При поиске родители найденной карточки остаются видимыми для контекста, а карточки, которые нельзя выбрать (уже связанные, только что добавленные или сама карточка), остаются на месте серым цветом, чтобы уровни вокруг читались правильно.
- **Порядок** — связанные карточки перечислены по алфавиту
...
- **Карточки, связанные с дочерними элементами** — если у карточки есть дочерние элементы, в каждой группе связей появляется чип **+N в дочерних элементах** со счётчиком карточек, связанных ниже по иерархии, — например приложений, привязанных к подспособностям способности. По клику открывается список только для чтения, где в каждой строке указан дочерний элемент, которому принадлежит связь (карточка, доступная через несколько дочерних элементов, показывается один раз со всеми источниками). В счётчик попадают только карточки, которых ещё нет в группе выше. Чтобы изменить связь, откройте владеющий ею дочерний элемент. Список разбит на сворачиваемые разделы по подтипу, поэтому подтип указывается один раз в заголовке раздела, а не в каждой строке. Внутри раздела первыми идут карточки, чья фаза жизненного цикла требует внимания (снятые с поддержки, затем выводимые из эксплуатации), а фаза каждой карточки показана цветной точкой рядом с названием — наведите на неё курсор, чтобы увидеть название фазы.

![Группы связей с чипом дочерних элементов](../assets/img/ru/59_kartochka_podelementy_chip.png)

![Карточки, связанные через дочерние элементы, сгруппированные по подтипу](../assets/img/ru/60_kartochka_podelementy_svyazi.png)

### Секция зависимостей

[Layered Dependency View](reports.md) карточки и всего, что находится на расстоянии одной связи, сгруппированное по четырём архитектурным слоям. Shift-щелчок по карточке заново центрирует вид, позволяя обходить ландшафт не покидая страницу.

Значок **открыть в новой вкладке** на панели инструментов открывает полный [отчёт «Зависимости»](reports.md) в новой вкладке с центром на той карточке, на которой вид центрирован в этот момент, — то есть на той, к которой вы перешли, а не обязательно на исходной. Пригодится, когда нужно то, что отчёт добавляет вокруг той же картины: путешествие во времени, метки переходов, табличное представление и сохранение вида как отчёта.
...
- **Вложенные файлы** — загружайте и управляйте файлами (документы, таблицы, презентации, файлы OpenDocument и прежних версий Office, изображения, архивы, сообщения Outlook и электронной почты, CSV, Markdown, JSON и XML — до 20 МБ). При загрузке выберите **категорию документа** из: Архитектура, Безопасность, Соответствие, Операции, Протоколы встреч, Дизайн или Прочее. Категория отображается в виде чипа рядом с каждым файлом.
...
- **Ссылки** — веб-адрес в комментарии кликабелен и открывается в новой вкладке
...
Если для роли в метамодели задан **цвет**, её группа отмечается им, так что владельца можно с первого взгляда отличить от наблюдателя.
...
Здесь отображается всё, что сдвигает дату **Изменено** карточки: ручное редактирование, импорт из таблицы, миграция платформы или синхронизация с ServiceNow, изменение тега, массовое редактирование или перемещение по иерархии, затронувшее эту карточку. Системное обслуживание не меняет ни того, ни другого: пересчёт показателей качества данных, повторный расчёт вычисляемых полей и заполнение уровней иерархии или идентификаторов карточек оставляют историю и дату **Изменено** нетронутыми.

## Вкладка «ADR»

На каждой карточке есть вкладка **ADR** со списком [архитектурных решений](delivery.md), привязанных к ней: номер, название, статус, все связанные карточки и дата последнего изменения. Щёлкните строку, чтобы открыть решение.

Если вам разрешено управлять привязками ADR, на вкладке также доступны **Привязать ADR** (прикрепить существующее решение) и **Создать ADR** (создать новое, сразу привязанное к этой карточке), а в каждой строке — действие отвязки. На карточках без привязанных решений вкладка скрыта, если у вас нет этого права, поэтому пользователи с доступом только на чтение никогда не видят пустую вкладку.
