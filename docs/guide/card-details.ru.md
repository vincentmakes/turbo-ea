...
- **Добавить связь** — нажмите **+**, чтобы создать новую связь; список подходящих карточек появляется сразу при открытии (наиболее точные совпадения с введённым текстом идут первыми, остальные подгружаются при прокрутке). Уже связанные карточки скрыты из списка, а подпись показывает их количество. После каждого выбора список остаётся открытым, поэтому можно добавить несколько связей подряд, и каждая добавленная карточка появляется в виде метки прямо над полем поиска — нажмите **×** на метке, чтобы отменить это добавление. Кнопка **Готово** закрывает строку
