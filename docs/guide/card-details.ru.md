...
- **Тип связи** — если администратор настроил типы связей для этого типа карточек, каждую связь «родитель — потомок» можно обозначить: например, одну дочернюю компанию как *коммерческую*, а другую как отвечающую за *продажи*. Обозначение принадлежит дочерней карточке: родительская карточка показывает его рядом с каждым потомком, а дочерняя — рядом со своим родителем. Если карточку убрать или перенести на верхний уровень, обозначение снимается, а изменение фиксируется на вкладке **История**.
