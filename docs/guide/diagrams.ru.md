...
![Редактор диаграмм с карточками, окрашенными по бизнес-критичности](../assets/img/ru/99_redaktor_diagramm.png)
...
Плавающая легенда в левом нижнем углу показывает по одной шкале на каждое активное правило. Правила по полям и **Статус одобрения** — альтернативы, а не слои: выбор одного очищает другое. Если снять все правила, холст вернётся к цветам карточек. Выбор сохраняется вместе с диаграммой. Та же легенда отображается и при открытии диаграммы в режиме просмотра, а также по её опубликованной или встроенной ссылке, чтобы читатели понимали значение цветов.

![Диаграмма в режиме просмотра с цветовой легендой](../assets/img/ru/99b_diagramma_prosmotr_legenda.png)
...
![Панель синхронизации в редакторе диаграмм](../assets/img/ru/99a_diagramma_sinkhronizatsiya.png)
...
- **Только вошедшие в систему** — посетители проходят проверку у вашего поставщика удостоверений, при желании с ограничением по почтовым доменам. Учётная запись Turbo EA для них не создаётся. Во встроенном режиме вход открывается в небольшом всплывающем окне, а Turbo EA должен работать по HTTPS, чтобы сеанс сохранялся внутри фрейма; посетители входят один раз для каждого сайта, в который встроена диаграмма.
...
!!! note "Вход внутри встраивания"
    Диаграмма, опубликованная для *Только вошедшие в систему*, показывает во встраивании кнопку **Войти**. Она открывает вашего поставщика удостоверений в небольшом всплывающем окне, и диаграмма появляется, как только это окно закрывается. Помешать могут три вещи: Turbo EA должен работать по **HTTPS** (браузеры сохраняют межсайтовый сеанс, необходимый встраиванию, только на защищённом источнике); встраивающая страница должна разрешать всплывающие окна из своих фреймов (фрейм в песочнице без `allow-popups` блокирует и всплывающее окно, и запасную ссылку); а в Safari используйте ссылку **Открыть диаграмму в новой вкладке**, которую предлагает экран входа. Кроме того, Chrome отказывается загружать встраивание с адреса частной сети (внутренний IP или имя хоста) в публичную страницу, такую как Confluence Cloud, — опубликуйте экземпляр на публичном HTTPS-имени хоста или встраивайте его из вики в той же сети.
