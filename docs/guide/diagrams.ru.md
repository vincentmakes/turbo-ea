...
То же выпадающее меню определяет и то, **что говорит каждая фигура**. В разделе **Показывать на карточке** отметьте **тип карточки**, **подтип** или любой атрибут типов карточек, присутствующих на холсте, — и каждая фигура получит небольшие строки подробностей под своим именем. Выпадающее меню разделено надвое: **Цвет по** выбирает единственный атрибут, который окрашивает фигуры, а **Показывать на карточке** — сколько угодно полей для печати на них. Поля перечислены под тем типом карточки, которому принадлежат; поле, общее для нескольких таких типов, собрано под заголовком **Общие**.

Первые два выбранных пункта рисуются на фигуре: карточка ограничена по размеру, и не помещающийся текст вышел бы за её границу. Всё отмеченное запоминается, поэтому расширение выбора позже меняет лишь то, какие два пункта показываются, — переотмечать ничего не нужно.

Эти строки сохраняются вместе с диаграммой, поэтому все читатели — включая тех, кто открывает опубликованную ссылку, — видят одни и те же фигуры. Раскрытые дочерние карточки сохраняют только имя: они слишком малы для чего-то большего.

Кнопка **Создать диаграмму** в [отчёте о зависимостях](reports.md) переносит собственные настройки отображения карточек, поэтому диаграмма, созданная из отчёта, открывается ровно с теми строками, которые показывал отчёт.
