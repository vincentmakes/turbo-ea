...
| **النطاق** | مطلوب. اختر **بطاقة محددة** — لتحديد نطاق النتيجة لتطبيق واحد أو مكوّن تقني — أو **المشهد بالكامل** لفجوة منهجية على مستوى الحيازة بأكملها لا تملكها أي بطاقة بعينها (مثل إجراء مفقود على مستوى المؤسسة). تُعرض نتائج المشهد باللون الأخضر في عمود **البطاقة**. |
| **المخاطرة المرتبطة** | اختياري — اربط مسبقًا بمخاطرة موجودة إن كانت تتتبع هذه الفجوة بالفعل. |

يجب أن يكون لكل نتيجة نطاق محدد إما لبطاقة بعينها أو للمشهد بالكامل — لا توجد حالة غامضة «بدون بطاقة». نطاق المشهد خيار كامل الأهلية وليس حقلاً فارغاً: تُنتج عمليات الفحص بالذكاء الاصطناعي أيضاً نتائج على مستوى المشهد لضوابط منهجية تمتد عبر كامل الحيازة.
...
افتح أي نتيجة — من شبكة الامتثال أو من علامة تبويب **الامتثال** في بطاقة — وانقر على **تعديل** في اللوحة لتغيير **حالة** الامتثال الخاصة بها (على سبيل المثال متوافق ← جزئي) أو الخطورة أو المتطلب أو الفجوة أو الدليل أو المعالجة أو المادة أو البطاقة المرتبطة بعد إنشائها. لا يؤثر تحرير المحتوى على قرار دورة حياة النتيجة؛ استخدم المخطط الزمني لدورة الحياة لذلك. يتطلب `compliance.manage`.
...
![تفاصيل البطاقة — علامة تبويب الامتثال](../assets/img/ar/56_card_compliance_tab.png)
