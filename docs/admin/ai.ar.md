...
![إعدادات الذكاء الاصطناعي](../assets/img/ar/26_admin_settings_ai.png)
...
| **Amazon Bedrock** | حساب AWS الخاص بك | منطقة AWS + معرّف النموذج أو ملف تعريف الاستدلال؛ دور IAM، أو مفتاح وصول اختياري |

يتطلب المزوّدون التجاريون مفتاح API، يُخزَّن مشفّرًا في قاعدة البيانات باستخدام تشفير Fernet المتماثل. ويُستثنى من ذلك Amazon Bedrock: فهو يستخدم افتراضيًا دور IAM الخاص بالحاوية للمصادقة، فلا يوجد مفتاح يُخزَّن أو يُبدَّل.
...
1. انتقل إلى **Settings > AI Suggestions** في واجهة المسؤول.
2. حدّد مزوّدك (OpenAI أو Google Gemini أو Azure OpenAI أو OpenRouter أو Anthropic Claude). أمّا Amazon Bedrock فانظر الخيار د أدناه.
...
### الخيار د: Amazon Bedrock

تعمل النماذج داخل حساب AWS الخاص بك، فلا تغادر المطالبات حدود الحساب أبدًا، ولا يوجد مفتاح API لطرف ثالث يحتاج إلى إدارة.

**1. فعّل الوصول إلى النماذج** في وحدة تحكّم AWS Bedrock ضمن **Model access** للمنطقة التي تنوي استخدامها. يُمنح الوصول لكل نموذج على حدة؛ ويطلب بعض المورّدين وصفًا لمرّة واحدة لحالة الاستخدام.

**2. امنح الأذونات.** يستخدم Turbo EA دور IAM الخاص بالحاوية (دور مهمة ECS أو حساب خدمة EKS أو ملف تعريف مثيل EC2). أرفق به هذه السياسة:
...
1. انتقل إلى **Settings > AI** في واجهة المسؤول.
2. حدّد **Amazon Bedrock** كمزوّد.
3. أدخل **منطقة AWS** — على سبيل المثال `eu-central-1`. يتوقّع هذا الحقل منطقة وليس عنوان URL.
4. اترك حقل **مفتاح API** فارغًا لاستخدام دور IAM. وخارج AWS أدخل بدلًا من ذلك `ACCESS_KEY_ID:SECRET_ACCESS_KEY`؛ ويُشفَّر قبل التخزين مثل أي مفتاح مزوّد آخر.
5. انقر **Test Connection**. ستُعرض كل النماذج وملفات تعريف الاستدلال التي توفّرها المنطقة.
6. اختر **النموذج** من القائمة وانقر **Save**.

!!! warning "النماذج الأحدث تحتاج إلى معرّف ملف تعريف استدلال"
    لا يمكن استدعاء النماذج الحديثة — ومنها Claude Sonnet 4 — بمعرّف النموذج البسيط. يردّ Bedrock بخطأ `ValidationException` يفيد بأن الإنتاجية عند الطلب غير مدعومة. استخدم بدلًا من ذلك **معرّف ملف تعريف الاستدلال** الإقليمي الذي يحمل بادئة جغرافية مثل `eu.` أو `us.` أو `apac.`: `eu.anthropic.claude-sonnet-4-20250514-v1:0`. ولهذا السبب يعرض اختبار الاتصال معرّفات ملفات التعريف أولًا.
---
...
ترك **جميع** أنواع البطاقات دون تحديد لا يعني تعطيل الميزة، بل يعني *عدم وجود قيد حسب النوع*، لذا يظهر زر الاقتراح على جميع أنواع البطاقات.
...
![لوحة اقتراحات الذكاء الاصطناعي في تفاصيل البطاقة](../assets/img/ar/27_ai_suggest_panel.png)
...
- **حساب AWS الخاص بك**: مع Amazon Bedrock يجري الاستدلال داخل حساب AWS الخاص بك. تبقى المطالبات ضمن حدود الحساب وتخضع لسياسات التحكّم في الخدمة الخاصة بك.
...
| يُرجِع Bedrock «AccessDeniedException» | سببان محتملان: إمّا أن دور IAM يفتقر إلى إذن `bedrock:InvokeModel`، أو أن الوصول إلى ذلك النموذج غير مفعّل في وحدة تحكّم Bedrock. تحقّق من كليهما. |
| يذكر Bedrock أن الإنتاجية عند الطلب غير مدعومة | لا يمكن الوصول إلى النموذج إلا عبر ملف تعريف استدلال إقليمي. شغّل اختبار الاتصال واختر معرّفًا يبدأ بـ `eu.` أو `us.` أو `apac.`. |
