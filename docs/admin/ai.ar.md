...
## البحث الدلالي (التضمينات) { #semantic-search-embeddings }

يتيح البحث الدلالي للمستخدمين العثور على البطاقات حسب **المعنى** بدلًا من الصياغة الحرفية — فالبحث عن «أنظمة الدفع الموجَّهة للعملاء» يُظهِر بطاقة باسم «NexaPay Gateway» قد يفوتها البحث النصّي البسيط. وهو يشغّل مفتاح **دلالي** في مربع بحث المخزون وأداة MCP المسمّاة `semantic_search_cards`.

يستخدم خلف الكواليس **نموذج تضمين** (نموذجًا صغيرًا لتحويل النص إلى متجه)، وليس نموذج محادثة لغويًا كبيرًا، ويدمج الترتيب القائم على المعنى مع مطابقة السلسلة الجزئية القائمة. ولأن التضمينات قدرة مستقلة — ولأن Anthropic، وهي مزوّد محادثة مدعوم، لا تملك واجهة برمجة تطبيقات للتضمينات — يُضبَط مزوّد التضمين **بشكل منفصل** عن مزوّد الذكاء الاصطناعي للمحادثة أعلاه.

### تفعيل البحث الدلالي

1. انتقل إلى **الإدارة ← الإعدادات ← الذكاء الاصطناعي ← البحث الدلالي (التضمينات)**.
2. فعّل **البحث الدلالي مُفعَّل**.
3. اختر **مزوّد تضمين**:
   - **Ollama** (افتراضي، مستضاف ذاتيًا، يعمل دون اتصال): اضبط عنوان URL للمزوّد (مثل `http://ollama:11434`) ونموذجًا يُخرِج متجهات بـ 768 بُعدًا — و`nomic-embed-text` هو الافتراضي المُوصى به. نزّله باستخدام `ollama pull nomic-embed-text`.
   - **متوافق مع OpenAI**: اضبط عنوان URL للمزوّد ومفتاح API؛ استخدم `text-embedding-3-small` (أو `-large`)، الذي يطلبه Turbo EA بـ 768 بُعدًا.
   - **Azure OpenAI**: اضبط عنوان URL للمورد ومفتاح API واسم نشر التضمينات.
4. انقر على **اختبار** للتحقق من الاتصال — يبلّغ الاختبار الناجح عن بُعد المتجه (768).
5. انقر على **حفظ**.

> يجب أن يُخرِج نموذج التضمين متجهات بـ **768 بُعدًا**. يفعل `nomic-embed-text` ذلك أصلًا؛ أما نماذج OpenAI من فئة `text-embedding-3-*` فتُطلَب تلقائيًا بـ 768.

### كيف تبقى التضمينات محدّثة

تُولَّد تضمينات البطاقات وتُحدَّث تلقائيًا بواسطة مهمة تعمل في الخلفية — فلا حاجة إلى تشغيل أي شيء يدويًا. وعند تفعيل الميزة لأول مرة، فإنها **تُعبّئ رجعيًا** المخزون الحالي (قد يستغرق ذلك من بضع دقائق إلى بضع ساعات على مخزون كبير مع Ollama يعمل على وحدة المعالجة المركزية فقط، ويعمل بهدوء في الخلفية). وبعد ذلك، تُعاد تضمين البطاقات الجديدة والمعدَّلة خلال دقيقة تقريبًا. وإذا غيّرت نموذج التضمين، يُعاد تضمين كل بطاقة تلقائيًا.

تحترم النتائج الدلالية **الأذونات نفسها وإخفاء حقول التكلفة نفسه** المطبَّقة على قائمة البطاقات العادية — فلا يرى المستخدم أبدًا في النتائج الدلالية بطاقةً لا يمكنه رؤيتها في المخزون. وإذا كان مزوّد التضمين غير متاح، يعود البحث بشفافية إلى مطابقة السلسلة الجزئية.
