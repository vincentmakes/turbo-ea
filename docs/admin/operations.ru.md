...
Вместо того чтобы следить за репозиторием, пусть экземпляр сообщит сам: Turbo EA ежедневно проверяет наличие более новой версии и уведомляет администраторов через колокольчик уведомлений, указывая номер версии и ссылку на заметки о выпуске. Он никогда ничего не загружает и не устанавливает — шаг 1 выше по-прежнему начинается с чтения журнала изменений. Проверка включена по умолчанию и отключается в разделе [Администрирование → Настройки → Общие](settings.md#update-notifications); отключение также убирает исходящий запрос на изолированных установках.
