...
### Проверьте лимит подключений

Единственная настройка, которую стоит подтвердить перед переходом, — это лимит подключений. Бэкенд работает в одном процессе и открывает **до `DB_POOL_SIZE + DB_MAX_OVERFLOW` подключений — по умолчанию 30**. Встроенный контейнер `db` допускает 100, поэтому в конфигурации по умолчанию это никогда не проявляется; управляемые тарифы начального уровня часто ограничивают базу данных ниже, и тогда PostgreSQL отвечает `too many connections for database "turboea"`.
...
`-1` означает «без отдельного ограничения». Если фактический лимит ниже 30, поднимите его либо уменьшите пул в `.env`, оставив несколько подключений для резервного копирования и административных сессий:
...
Чтобы увидеть, что подключено прямо сейчас:
...
### Как узнать о доступном обновлении

Вместо того чтобы следить за репозиторием, пусть экземпляр сообщит сам: Turbo EA ежедневно проверяет наличие более новой версии и уведомляет администраторов через колокольчик уведомлений, указывая номер версии и заметки о выпуске. Он никогда ничего не загружает и не устанавливает — шаг 1 выше по-прежнему начинается с чтения журнала изменений. Проверка включена по умолчанию и отключается в разделе [Администрирование → Настройки → Общие](settings.md#update-notifications); отключение также убирает исходящий запрос на изолированных установках. После завершения обновления каждый пользователь получает в колокольчике уведомление о том, что приложение обновлено, и может прочитать журнал изменений пройденных версий. Каждое уведомление продолжает показывать выпуск, о котором сообщало (заметки берутся из журнала изменений, встроенного в образ), поэтому старое уведомление никогда не отвечает заметками новейшего выпуска.
...
## Восстановление доступа администратора { #recovering-administrator-access }

Turbo EA отклоняет два изменения, которые чаще всего лишают администратора доступа: вы не можете сменить собственную роль на неадминистраторскую и не можете деактивировать собственную учётную запись (см. [Пользователи и роли](users.md)). Остаётся обычный случай — забытый пароль или экземпляр, единственный администратор которого покинул компанию. Двигайтесь по списку сверху вниз; базы данных касается только последний шаг.

1. **Попросите другого администратора.** В разделе **Admin → Пользователи → значок редактирования → Пароль** можно задать новый пароль для любой локальной учётной записи. Это обычный путь, доступ к серверу не нужен.
2. **Воспользуйтесь самостоятельным сбросом.** Ссылка **Забыли пароль** на странице входа отправляет ссылку для сброса по электронной почте. Она работает только для локальных учётных записей и только если [настроен SMTP](settings.md): без почтового сервера самостоятельного сброса нет. У учётных записей SSO пароля для сброса нет — их исправляют на стороне поставщика удостоверений.
3. **Сбросьте пароль в базе данных.** Только когда войти как администратор не может уже никто.

### Сброс пароля напрямую в базе данных

Сформируйте хеш собственной хеш-функцией приложения, чтобы сохранённое значение в точности соответствовало ожиданиям проверки при входе. Не собирайте его вручную с помощью `htpasswd` или `openssl`:
...
Ввод пароля в ответ на приглашение не даёт ему попасть в историю командной оболочки. Скопируйте выведенную строку `$2b$…`, затем выполните:

```bash
docker compose exec -T db psql -v ON_ERROR_STOP=1 -U turboea -d turboea -c \
  "UPDATE users
      SET password_hash = '<вставьте хеш сюда>',
          auth_provider = 'local',
          is_active = true,
...
Каждое присваивание нужно, потому что вход проверяет их все:

| Присваивание | Зачем оно |
|---|---|
| `password_hash` | Новые учётные данные. Оставьте одинарные кавычки: хеши bcrypt содержат `$`, который иначе развернёт оболочка. |
| `auth_provider = 'local'` | Пароль никогда не аутентифицирует учётную запись, помеченную как SSO. |
| `is_active = true` | Неактивная учётная запись отклоняется *после* проверки пароля, и выглядит это в точности как неверный пароль. |
| `password_setup_token = NULL` | Аннулирует непогашенную ссылку-приглашение, чтобы она позже не перезаписала только что заданный пароль. |

Если учётная запись была ещё и понижена в роли, восстановите роль в том же запросе, добавив `role = 'admin'`.

Ожидаемый результат — `UPDATE 1`. `UPDATE 0` означает, что ни одна строка не совпала: адреса электронной почты хранятся в том виде, в каком были введены, и именно `lower()` с обеих сторон делает сравнение независимым от регистра. Чтобы сначала найти нужную строку:
...
- **Скорректируйте подключение.** Используйте собственные `POSTGRES_USER` / `POSTGRES_DB`, если вы их меняли. В управляемой службе PostgreSQL контейнера `db` нет — подключайтесь напрямую через `psql`.
- **Другие сеансы остаются активными.** Сеансы — это JWT без отзыва на стороне сервера, поэтому токены, выданные до сброса, действуют до истечения срока (`ACCESS_TOKEN_EXPIRE_MINUTES`, по умолчанию 24 часа). Смена пароля никого не отключает.

!!! note "Разрешённое исключение из правила «не редактируйте базу данных»"
    В остальных случаях прямое редактирование базы данных — подводный камень, и не зря. Эта процедура безопасна, потому что обновляет несколько столбцов одной строки `users`: она не меняет схему и потому не может конфликтовать с будущей миграцией, и это единственное состояние, которое интерфейс и API по определению не в состоянии исправить, — чтобы восстановить доступ, нужен доступ.
...
5. **Прямое редактирование базы данных** — состояние схемы принадлежит Alembic, и ручной DDL столкнётся с будущими миграциями. То же с данными: используйте API или интерфейс, чтобы права доступа, события аудита и пересчёт качества данных оставались корректными. Единственное разрешённое исключение — [восстановление доступа администратора](#recovering-administrator-access) для случая, когда войти и воспользоваться API или интерфейсом уже не может никто.
...
8. **Переход на управляемый PostgreSQL без проверки его лимита подключений** — бэкенду по умолчанию нужно до 30 подключений. Более низкий лимит проявляется как `too many connections for database "turboea"` при обычной работе; см. раздел «Проверьте лимит подключений» выше.
