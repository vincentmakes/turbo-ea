...
| **Integrações** | `/admin/settings?tab=integrations` | Sincronização ServiceNow e integrações adicionadas por extensões | [Integração com ServiceNow](servicenow.md) |
...
Selecione o mês em que o ano fiscal da sua organização começa (janeiro a dezembro). Esta configuração afeta como as **linhas de orçamento** no módulo PPM são agrupadas por ano fiscal. Por exemplo, se o ano fiscal começa em abril, uma linha de orçamento de junho de 2026 pertence ao AF 2026–2027. Também determina os anos fiscais para os quais o [Relatório de Custos](../guide/reports.md) apresenta os custos e quando começa cada um.
...
![Envio de e-mail com a lista de métodos de envio](../assets/img/pt/95a_admin_config_email.png)
...
| **Porta SMTP** | A porta do servidor (587 para STARTTLS, 465 para TLS/SSL implícito) |
| **Usuário SMTP** | O nome de usuário de autenticação |
| **Senha SMTP** | A senha de autenticação (armazenada criptografada) |
| **Usar TLS** | Ativar a criptografia STARTTLS (recomendado). Ignorado na porta 465, que sempre usa TLS/SSL implícito |
...
![Botões dos módulos em Configurações → Geral](../assets/img/pt/95_admin_config_modulos.png)
