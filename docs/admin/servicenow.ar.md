...
يتيح تكامل ServiceNow (**الإدارة > الإعدادات > عمليات التكامل > ServiceNow**) المزامنة ثنائية الاتجاه بين Turbo EA وقاعدة بيانات الإدارة (CMDB) في ServiceNow الخاصة بك. يغطي هذا الدليل كل شيء بدءًا من الإعداد الأولي وصولًا إلى الوصفات المتقدمة وأفضل الممارسات التشغيلية.
...
| **SNOW Field** | اسم عمود API في ServiceNow (مثل `name` و`short_description`). اتركه فارغًا لكتابة قيمة ثابتة (انظر أدناه) |
| **القيمة الافتراضية** | القيمة التي تُكتب عندما يكون حقل ServiceNow فارغًا أو غير موجود (انظر أدناه) |
...
### القيم الافتراضية والثابتة

تتيح لك **القيمة الافتراضية** في تعيين الحقل ملء البيانات التي لا يوفرها ServiceNow. تُطبَّق **عند الإدخال فقط** (أثناء السحب)، وتعمل بطريقتين حسب ضبط **حقل SNOW**:

- **الاحتياطي** — عند ضبط حقل SNOW وقيمة افتراضية معًا، تُستخدم القيمة الافتراضية فقط عندما تكون قيمة ServiceNow فارغة أو غير موجودة. تفوز قيمة ServiceNow الحقيقية دائمًا.
- **ثابت** — اترك حقل SNOW فارغًا واضبط قيمة افتراضية فقط لكتابة قيمة ثابتة على كل بطاقة تتم مزامنتها، بغض النظر عن ServiceNow. على سبيل المثال، عيّن `subtype` بدون حقل SNOW وبقيمة افتراضية `hardware` بحيث يصبح كل CI مسحوب مكوّن تقنية معلومات من نوع الأجهزة.

تُحوَّل القيمة الافتراضية إلى نوع الحقل الهدف: تحلّل حقول `boolean` و`number` و`cost` القيمة؛ **تقبل حقول التحديد المتعدد قائمة مفصولة بفواصل** (مثل `web, backend` تصبح قيمتين). لدمج عدة حقول من ServiceNow في حقل واحد في Turbo EA، استخدم [حقلًا محسوبًا](calculations.md) يجمع الحقول المعيّنة الوسيطة — تعيّن طبقة التعيين عمودًا واحدًا في كل مرة.
...
بعد مزامنة البطاقة، تعرض علامة التبويب **الموارد** الخاصة بها قسم **ServiceNow** يتضمن رابطًا مباشرًا يفتح السجل المطابق (`https://<نسختك>/<الجدول>.do?sys_id=<id>`) في علامة تبويب جديدة. الرابط للقراءة فقط ويُحدَّث تلقائيًا عبر المزامنة — لا شيء لإعداده. البطاقات التي لم تتم مزامنتها لا تعرض القسم؛ ولا يزال بإمكانك إضافة رابط يدوي ضمن **روابط المستندات** في العلامة نفسها.
...
| Deleted | البطاقات المؤرشَفة (محذوفة حذفًا ناعمًا). تخضع لفترة الاحتفاظ المعتادة للأرشيف — يمكن استعادتها حتى انتهائها، ثم تُحذف نهائيًا. |
