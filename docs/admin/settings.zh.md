...
| **集成** | `/admin/settings?tab=integrations` | ServiceNow 同步及扩展添加的集成 | [ServiceNow 集成](servicenow.md) |
...
设置本实例的名称。它会在浏览器标签页、登录页面以及实例发送的邮件中取代「Turbo EA」。

浏览器标签页会把当前页面显示在它前面——清单页显示 `Inventory | Turbo EA`，卡片页显示 `SAP S/4HANA | Turbo EA`，按特定选项卡打开的页面显示 `GRC · Risk | Turbo EA`——这样一排打开的标签页依然易于辨认。首页和登录页仅显示标题本身。
...
选择您组织财年开始的月份（一月到十二月）。此设置影响 PPM 模块中**预算行**按财年分组的方式。例如，如果财年从四月开始，2026 年六月的预算行属于 2026–2027 财年。 它还决定[成本报告](../guide/reports.md)按哪些财年显示成本，以及每个财年从何时开始。
...
![电子邮件发送设置及发送方式列表](../assets/img/zh/95a_admin_settings_email.png)
...
![设置 → 常规 中的模块开关](../assets/img/zh/95_admin_settings_modules.png)
...
若团队规模很小、由同一人起草并签署，可保持关闭。启用与否不会改变记录内容：提交、审批、驳回和撤回都会写入卡片的**历史**选项卡。
...
Turbo EA 每天检查一次是否发布了更新版本；如果有，会在每位角色具备 `admin.settings` 权限的用户的通知铃中放入一条通知。点击通知会在 Turbo EA 内的对话框中打开发行说明，即该版本的变更日志。无论通知在铃中停留多久，它始终显示自己当初公告的那个版本：说明内容读取自镜像内置的变更日志，因此不会产生任何对外请求，在气隙环境中表现完全一致。只有尚未安装的版本才改为来自每日检查的缓存，因为构建时写入的变更日志无法描述它；对于这类版本，**在 GitHub 上查看**按钮会在新标签页中打开发行页面。

通知标题使用本实例配置的名称，因此重命名后的部署不会以其他产品名义发出公告。

该检查**仅作提醒**：不会下载任何内容，也不会改动主机上的任何东西。升级仍然是[运维](operations.md#the-upgrade-procedure)中所述的、事先备份的既定流程。不希望收到提醒的管理员，可以在自己的通知偏好中关闭**有可用更新**这一行。

关闭该开关后，每天向 github.com 发出的请求将完全停止，这正是气隙部署或出站受限环境所需要的。无论开启与否，实例都照常运行：当无法访问发行版信息时，失败会被安静地记录下来，不会显示任何内容。

### 升级完成之后

第二个开关**向用户公告升级**负责另一半场景。当实例以更新的版本重启时，**所有**用户——不只是管理员——都会收到一条通知，说明应用已更新；点击后会显示本次升级跨越的所有版本的变更日志。从 2.57.0 升到 2.60.0 的实例会显示全部四个版本，而不只是最后一个。每条这样的通知都与自己那次升级绑定，因此打开一年前的通知，看到的仍是*那次*升级所跨越的版本。

公告**每个版本只发一次**：同一版本重启十次也只产生一条通知，回滚则不产生任何通知。全新安装不会公告任何内容，因为并不存在可描述的升级。这些说明来自镜像内置的变更日志，因此这一半完全不需要网络。

该通知**仅在应用内**发送，绝不发送邮件——它会在每次升级时触达每一位活跃用户，若开放邮件通道，每个补丁版本都会变成一次群发。用户仍可在自己的通知偏好中的**更新通知**处将其静音，那里的邮件开关显示为禁用状态。

### 扩展商店通知

第三个开关 **扩展商店通知** 为[扩展商店](extensions.md)提供同样的功能。实例每天读取一次商店的公开目录，发现变化时会通知所有角色具备 `admin.manage_extensions` 权限的用户 — 也就是能够打开「扩展」页面的同一项权限。通知涵盖两种情况：商店中发布了您尚未安装的扩展，以及您已安装扩展有了更新版本。

即使在发布密集的日子里也依然清晰：无论有多少扩展发生变化，每位管理员每种类型只会收到**一条**通知（例如「有 3 个扩展更新可用」），而不是每个扩展一条。每项变化只公告**一次** — 一个月不变的目录只产生一条通知，而不是三十条 — 点击后会在 Turbo EA 内打开「商店」标签页。

首次成功读取目录时**不会**公告任何新扩展：否则初次接触商店的实例会把目录中的全部内容都报告一遍。已安装扩展的更新则会立即报告，因为数量总是很少且可以马上处理。

与版本检查一样，这**仅用于提醒** — 不会下载或安装任何内容，安装始终是「扩展」页面上的主动操作。关闭该开关后，每日向商店发出的请求会完全停止。各位管理员可以在自己的通知偏好设置中分别静音 **有新扩展可用** 与 **有扩展更新可用** 两行。
