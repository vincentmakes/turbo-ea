...
Turbo EA 每天检查一次是否发布了更新版本；如果有，会在每位角色具备 `admin.settings` 权限的用户的通知铃中放入一条通知。点击通知会在 Turbo EA 内的对话框中打开发行说明，即该版本的变更日志。说明内容由每日检查缓存，因此阅读时不会产生任何对外请求；即使实例之后失去网络连接也照常可读。对话框中的**在 GitHub 上查看**按钮可在新标签页中打开发行页面，供有需要的人使用。
...
第二个开关**向用户公告升级**负责另一半场景。当实例以更新的版本重启时，**所有**用户——不只是管理员——都会收到一条通知，说明 Turbo EA 已更新；点击后会显示本次升级跨越的所有版本的变更日志。从 2.57.0 升到 2.60.0 的实例会显示全部四个版本，而不只是最后一个。
