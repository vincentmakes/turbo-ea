...
适用于执行 GxP、ISO 9001 或类似质量体系的组织。**默认关闭**——在管理员启用之前，一切保持不变。

| 开关 | 默认值 | 作用 |
|------|--------|------|
| **允许撤回已发布的流程图** | 关闭 | 在已发布的流程图上添加「撤回」按钮。关闭时，无论拥有何种权限，所有人的撤回请求都会被拒绝。 |
| **要求由他人审批** | 开启 | 提交版本的人不得同时审批该版本。仅在上述开关开启时生效——若团队很小、由同一人完成两项工作，可将其关闭。 |

撤回被特意设置了两道关卡：

- **本设置**是组织层面的决定：贵组织的质量体系是否允许进行受控撤回。
- **权限**是个人层面的决定：谁可以执行。撤回还需要「撤回（取消发布）已发布的 BPMN 流程版本」权限，或利益相关者角色上的同等卡片级权限。**默认没有任何角色拥有该权限**，因此仅启用本设置不会赋予任何人权限。请在角色中授予，或在[元模型](metamodel.md)中授予「流程负责人」利益相关者角色。

已撤回的版本只是取消发布，绝不会被删除：原有审批记录、图示和版本号均予保留，撤回操作会连同操作人、时间戳和书面理由一并记录。用户端的呈现方式请参见 [Business Process Management](../guide/bpm.md)。
