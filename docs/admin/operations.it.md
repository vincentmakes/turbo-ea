...
Adattate utente e nome del database se avete modificato `POSTGRES_USER` / `POSTGRES_DB`. Uno snapshot del volume `postgres_data` è un'alternativa equivalente.
...
1. Fermate lo stack.
2. Ripristinate il backup del database eseguito prima dell'aggiornamento.
...
- **Ripristino del database**: ripristinate un `pg_dump` di produzione nel database di staging. I segreti cifrati nel database derivano da `SECRET_KEY`, quindi lo staging ha bisogno dello stesso `SECRET_KEY`, altrimenti dovrete reinserire lì le credenziali di integrazione.
