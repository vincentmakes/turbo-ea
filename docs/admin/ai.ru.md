...
Смысловой поиск позволяет пользователям находить карточки по **смыслу**, а не по точной формулировке — поиск по запросу «клиентские платёжные системы» выдаёт карточку с именем «NexaPay Gateway», которую обычный текстовый поиск пропустил бы. Он обеспечивает работу MCP-инструмента `semantic_search_cards`.
