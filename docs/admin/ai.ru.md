...
![Настройки ИИ](../assets/img/ru/26_admin_nastroyki_ii.png)
...
| **Azure Hosted OpenAI** | Коммерческий | API-ключ + endpoint ресурса Azure + имя deployment + версия API (по умолчанию `2025-01-01`) |
| **OpenRouter** | Коммерческий | API-ключ + имя модели |
| **Anthropic Claude** | Коммерческий | API-ключ + имя модели |
| **Amazon Bedrock** | Ваша учётная запись AWS | Регион AWS + идентификатор модели или профиля инференса; IAM-роль либо необязательный ключ доступа |

Коммерческие провайдеры требуют API-ключ, который хранится в базе данных в зашифрованном виде с использованием симметричного шифрования Fernet. Amazon Bedrock — исключение: по умолчанию он аутентифицируется через IAM-роль контейнера, поэтому хранить и менять ключ не нужно.
...
1. Перейдите в **Настройки > ИИ-предложения** в интерфейсе администратора.
2. Выберите вашего провайдера (OpenAI, Google Gemini, Azure OpenAI, OpenRouter или Anthropic Claude). Для Amazon Bedrock см. вариант D ниже.
...
### Вариант D: Amazon Bedrock

Модели работают внутри вашей учётной записи AWS, поэтому запросы никогда не покидают её границу и нет стороннего API-ключа, которым нужно управлять.

**1. Включите доступ к моделям** в консоли AWS Bedrock, в разделе **Model access**, для нужного региона. Доступ предоставляется для каждой модели отдельно; некоторые поставщики просят однократно описать сценарий использования.

**2. Выдайте разрешения.** Turbo EA использует IAM-роль контейнера (роль задачи ECS, сервисный аккаунт EKS или профиль экземпляра EC2). Прикрепите к ней эту политику:
...
**3. Настройте Turbo EA:**

1. Перейдите в **Настройки > ИИ** в интерфейсе администратора.
2. Выберите **Amazon Bedrock** в качестве провайдера.
3. Введите **Регион AWS** — например `eu-central-1`. В это поле вводится регион, а не URL.
4. Оставьте поле **API-ключ** пустым, чтобы использовать IAM-роль. За пределами AWS введите `ACCESS_KEY_ID:SECRET_ACCESS_KEY`; значение шифруется перед сохранением, как и любой другой ключ провайдера.
5. Нажмите **Тест подключения**. Будут перечислены все модели и профили инференса, доступные в регионе.
6. Выберите **модель** из списка и нажмите **Сохранить**.

!!! warning "Новым моделям нужен идентификатор профиля инференса"
    Новые модели — в том числе Claude Sonnet 4 — нельзя вызвать по обычному идентификатору модели. Bedrock возвращает `ValidationException` с сообщением о том, что пропускная способность по запросу не поддерживается. Используйте региональный **идентификатор профиля инференса** с географическим префиксом `eu.`, `us.` или `apac.`: `eu.anthropic.claude-sonnet-4-20250514-v1:0`. Именно поэтому тест подключения выводит идентификаторы профилей первыми.
---
...
Если **ни один** тип карточки не отмечен, функция не отключается — это означает *отсутствие ограничения по типам*, поэтому кнопка предложения появляется для всех типов карточек.
...
![Панель ИИ-предложений на странице карточки](../assets/img/ru/27_panel_predlozheniy_ii.png)
...
- **Ваша собственная учётная запись AWS**: с Amazon Bedrock инференс выполняется внутри вашей учётной записи AWS. Запросы остаются внутри её границы и регулируются вашими собственными service control policies.
...
| Bedrock возвращает «AccessDeniedException» | Две возможные причины: у IAM-роли нет разрешения `bedrock:InvokeModel` либо доступ к этой модели не включён в консоли Bedrock. Проверьте оба варианта. |
| Bedrock сообщает, что пропускная способность по запросу не поддерживается | Модель доступна только через региональный профиль инференса. Запустите тест подключения и выберите идентификатор с префиксом `eu.`, `us.` или `apac.`. |
