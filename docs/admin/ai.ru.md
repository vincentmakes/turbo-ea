...
## Смысловой поиск (эмбеддинги) { #semantic-search-embeddings }

Смысловой поиск позволяет пользователям находить карточки по **смыслу**, а не по точной формулировке — поиск по запросу «клиентские платёжные системы» выдаёт карточку с именем «NexaPay Gateway», которую обычный текстовый поиск пропустил бы. Он обеспечивает работу переключателя **Смысловой** в поле поиска инвентаря и MCP-инструмента `semantic_search_cards`.

Под капотом он использует **модель эмбеддингов** (небольшую модель «текст → вектор»), а не чат-LLM, и объединяет ранжирование по смыслу с существующим поиском по подстроке. Поскольку эмбеддинги — это отдельная возможность, а у Anthropic (поддерживаемого чат-провайдера) нет API эмбеддингов, провайдер эмбеддингов настраивается **отдельно** от чат-провайдера ИИ выше.

### Включение смыслового поиска

1. Перейдите в **Администрирование → Настройки → ИИ → Смысловой поиск (эмбеддинги)**.
2. Включите **Смысловой поиск включён**.
3. Выберите **провайдера эмбеддингов**:
   - **Ollama** (по умолчанию, локальный, работает офлайн): укажите URL провайдера (например, `http://ollama:11434`) и модель, выдающую 768-мерные векторы — рекомендуемое значение по умолчанию `nomic-embed-text`. Загрузите её командой `ollama pull nomic-embed-text`.
   - **OpenAI-совместимый**: укажите URL провайдера и API-ключ; используйте `text-embedding-3-small` (или `-large`), которую Turbo EA запрашивает в 768 измерениях.
   - **Azure OpenAI**: укажите URL ресурса, API-ключ и имя развёртывания эмбеддингов.
4. Нажмите **Проверить**, чтобы проверить подключение — успешная проверка сообщает размерность вектора (768).
5. Нажмите **Сохранить**.

> Модель эмбеддингов должна выдавать **768-мерные** векторы. `nomic-embed-text` делает это нативно; модели OpenAI `text-embedding-3-*` автоматически запрашиваются в 768 измерениях.

### Как эмбеддинги поддерживаются в актуальном состоянии

Эмбеддинги карточек создаются и обновляются автоматически фоновой задачей — вам не нужно ничего запускать вручную. При первом включении функции она **дозаполняет** существующий инвентарь (на крупном ландшафте с Ollama только на CPU это может занять от нескольких минут до нескольких часов и выполняется незаметно в фоне). После этого новые и изменённые карточки повторно встраиваются примерно в течение минуты. При смене модели эмбеддингов каждая карточка повторно встраивается автоматически.

Смысловые результаты соблюдают **те же права доступа и то же скрытие полей стоимости**, что и обычный список карточек — пользователь никогда не увидит в смысловых результатах карточку, которую не смог бы увидеть в инвентаре. Если провайдер эмбеддингов недоступен, поиск прозрачно возвращается к поиску по подстроке.
