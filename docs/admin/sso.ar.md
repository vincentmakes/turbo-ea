...
**تعيين الأدوار (اختياري).** افتراضيًا يصل الجميع إلى الدور الافتراضي المُهيّأ ثم يرقّيهم المسؤول من هناك. وإذا كان مزوّد الهوية لديكم يعرف الإجابة أصلًا — تسجيل تطبيق Entra يعلن أدوار تطبيقه الخاصة، أو oauth2-proxy يمرّر عضوية المجموعات — فبإمكان Turbo EA قراءتها وإسناد الدور بنفسه:
...
كل زوج يُكتب `قيمة_الدليل:مفتاح-دور-turbo-ea`. وعندما يحمل المستخدم عدة أدوار من الدليل، **يفوز أول مدخل في التعيين** — ترتيب التعيين، لا الترتيب الذي أرسلها به المزوّد، لأن صيغتَي هوية Azure لا تتفقان عليه. وتتجاهل المطابقة حالة الأحرف في جانب الدليل. وفي وضع الوكيل العام يقرأ التعيين نفسه ترويسة مفصولة بفواصل بدلًا من مطالبة: `TURBO_EA_PROXY_AUTH_ROLE_HEADER=X-Forwarded-Groups`.

!!! warning "التعيين هو المرجع عند كل تسجيل دخول"
    وليس عند إنشاء الحساب فقط. أي دور يُمنح يدويًا من **الإدارة ← المستخدمون** يُلغى عند تسجيل الدخول التالي لذلك الشخص — وهذا هو المقصود، لأن سحب دور أحدهم من الدليل يجب أن يسري فعلًا. اترك `TURBO_EA_PROXY_AUTH_ROLE_MAP` غير مضبوط ولن يتغير شيء: تبقى الأدوار يدوية بالكامل.

الحالات الحدّية، وقد اختيرت جميعها كي لا يحبسك خطأ في التهيئة خارج نظامك:

- **`TURBO_EA_PROXY_AUTH_BOOTSTRAP_ADMIN_EMAIL` يفوز دائمًا** على التعيين. فإن اختلفا، فذلك العنوان هو المسؤول.
- **القيمة التي لا تطابق شيئًا في التعيين** — أو تشير إلى دور في Turbo EA غير موجود أو مؤرشف — تعود إلى الدور الافتراضي.
- **المطالبة الغائبة تمامًا** تترك دور المستخدم الحالي كما هو. وهذا مختلف عن الحالة السابقة عن قصد: فخطأ إملائي في `ROLE_CLAIM`، أو مخزن رموز توقف عن التمرير، كان سيخفّض رتبة كل مستخدمي المثيل دفعة واحدة.
- **يجب أن تكون الهوية جديرة بأن تُؤتمن على الصلاحيات.** يسري تعيين الأدوار عندما يكون رمز الهوية قد جرى التحقق منه (`TURBO_EA_PROXY_AUTH_VERIFY_ID_TOKEN=true`) أو عندما يكون هناك سر مشترك مُهيّأ. أما على App Service مع تعطيل مخزن الرموز ودون سر، فيُتجاهل التعيين ويُكتب في السجل سطر يوضح ذلك — وهو المنطق نفسه الذي يمنع ترويسة لم يجرِ التحقق منها من إنشاء حساب.
...
| `TURBO_EA_PROXY_AUTH_ROLE_MAP` | — | `قيمة_الدليل:مفتاح-الدور،…` — الفراغ يعني أن الأدوار تبقى يدوية |
| `TURBO_EA_PROXY_AUTH_ROLE_CLAIM` | `roles` | المطالبة التي تحمل دور الدليل (وضع Azure) |
| `TURBO_EA_PROXY_AUTH_ROLE_HEADER` | `X-Forwarded-Groups` | وضع `header`: ترويسة أدوار مفصولة بفواصل |
