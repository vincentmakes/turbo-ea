...
1. On the **Relation Types** tab, find **Hierarchy link types** above the relation list — every card type with hierarchy switched on is listed there. A card type's own **Relations** tab shows the same section, scoped to that type.
