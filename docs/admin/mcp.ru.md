...
| `MCP_PUBLIC_URL` | `http://localhost:8920/mcp` (docker compose) | Публичный URL MCP-сервера (используется в URI перенаправления OAuth). При автономном запуске контейнера значение по умолчанию в коде — `http://localhost:8001` |
...
![Раздел «Интеграция MCP» на вкладке «ИИ»](../assets/img/ru/95b_admin_nastroyki_mcp.png)
...
MCP-сервер предоставляет **51 инструментов**, разделённых на две группы: **32 инструментов чтения**, которые запрашивают данные EA, и **19 инструментов записи** (14 добавляющих, 5 разрушающих), которые создают и сопровождают карточки, связи, диаграммы, риски, ADR и многое другое — в том числе превращают артефакты, находящиеся в собственном контексте ИИ-инструмента (таблицы, BPMN XML, DrawIO XML, документы, изображения), в структурированные данные EA. Каждый инструмент несёт MCP-аннотации `ToolAnnotations` (подсказки «только чтение» / «разрушающий» / «идемпотентный»), чтобы коннекторы могли отображать степень разрушительности в своём интерфейсе.
...
Сервер предоставляет 32 инструментов чтения, сгруппированных в восемь кластеров.
...
| `resolve_card_refs` | Предварительная проверка ссылок на карточки по имени (имя → UUID) перед массовым импортом — только разрешает ссылки, никогда не записывает |
| `analyze_impact` | Анализ радиуса поражения зависимостей для предлагаемого изменения карточки |
...
| Инструмент | Описание |
|------------|----------|
| `list_diagrams` | Список свободных (DrawIO) диаграмм, опционально отфильтрованный по одной карточке |
| `get_diagram` | Получение одной диаграммы по id, включая её DrawIO XML |

**Аудит и история изменений**

| Инструмент | Описание |
|------------|----------|
| `get_change_history` | Запрос к журналу пакетов мутаций (по id пакета, исполнителю, инструменту или происхождению) для точного восстановления того, что изменил предыдущий коммит MCP |
...
Сервер предоставляет 19 инструментов записи, каждый из которых аннотирован как **добавляющий** (создаёт или расширяет данные) или **разрушающий** (изменяет или удаляет существующие данные), чтобы коннекторы могли предупреждать соответствующим образом.
...
| `transition_card_lifecycle` | Переводит карточку по фазам утверждения или жизненного цикла. |
| `create_risks` | Создаёт записи в Реестре рисков EA. |
| `update_risks` | Обновляет записи Реестра рисков (поля, связанные карточки). |
| `add_card_comment` | Публикует комментарий на карточке — неразрушающая, доступная для рецензирования заметка вместо изменения полей. |
| `create_soaw` | Создаёт Statement of Architecture Work для инициативы. |
| `assign_stakeholders` | Назначает или удаляет роли заинтересованных лиц на карточках. |
| `update_cards_bulk` | Точечные изменения полей на многих карточках одним вызовом. |
| `create_adr` | Создаёт Architecture Decision Record. |
| `update_adr` | Обновляет ADR (заголовок, разделы, статус, связанные карточки). |
| `sign_adr` | Подписывает ADR (требует разрешения `adr.sign`; иначе возвращает глубокую ссылку на интерфейс для подписания в браузере). |
...
| Инструмент | Описание |
|------------|----------|
| `upsert_relations_bulk` | Создаёт или удаляет связи между карточками. Источник / цель / тип проверяются по метамодели. Удаление отклоняется, если оператор не включил его явно (см. защитные ограждения). |
| `archive_cards` | Мягкое удаление карточек. Обратимо — архивированные карточки можно восстановить в течение 30 дней до автоматической очистки. |
| `update_diagram` | Заменяет DrawIO XML, имя или связи с карточками у диаграммы. |
| `rollback_batch` | Откатывает записи, выполненные в рамках предыдущего пакета мутаций. |
...
- **Аварийный выключатель.** `MCP_WRITES_ENABLED=false` отключает все 19 инструментов записи без повторного развёртывания кода. 32 инструментов чтения продолжают работать.
- **Скачивание логотипов ограничено списком хостов.** Встроенные наборы иконок не покрывают все продукты заказчика, а ассистент часто работает в песочнице без доступа к вебу, поэтому `set_card_logos` принимает `image_url`, и его скачивает MCP-сервер. Только `https`, только хосты из `MCP_LOGO_FETCH_HOSTS`, только публичные адреса, не более двух перенаправлений (каждое проверяется заново), не более 1 МБ потоковым чтением, и байты обязаны нести настоящую сигнатуру PNG/JPEG/WebP/GIF. Скачивание происходит на границе MCP, никогда в бэкенде, после чего изображение загружается обычным путём: выбранный моделью URL никогда не достигает процесса, который держит базу данных. `MCP_LOGO_FETCH_ENABLED=false` отключает всё это.
...
- **Пакеты мутаций.** Каждый вызов записи MCP открывает пакет мутаций до каких-либо записей; каждое событие, эмитированное во время вызова, помечается id пакета. Администраторы (или инструмент `get_change_history`) могут восстановить полный поэлементный дифф коммита по одному id, а `rollback_batch` может его откатить. Коммиты, затрагивающие больше строк, чем `MCP_BATCH_CONFIRMATION_THRESHOLD`, должны вернуть одноразовый `confirm_token`, выданный предшествующим сухим прогоном (TTL 15 минут), поэтому крупный коммит всегда следует за просмотренным предпросмотром.
- **Нет жёсткого удаления.** Набор инструментов намеренно не включает необратимое удаление карточек. `archive_cards` и `update_cards_bulk` *представлены*, но архивирование — это обратимое мягкое удаление (окно восстановления 30 дней), и оба инструмента аннотированы как разрушающие и защищены сухим прогоном. Добавление любого инструмента, выполняющего необратимую мутацию (жёсткое удаление, принудительная очистка), потребует явного дизайн-обзора.

Шесть переменных окружения защитных ограждений на контейнере MCP:

| Переменная | По умолчанию | Эффект |
|------------|--------------|--------|
| `MCP_WRITES_ENABLED` | `true` | Главный переключатель инструментов записи. `false` → MCP только для чтения. |
| `MCP_MAX_CARDS_PER_CALL` | `200` | Жёсткое ограничение строк `create_cards_bulk` / `update_cards_bulk` на запрос. |
...
| `MCP_LOGO_FETCH_ENABLED` | `true` | При `true` инструмент `set_card_logos` принимает `image_url`: MCP-сервер скачивает изображение и загружает его обычным путём, поэтому все проверки прав, размера и формата по-прежнему выполняются. Значение `false` полностью запрещает путь по URL — slug'и иконок и вставленные байты продолжают работать. |
| `MCP_LOGO_FETCH_HOSTS` | *(built-in)* | Список хостов, с которых разрешено скачивать логотип: через запятую, точное совпадение. По умолчанию: `raw.githubusercontent.com`, `cdn.jsdelivr.net`, `cdn.simpleicons.org`, `upload.wikimedia.org` — публичный хостинг иконок, где запрос содержит только имя файла и ничего о вашем ландшафте. Можно добавить сервис поиска логотипа по домену, но тогда он узнает, каких поставщиков вы используете. |
| `MCP_BATCH_CONFIRMATION_THRESHOLD` | `20` | Коммиты, затрагивающие больше строк, чем это значение, требуют `confirm_token` от предшествующего сухого прогона. |
| `MCP_REQUIRE_DRYRUN_FIRST` | `true` | Включает описанный выше механизм confirm-token. Устанавливайте `false` только для доверенных конвейеров автоматизации, которые сознательно пропускают этап предварительного просмотра. |
...
| **Все аутентифицированные пользователи** | Доступ на чтение управляется их существующим RBAC. Инструменты записи требуют соответствующих бэкенд-разрешений — `inventory.create` / `inventory.edit` / `inventory.archive` (карточки), `relations.manage` (связи), `diagrams.manage` (диаграммы), `bpm.edit` (BPMN), `risks.manage` (Реестр рисков), `comments.create` (комментарии), `stakeholders.manage` (заинтересованные лица), `soaw.create` (SoAW), `adr.create` / `adr.sign` (ADR). |
