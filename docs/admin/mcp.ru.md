...
Сервер предоставляет 30 инструментов чтения, сгруппированных в восемь кластеров.
...
Сервер предоставляет 18 инструментов записи, каждый из которых аннотирован как **добавляющий** (создаёт или расширяет данные) или **разрушающий** (изменяет или удаляет существующие данные), чтобы коннекторы могли предупреждать соответствующим образом.
...
- **Аварийный выключатель.** `MCP_WRITES_ENABLED=false` отключает все 18 инструментов записи без повторного развёртывания кода. 30 инструментов чтения продолжают работать.
