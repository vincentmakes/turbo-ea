...
Сервер предоставляет 31 инструментов чтения, сгруппированных в восемь кластеров.
...
| `semantic_search_cards` | Смысловой (эмбеддинг) поиск карточек в сочетании с поиском по подстроке; ранжированные результаты со score/match/фрагментом. Требует провайдера эмбеддингов (иначе — откат к подстроке) |
...
Сервер предоставляет 17 инструментов записи, каждый из которых аннотирован как **добавляющий** (создаёт или расширяет данные) или **разрушающий** (изменяет или удаляет существующие данные), чтобы коннекторы могли предупреждать соответствующим образом.
...
- **Аварийный выключатель.** `MCP_WRITES_ENABLED=false` отключает все 17 инструментов записи без повторного развёртывания кода. 31 инструментов чтения продолжают работать.
