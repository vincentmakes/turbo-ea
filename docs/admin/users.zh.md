...
!!! note "你无法给自己降级或停用自己"
    在你自己那一行，**角色**下拉框和**停用**按钮均为禁用状态，API 同样会拒绝这两种改动。管理员一旦放弃自己的角色，就会在同一次点击中失去所有管理界面，而只有另一位管理员才能把它交还。若要交接，请让第二位管理员为你更改角色。如果已经没有人能以管理员身份登录，请参阅[恢复管理员访问权限](operations.md#recovering-administrator-access)。
...
「最后一名管理员」保护始终生效：会导致零名活跃管理员的批量角色变更会被拒绝；停用或删除最后一名管理员同样会被拒绝。出于同样的原因，批量角色变更和批量停用都会跳过你自己的账户。
...
权限同时适用于地址栏和菜单。角色无权打开的页面会显示**访问被拒绝**提示，并提供返回仪表板的链接，无论是通过导航、书签还是他人分享的链接进入的——因此隐藏菜单项与阻止页面始终是同一个决定。

角色的权限默认适用于所有卡片类型。管理员可以按卡片类型收紧权限——逐一允许或拒绝**创建**、**编辑**、**归档**和**删除**——具体位于[元模型](metamodel.md#permissions)中该卡片类型的**权限**选项卡。
