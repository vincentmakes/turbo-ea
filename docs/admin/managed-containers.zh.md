...
- **边缘监听 8920。** 它的默认端口是 8080，而前端镜像在同一命名空间内已经占用了 8080，因此每个模板都设置 `NGINX_HTTP_PORT=8920` 并把平台入口指向它。边缘仍然掌管所有安全头、工作区传输 512 MB 的上传限制、事件流设置和 `/mcp` 路由——平台侧没有任何东西能替代它。
...
| 最大上传（工作区导入最高 512 MB） | Microsoft 未记录——依赖前请先测试 512 MB 导入 | **HTTP/1 下每个请求 32 MiB** | 平台无限制 |
