...
## 语义搜索（嵌入） { #semantic-search-embeddings }

语义搜索让用户能够按**含义**而非精确措辞来查找卡片——搜索「面向客户的支付系统」会找到名为「NexaPay Gateway」的卡片，而纯文本搜索会遗漏它。它为清单搜索框中的**语义**切换开关以及 MCP 工具 `semantic_search_cards` 提供支持。

在底层，它使用**嵌入模型**（一个小型的文本转向量模型），而非聊天大语言模型，并将基于含义的排序与现有的子串匹配相融合。由于嵌入是一项独立的能力——而且 Anthropic（一个受支持的聊天提供程序）没有嵌入 API——嵌入提供程序需与上文的聊天 AI 提供程序**分开**配置。

### 启用语义搜索

1. 前往 **管理 → 设置 → AI → 语义搜索（嵌入）**。
2. 开启**语义搜索已启用**。
3. 选择一个**嵌入提供程序**：
   - **Ollama**（默认，自托管，可离线工作）：设置提供程序 URL（例如 `http://ollama:11434`）以及一个输出 768 维向量的模型——`nomic-embed-text` 是推荐的默认值。使用 `ollama pull nomic-embed-text` 拉取它。
   - **OpenAI 兼容**：设置提供程序 URL 和 API 密钥；使用 `text-embedding-3-small`（或 `-large`），Turbo EA 会以 768 维请求它。
   - **Azure OpenAI**：设置资源 URL、API 密钥以及一个嵌入部署名称。
4. 点击**测试**以验证连通性——测试成功会报告向量维度（768）。
5. 点击**保存**。

> 嵌入模型必须输出 **768 维**向量。`nomic-embed-text` 原生即为此维度；OpenAI 的 `text-embedding-3-*` 模型会自动以 768 维请求。

### 嵌入如何保持最新

卡片嵌入由后台任务自动生成并刷新——你无需手动触发任何操作。首次启用该功能时，它会**回填**现有清单（在使用纯 CPU Ollama 的大型资产上，这可能需要几分钟到几小时，并在后台安静运行）。此后，新建和编辑的卡片会在大约一分钟内重新嵌入。如果你切换嵌入模型，每张卡片都会自动重新嵌入。

语义结果遵循与普通卡片列表**相同的权限和成本字段脱敏**——用户在语义结果中绝不会看到其在清单中无法看到的卡片。如果嵌入提供程序不可用，搜索会透明地回退到子串匹配。
