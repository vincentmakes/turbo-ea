...
## Контролируемая публикация процессов

Для организаций, работающих по GxP, ISO 9001 или аналогичной системе качества. **По умолчанию отключено** — ничего не меняется, пока администратор не включит эту возможность.

| Переключатель | По умолчанию | Действие |
|---------------|--------------|----------|
| **Разрешить отзыв опубликованной схемы процесса** | Выкл. | Добавляет кнопку **Отозвать** к опубликованной схеме процесса. Пока переключатель выключен, отзыв запрещён всем, независимо от имеющихся разрешений. |
| **Требовать отдельного утверждающего** | Вкл. | Тот, кто подаёт редакцию, не может её утверждать. Действует только при включённом переключателе выше — отключите его для небольшой команды, где один человек делает и то и другое. |

Отзыв намеренно защищён двумя барьерами:

- **Эта настройка** — организационное решение: допускает ли ваша система качества контролируемый отзыв в принципе.
- **Разрешение** — персональное решение: кто именно может это сделать. Для отзыва дополнительно нужно разрешение *Отозвать (снять с публикации) опубликованную версию схемы BPMN* либо его аналог на уровне карточки у роли заинтересованного лица. **По умолчанию им не обладает ни одна роль**, поэтому включение настройки само по себе никому ничего не даёт. Выдайте его в разделе Роли или роли заинтересованного лица **Владелец процесса** в [Метамодели](metamodel.md).

Отозванная версия снимается с публикации, но никогда не удаляется: исходное утверждение, схема и номер редакции сохраняются, а сам отзыв фиксируется с автором, отметкой времени и письменным обоснованием. О том, что видит пользователь, см. [Business Process Management](../guide/bpm.md).
