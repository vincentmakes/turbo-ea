...
Turbo EA раз в сутки проверяет, вышла ли более новая версия, и при её наличии помещает уведомление в колокольчик каждого пользователя, чья роль даёт `admin.settings`. По клику заметки о выпуске — журнал изменений этой версии — открываются в диалоговом окне внутри Turbo EA. Заметки кэшируются ежедневной проверкой, поэтому их чтение не порождает исходящих запросов и работает даже если экземпляр впоследствии потеряет доступ к сети. Кнопка **Открыть на GitHub** в диалоге открывает страницу выпуска в новой вкладке для тех, кому это нужно.
...
Второй переключатель — **Сообщать пользователям об обновлениях** — закрывает вторую половину задачи. Когда экземпляр перезапускается на более новой версии, **каждый** пользователь, а не только администраторы, получает уведомление о том, что Turbo EA обновлён; по клику открывается журнал изменений всех версий, через которые прошло обновление. Экземпляр, перешедший с 2.57.0 на 2.60.0, покажет все четыре выпуска, а не только последний.
