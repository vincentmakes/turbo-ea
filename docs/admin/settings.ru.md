...
Turbo EA раз в сутки проверяет, вышла ли более новая версия, и при её наличии помещает уведомление в колокольчик каждого пользователя, чья роль даёт `admin.settings`. В уведомлении указан номер новой версии и ссылка на заметки о выпуске.
