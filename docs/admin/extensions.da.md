...
Kataloget er inddelt i fem sektioner — **Strategi, planlægning & transformation**, **Governance & automatisering**, **Integrationer**, **Regulering** og **Tjenester** — hver med sin egen overskrift, så du med det samme kan se, hvilken slags tilbud det er; alt, hvad kataloget ikke har klassificeret, vises under **Andre** til sidst. Inden for sektionerne fungerer tag-piller (gratis eller kommerciel, plus emner som integration) som filter: en filterbjælke vises over listen, klik på piller indsnævrer den (flere piller kombineres), sektioner uden match forsvinder, og **Alle** nulstiller visningen.

Et **tjenestetilbud** — noget, der ikke installeres, fx et supportabonnement — sælges og licenseres som en udvidelse, men uden bundle. Dets flise og detaljevindue tilbyder kun **Køb**, viser ingen version og siger det ligeud; når købet er bekræftet, vises den samme licenschip (*Fornyes den …*) på flisen, og din bekræftelsesmail forklarer, hvordan du bruger det, du har købt.
...
- `core.notifications.channel` — levere de notifikationer, du slår til, på en kanal, der tilhører udvidelsen selv, for eksempel en chatbesked. Udvidelsen modtager en notifikations titel, besked og link samt dit bruger-id; den modtager ikke din e-mailadresse, medmindre den også erklærer `core.users.read`, og den modtager aldrig en type, du ikke har slået til for den. Notifikationer, der kun bliver i klokken, sendes aldrig til en udvidelseskanal. Hver udvidelseskanal starter **slået fra** for alle notifikationstyper — også de administrative meddelelser (ny udvidelse, opdatering af udvidelse, Turbo EA-opdatering): åbn dine notifikationsindstillinger fra klokken, og slå kanalens kolonne til for de typer, du vil have der. En meddelelse, der allerede blev vist i klokken, før kanalen blev slået til, sendes ikke igen.
- `core.adr.read` — læse arkitekturbeslutninger (reference, titel, status, tilknyttede kort og udvidelsens egne data på dem).
- `core.adr.write` — oprette en arkitekturbeslutning som **kladde** og knytte den til aktive kort, for eksempel en udvidelse der registrerer resultatet af en analyse som en beslutning til gennemsyn. Skrivning indebærer læsning. En udvidelse kan aldrig underskrive en beslutning, sende den til gennemsyn eller ændre dens status — det bliver hos jeres brugere — og kan kun gemme data på posten under sine egne nøgler `ext.<key>.*`. Hver oprettelse registreres i Admin → Revisionslog som en `ext:<key>`-batch.
- `core.risks.read` — læse risikoregistret (reference, titel, kategori, niveau, status, ejer, tilknyttede kort).
- `core.risks.write` — oprette en risiko og knytte den til kort, for eksempel en regel der registrerer en risiko, når en applikation uden ejer overskrider en omkostningsgrænse. Skrivning indebærer læsning. En risiko oprettet på denne måde viser **Udvidelse** som kilde i registret, får sin reference fra samme sekvens som alle andre, og dens ejer modtager den sædvanlige opgave og notifikation. En udvidelse kan senere ændre titel, beskrivelse, ejer og måldato for de risici, **den** selv oprettede — aldrig andres, og aldrig en risikos status: at flytte den gennem forløbet, acceptere og lukke den bliver hos jeres brugere.
- `core.notifications.send` — sende en notifikation til navngivne personer, for eksempel »en regel, du har sat op, blev netop udløst«. Den ankommer som typen **Udvidelsesbesked** eller som en type, udvidelsen selv erklærer, og som så får sin egen række i hver persons notifikationsindstillinger (i appen slået til, e-mail slået fra som standard) — i begge tilfælde bestemmer hver person selv, om den når klokken, indbakken eller en udvidelseskanal, og kan slå den fra. Nogle af disse notifikationer åbner deres detaljer i appen ved klik og tilbyder kun det relaterede kort og udvidelsens egen side til dem, der må åbne dem. Linket i sådan en notifikation kan kun pege ind i Turbo EA.
- `core.stakeholders.write` — tildele eller fjerne interessentroller på kort med præcis de roller, korttypen definerer. Det er en særskilt grant fra `core.cards.write`, fordi en interessentrolle giver personen rettigheder på kortet.
- `core.surveys.read` — læse en undersøgelses navn, status og svarandel.
- `core.surveys.write` — sende en datavedligeholdelsesundersøgelse til interessenterne på et sæt kort, for eksempel en planlagt regel der beder ejerne af hver forretningskritisk applikation under datakvalitetsmålet om at udfylde deres kort — én anmodning pr. person i stedet for én opgave pr. kort. Skrivning indebærer læsning. Undersøgelsen vises i **Admin → Undersøgelser** uden ophavsmand, dens felter er præcis dem, korttypen definerer, og hver person får den sædvanlige undersøgelsesnotifikation. En udvidelse kan aldrig lukke en undersøgelse eller anvende dens svar — det bliver hos jer — og en tilbagerulning af den kørsel, der sendte den (Admin → Revisionslog), lukker undersøgelsen og trækker de anmodninger tilbage, som ingen endnu havde besvaret.
