...
卡片同步后，其**资源**选项卡会显示一个 **ServiceNow** 部分，其中包含一个直接链接，可在新标签页中打开对应记录（`https://<您的实例>/<表>.do?sys_id=<id>`）。该链接为只读，由同步自动维护 —— 无需配置。从未同步的卡片不会显示该部分；您仍可在同一选项卡的**文档链接**下手动添加链接。
...
| 已更新 | 更新的现有卡片数 |
| 已删除 | 归档（软删除）的卡片数。它们遵循正常的归档保留期——在保留期内可以恢复，到期后被永久删除。 |
| 错误 | 处理失败的记录数 |
| 时长 | 实际时间 |
