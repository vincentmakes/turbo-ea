...
![Démarrer une migration LeanIX dans Paramètres → Migration](../assets/img/fr/94_admin_migration_leanix.png)
...
2. **Examiner** chaque type d'entité dans la vue par onglets. Chaque ligne staged porte une action :
    ...
    Chaque onglet affiche au-dessus du tableau une rangée de **pastilles de filtre** — une par type de fiche le cas échéant, sinon par action — pour restreindre une liste longue (centaines de fiches, dizaines de types de fact sheets) à une tranche à la fois. L'onglet **Cartes** montre le **nom** de fiche résolu à côté de l'UUID source. La colonne *Note* affiche le motif complet du conflit ; les lignes `update` listent les noms de champs modifiés avec une infobulle détaillant la transition `ancien → nouveau`.

    Les onglets **Nouveaux types**, **Champs personnalisés** et **Nouvelles relations** affichent le métamodèle personnalisé du tenant issu de votre workspace source. Par défaut, ils sont acceptés tels quels et créent les types de fiches / champs / types de relations non-built-in correspondants dans Turbo EA.
...
3. **Mapper les champs importés** (optionnel, dans l'onglet **Champs personnalisés**). Pour chaque colonne personnalisée de la plateforme source, choisissez l'une des trois options dans la liste déroulante à côté de la ligne :
    - **Importer comme nouveau champ personnalisé** (par défaut) — la colonne devient un nouvel attribut sur le type de fiche cible, sous une section synthétique *Imported from {source}*.
    - **Mapper vers un champ Turbo EA existant** — la valeur est routée vers un champ built-in du type de fiche cible (p. ex. envoyer `businessCriticality` LeanIX vers l'emplacement `businessCriticality` natif de TEA). La ligne du champ de métamodèle est alors ignorée lors de l'apply, donc aucune colonne orpheline n'est créée.
...
4. **Appliquer** quand vous êtes satisfait. Le pipeline d'application exécute 12 passes ordonnées par dépendances (types de métamodèle → champs de métamodèle → types de relations de métamodèle → utilisateurs → fiches → groupes de tags → tags → liaisons fiche-tag → relations → souscriptions → documents → commentaires) dans des savepoints individuels — une ligne en échec n'empoisonne pas le reste de l'import. Le statut passe de `applying → applied` (ou `failed` si les erreurs franchissent le seuil de sécurité).
...
| LeanIX | Turbo EA |
|---|---|
| Application, ITComponent, Business Capability, Business Context, Process, DataObject, Interface, Provider, TechCategory, Platform, Objective, Project / Initiative | Mapping direct 1:1 de type de fiche |
...
| Tags (groupes single/multi) | Groupes de tags + tags + jointures par fiche |
...
| Types de fact sheet personnalisés du tenant (p. ex. `ESGCapability`, `Server`, `System`, `TechPlatform`, `TechnicalStack`) | Nouveaux types de fiches non-built-in avec `has_hierarchy=true`, `has_successors=true` et une section `Imported from LeanIX` pré-remplie |
...
Si vous devez refaire un import (p. ex. vous avez supprimé en masse les fiches importées dans l'UI et voulez les ré-importer), utilisez l'icône poubelle sur la ligne de migration pour la supprimer, puis re-uploadez. Les migrations `applied` sont supprimables ; cela libère le verrou d'idempotence par hash de fichier permettant de re-charger le même snapshot. Les lignes orphelines de `migration_identity_map` pointant vers des fiches inexistantes sont automatiquement élaguées lors de la prochaine passe de staging — un nettoyage manuel de l'identity map n'est jamais requis.
