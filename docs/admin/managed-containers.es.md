...
- **El borde escucha en 8920.** Su puerto por defecto es 8080, que la imagen del frontend ya ocupa en el mismo espacio de nombres; por eso cada plantilla define `NGINX_HTTP_PORT=8920` y apunta el ingress de la plataforma a él. El borde sigue siendo dueño de todas las cabeceras de seguridad, del límite de 2 GB para las importaciones de espacio de trabajo, de la configuración del flujo de eventos y del enrutado `/mcp`: nada en la plataforma lo sustituye.
...
| Subida máxima (la importación de espacio de trabajo llega a 2 GB) | No documentado por Microsoft: pruebe una importación de 2 GB antes de confiar en ello | **32 MiB por petición sobre HTTP/1** | Sin límite de plataforma |
...
**Amazon Bedrock.** Para usar Bedrock como proveedor de IA, adjunte la política IAM de [Funciones de IA](ai.md) al rol de tarea del stack; Turbo EA se autentica entonces con ese rol y no necesita ninguna clave API.
