...
Интеграция с ServiceNow (**Администрирование > Настройки > Интеграции > ServiceNow**) обеспечивает двунаправленную синхронизацию между Turbo EA и вашей CMDB ServiceNow. В этом руководстве описано всё — от первоначальной настройки до продвинутых сценариев и лучших практик эксплуатации.

![Настройки ServiceNow](../assets/img/ru/31_admin_nastroyki_servicenow.png)
...
| **Поле SNOW** | Имя API столбца ServiceNow (например, `name`, `short_description`). Оставьте пустым, чтобы записать фиксированную константу (см. ниже) |
| **Значение по умолчанию** | Значение, записываемое, когда поле ServiceNow пустое или отсутствует (см. ниже) |
...
| `subtype` | Подтип карточки (из списка подтипов типа карточки) | `"hardware"` |
...
**Значение по умолчанию** в сопоставлении поля позволяет заполнить данные, которые ServiceNow не предоставляет. Оно применяется **только на входе** (при извлечении) и ведёт себя двумя способами в зависимости от того, задано ли **поле SNOW**:

- **Запасное значение** — Если заданы и поле SNOW, и значение по умолчанию, последнее используется только когда значение ServiceNow пустое или отсутствует. Реальное значение ServiceNow всегда имеет приоритет.
- **Константа** — Оставьте поле SNOW пустым и задайте только значение по умолчанию, чтобы записывать фиксированное значение в каждую синхронизируемую карточку независимо от ServiceNow. Например, сопоставьте `subtype` без поля SNOW со значением по умолчанию `hardware`, чтобы каждый извлечённый CI становился аппаратным ИТ-компонентом.

Значение по умолчанию приводится к типу целевого поля: поля `boolean`, `number` и `cost` разбирают значение; **поля с множественным выбором принимают список через запятую** (например, `web, backend` превращается в два значения). Чтобы объединить несколько полей ServiceNow в одно поле Turbo EA, используйте [вычисляемое поле](calculations.md), которое агрегирует промежуточные сопоставленные поля — слой сопоставления сопоставляет по одному столбцу за раз.
...
### Открытие записи ServiceNow

После синхронизации карточки на её вкладке **Ресурсы** появляется раздел **ServiceNow** с прямой ссылкой, которая открывает соответствующую запись (`https://<ваш-экземпляр>/<таблица>.do?sys_id=<id>`) в новой вкладке. Ссылка доступна только для чтения и поддерживается синхронизацией автоматически — настраивать ничего не нужно. Карточки, которые никогда не синхронизировались, этот раздел не показывают; вы по-прежнему можете добавить ссылку вручную в разделе **Ссылки на документы** на той же вкладке.
...
| Удалено | Карточек архивировано (мягкое удаление). К ним применяется обычный срок хранения архива: до его истечения карточку можно восстановить, после — она удаляется безвозвратно. |
