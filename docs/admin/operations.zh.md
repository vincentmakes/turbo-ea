...
与其盯着代码仓库，不如让实例主动告知：Turbo EA 每天检查是否有更新版本，并通过通知铃提醒管理员，附带版本号和发行说明链接。它从不下载或安装任何内容——上面的第 1 步仍然是先阅读变更日志。该检查默认开启，可在[管理 → 设置 → 常规](settings.md#update-notifications)中关闭；关闭后，气隙部署也不会再发出对外请求。
