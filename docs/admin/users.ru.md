...
!!! note "Вы не можете понизить в роли или деактивировать самого себя"
    В вашей собственной строке список **Роль** и кнопка **Деактивировать** отключены, и API также отклоняет оба изменения. Администратор, отказавшийся от собственной роли, тем же щелчком теряет разделы администрирования, и вернуть их может только другой администратор. Чтобы передать полномочия, попросите второго администратора сменить вам роль. Если войти как администратор не может уже никто, воспользуйтесь разделом [Восстановление доступа администратора](operations.md#recovering-administrator-access).
...
Действует защита «последнего администратора»: массовая смена ролей, после которой не останется ни одного активного администратора, отклоняется. То же касается деактивации или удаления последнего администратора. По той же причине ваша собственная учётная запись пропускается при массовой смене ролей и массовой деактивации.
...
Разрешения действуют и в адресной строке, и в меню. Страница, которую роль не может открыть, показывает уведомление **Доступ запрещён** со ссылкой на панель управления — независимо от того, открыта ли она из навигации, закладки или присланной ссылки. Скрыть пункт меню и заблокировать страницу — это всегда одно и то же решение.

По умолчанию разрешения роли действуют для всех типов карточек. Администратор может сузить их для отдельного типа — разрешив или запретив **Создание**, **Редактирование**, **Архивирование** и **Удаление** — на вкладке **Разрешения** этого типа карточек в [Метамодели](metamodel.md#permissions).
