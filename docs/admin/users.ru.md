...
- Если пароль не задан (учётная запись «Ожидание настройки»), генерируется одноразовый токен для установки пароля. Пользователь задаёт свой пароль при первом входе по ссылке — доставленной письмом-приглашением, если вы установили флажок, либо через копируемую ссылку, показанную после создания. Он также может получить её через пункт «Забыли пароль?» на странице входа, даже если у него никогда не было пароля
