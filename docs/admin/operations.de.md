...
Die eine Einstellung, die Sie vor der Umstellung bestätigen sollten, ist das Verbindungslimit. Das Backend läuft als einzelner Prozess und öffnet **bis zu `DB_POOL_SIZE + DB_MAX_OVERFLOW` Verbindungen — standardmäßig 30**. Der mitgelieferte `db`-Container erlaubt 100, im Standard-Stack tritt das also nie auf; verwaltete Einstiegstarife begrenzen die Datenbank häufig niedriger, und PostgreSQL antwortet dann mit `too many connections for database "turboea"`.
...
`-1` bedeutet «keine spezifische Begrenzung». Liegt das effektive Limit unter 30, erhöhen Sie es oder verkleinern Sie den Pool in `.env` — und lassen Sie einige Verbindungen für Backups und administrative Sitzungen frei:
...
So sehen Sie, was gerade tatsächlich verbunden ist:
...
Statt das Repository zu beobachten, kann die Instanz Sie informieren: Turbo EA prüft täglich auf eine neuere Version und benachrichtigt Administratoren über die Benachrichtigungsglocke — mit Versionsnummer und den Release Notes. Es wird dabei nie etwas heruntergeladen oder installiert; Schritt 1 oben beginnt weiterhin mit dem Lesen des Changelogs. Die Prüfung ist standardmäßig aktiv und lässt sich unter [Administration → Einstellungen → Allgemein](settings.md#update-notifications) abschalten, was auf Air-Gap-Installationen auch die ausgehende Anfrage unterbindet. Sobald das Upgrade abgeschlossen ist, wird jedem Benutzer über die Benachrichtigungsglocke mitgeteilt, dass die App aktualisiert wurde; er kann dort das Changelog der übersprungenen Versionen lesen. Jede Benachrichtigung zeigt weiterhin das Release, das sie angekündigt hat — die Notes stammen aus dem im Image mitgelieferten Changelog —, sodass eine alte Meldung nie mit den Notes des neuesten Releases beantwortet wird.
