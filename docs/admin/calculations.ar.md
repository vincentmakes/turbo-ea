...
### بيانات PPM على بطاقات Initiative { #ppm-data-on-initiative-cards }

يُتيح الجذر `ppm` للصيغ الوصول إلى بيانات وحدة PPM: سطور الميزانية والتكلفة، مفصولةً بين capex و opex وموزَّعةً حسب السنة المالية — وهو تفصيل لا تستطيع السمتان المجمَّعتان `data.costBudget` / `data.costActual` على البطاقة تقديمه — إضافةً إلى أرقام تنفيذ المبادرة: نسبة إنجازها الإجمالية، وحزم عملها ومهامها ومخاطرها، وأحدث تقرير حالة.
...
| `ppm.completion` | نسبة الإنجاز الإجمالية بالمئة — متوسط حزم العمل في المستوى الأعلى، وهو الرقم نفسه الذي تعرضه علامة تبويب النظرة العامة للمبادرة |
| `ppm.wbsCount`, `ppm.milestoneCount` | عدد حزم العمل وعدد المعالم الرئيسية |
| `ppm.taskCount`, `ppm.tasksTodo`, `ppm.tasksInProgress`, `ppm.tasksDone`, `ppm.tasksBlocked` | أعداد المهام، إجمالًا وحسب الحالة |
| `ppm.tasksOverdue` | المهام التي تجاوزت تاريخ استحقاقها ولم تُنجَز بعد |
| `ppm.riskCount`, `ppm.risksOpen`, `ppm.riskScoreMax` | مخاطر PPM: جميعها، وما لا يزال مفتوحًا منها، وأعلى درجة خطورة |
| `ppm.reportCount`, `ppm.reportDate` | عدد تقارير الحالة الموجودة، وتاريخ أحدثها (`None` ما لم يوجد تقرير) |
| `ppm.scheduleHealth`, `ppm.costHealth`, `ppm.scopeHealth` | مؤشرات الصحة في أحدث تقرير حالة: `onTrack` أو `atRisk` أو `offTrack` (`None` ما لم يوجد تقرير) |
...
# التقدم الإجمالي، ومؤشر تنفيذ من أحدث تقرير حالة
ppm.completion
...
* البطاقة التي ليست مبادرة تقرأ كل مقاييس `ppm` بقيمة `0` مع `byYear` فارغ، فتُعيد الصيغةُ على النوع الخطأ صفرًا بدل أن تُخفق. أما حقول أحدث تقرير فتُقرأ بقيمة `None` لا `0` — فـ«لا تقرير بعد» و«على المسار الصحيح» حقيقتان مختلفتان — لذا احمِها بـ `COALESCE`.

يؤدي تعديل سطر ميزانية أو تكلفة في PPM، أو حزمة عمل، أو مهمة، أو خطر، أو تقرير حالة إلى إعادة تشغيل حسابات المبادرة، فيتحدَّث كل ما يُشتق من هذه البيانات فورًا ويظهر التغيير في علامة تبويب **السجل** الخاصة بالبطاقة. أما البطاقات التي تقرأ بيانات PPM لبطاقة *أخرى* عبر علاقة فلا تُحدَّث — انظر [متى تُنفَّذ الحسابات](#when-calculations-run).

#### عرض تقدم المبادرة على بطاقة أو في بوابة { #show-initiative-progress-on-a-card-or-a-portal }

يقع تقدم المبادرة في علامة تبويب النظرة العامة الخاصة بها؛ ولا تعرض البطاقات والبوابات المنشورة سوى السمات. لوضع الرقم نفسه على البطاقة وفي [بوابة إلكترونية](web-portals.md):

1. في **Admin → Metamodel**، أضِف حقلًا من النوع **نسبة مئوية** إلى النوع Initiative — وليكن `progress` بالتسمية *التقدم*. يُعرض كشريط تقدم.
2. في **Admin → Calculations**، أضِف حسابًا على Initiative بالصيغة `ppm.completion`، واجعل ذلك الحقل هدفه، ثم فعِّله.
3. في خصائص البوابة، علِّم على الحقل لقائمة البطاقات ولعرض التفاصيل.

منذ ذلك الحين، يؤدي تعليم مهمة كمنجَزة، أو تعديل نسبة إنجاز حزمة عمل، أو حذف إحداها إلى تحريك الشريط أينما عُرضت البطاقة.
...
## متى تُنفَّذ الحسابات { #when-calculations-run }
...
* يتغير سطر ميزانية أو سطر تكلفة أو حزمة عمل أو مهمة أو خطر أو تقرير حالة في PPM على البطاقة، بالنسبة إلى Initiative.
