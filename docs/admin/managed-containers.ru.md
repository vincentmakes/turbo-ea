...
| Максимальная загрузка (импорт рабочего пространства до 2 ГБ) | Microsoft не документирует — проверьте импорт на 2 ГБ, прежде чем полагаться на него | **32 МиБ на запрос по HTTP/1** | Ограничения платформы нет |
...
**Amazon Bedrock.** Чтобы использовать Bedrock в качестве провайдера ИИ, прикрепите IAM-политику из раздела [Возможности ИИ](ai.md) к роли задачи стека; после этого Turbo EA аутентифицируется через эту роль, и API-ключ не нужен.
