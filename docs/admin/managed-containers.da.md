...
- **Edge'en lytter på 8920.** Dens standardport er 8080, som frontend-imaget allerede ejer i samme navnerum, så hver skabelon sætter `NGINX_HTTP_PORT=8920` og peger platformens ingress på den. Edge'en ejer stadig alle sikkerhedsheadere, grænsen på 512 MB for upload af arbejdsområdeoverførsler, indstillingerne for hændelsesstrømmen og `/mcp`-routingen — intet på platformssiden erstatter den.
...
| Største upload (arbejdsområdeimport op til 512 MB) | Ikke dokumenteret af Microsoft — test en import på 512 MB, før du stoler på det | **32 MiB pr. anmodning over HTTP/1** | Ingen platformsgrænse |
