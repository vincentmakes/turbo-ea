...
The catalogue is arranged in five sections — **Strategy, Planning & Transformation**, **Governance & Automation**, **Integrations**, **Regulations** and **Services** — each with its own heading, so you can see at a glance what kind of listing each one is; anything the catalogue has not classified appears under **Other** at the end. Within the sections, tag pills (free or commercial, plus topics such as integration) work as a filter: a filter bar appears above the list, clicking pills narrows it (several pills combine), sections with no match disappear, and **All** resets the view.

A **service listing** — an offering with nothing to install, such as a support subscription — is sold and licensed like an extension but carries no bundle. Its tile and details window offer **Buy** only, show no version, and say so plainly; once the purchase is confirmed the same entitlement chip (*Renews on …*) appears on the tile, and your confirmation email explains how to use what you bought.
...
- `core.risks.read` — read the risk register (reference, title, category, level, status, owner, linked cards).
- `core.risks.write` — raise a risk and link it to cards, for example a rule that files a risk when an application crosses a cost threshold without an owner. Write implies read. A risk raised this way shows **Extension** as its source in the register, gets its reference from the same sequence as everyone else's, and its owner receives the usual todo and notification. An extension may later update the title, description, owner and target date of the risks **it** raised — never anyone else's, and never a risk's status: moving a risk through its workflow, accepting it and closing it stay with your users.
- `core.notifications.send` — send a notification to named people, for example "a rule you set up just fired". It arrives as the **Extension notice** type, or as a type the extension declares for itself, which then gets a row of its own in each person's notification preferences (in-app on, email off by default) — either way each person decides whether it reaches their bell, their inbox or an extension channel, and can switch it off. Some of these notifications open their details in the app when clicked, offering the related card and the extension's own page only to people who may open them. The link in such a notification can only point inside Turbo EA.
- `core.stakeholders.write` — assign or remove stakeholder roles on cards, using exactly the roles the card type defines. This is a separate grant from `core.cards.write` because a stakeholder role gives the person permissions on the card.
- `core.surveys.read` — read a survey's name, status and how many of its requests were answered.
- `core.surveys.write` — send a data-maintenance survey to the stakeholders of a set of cards, for example a scheduled rule that asks the owners of every mission-critical application below the data-quality target to complete their cards — one request per person instead of one todo per card. Write implies read. The survey appears in **Admin → Surveys** with no author, its fields are exactly those the card type defines, and each person gets the usual survey notification. An extension can never close a survey or apply its answers — those stay with you — and rolling back the run that sent it (Admin → Audit log) closes the survey and withdraws the requests nobody had answered yet.
