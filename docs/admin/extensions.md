...
- `core.notifications.send` — send a notification to named people, for example "a rule you set up just fired". It always arrives as the **Extension notice** type, so each person decides in their own notification preferences whether it reaches their bell, their inbox or an extension channel, and can switch it off. The link in such a notification can only point inside Turbo EA.
