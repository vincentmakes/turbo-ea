...
| **CVE** | Common Vulnerabilities and Exposures — identificatore pubblico di una vulnerabilità software nota (es. `CVE-2024-12345`). Sicurezza e conformità di TurboLens ricerca i CVE per card nell'NVD del NIST |
| **CVSS** | Common Vulnerability Scoring System — punteggio standard del settore da 0.0 a 10.0 che valuta la gravità di un CVE. TurboLens registra il punteggio base, il vettore di attacco e i sotto-punteggi di sfruttabilità / impatto |
| **NVD** | NIST National Vulnerability Database — archivio del governo USA dei record CVE utilizzato da TurboLens per recuperare i dati di vulnerabilità. Limiti: 5 richieste / 30 secondi senza autenticazione, 50 / 30 s con `NVD_API_KEY` |
