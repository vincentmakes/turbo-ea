...
| **Activité d'appel** | Une tâche BPMN au bord épais qui invoque un processus défini de façon autonome -- la manière propre à BPMN de dire « cette étape est un autre processus », et le seul type d'étape pour lequel Turbo EA demande le processus métier au moment où vous la placez. Toute autre étape lie un processus de la même façon, depuis le panneau des propriétés, le menu contextuel ou le tableau des étapes ; la publication du flux crée une relation *appelle* entre les deux processus |
...
| **Macro-capacité** | Regroupement de niveau 0 au-dessus de L1 dans le Catalogue des capacités. Atterrit comme une fiche `BusinessCapability` avec `attributes.capabilityLevel = "Macro"` et un `catalogueId` préfixé `MC-`. Relâche la limite de profondeur de hiérarchie à 6 |
| **Vue de dépendances en couches (LDV)** | Notation maison de Turbo EA pour les diagrammes de dépendances : fiches regroupées dans les quatre couches EA comme des couloirs de nage, colorées par type de fiche, avec les fiches proposées rendues comme des nœuds à bordure pointillée et un badge vert « NEW ». Utilisée par le rapport Dépendances, la section dépendances du détail de fiche et l'architecture cible de TurboLens Architect |
