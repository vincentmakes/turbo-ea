...
| **事件定义** | BPMN 事件的类型：消息、定时器、信号、错误、升级、条件、链接、补偿、取消或终止。在流程步骤表中显示于元素类型旁边，并附带该事件所引用的消息、信号或错误的名称 |
| **泳道** | 泳池内部的一条水平带，用于归集由某一角色、团队或部门执行的步骤 |
| **调用活动** | 带粗边框的 BPMN 任务，用于调用独立定义的流程 —— 这是 BPMN 表达「此步骤是另一个流程」的原生方式，也是 Turbo EA 在放置时唯一会询问业务流程的步骤类型。其他步骤同样可以链接流程，通过属性面板、上下文菜单或步骤表格；发布流程时会在两个流程之间创建「调用」关系 |
| **消息流** | BPMN 协作中两个泳池之间交换的消息。列在已发布流程的元素表格下方，可在此关联到承载它的接口卡片 |
| **泳池** | BPMN 协作中的一个参与方 —— 客户、部门或合作伙伴系统 —— 绘制为容纳该方自身流程的容器 |
| **业务能力** | 组织能做什么，不论如何实现 |
...
| **文件附件** | 通过资源选项卡直接上传到卡片的二进制文件（文档、表格、演示文稿、OpenDocument 和早期 Office 文件、图片、压缩包、Outlook 和电子邮件、CSV、Markdown、JSON 和 XML，最大 20 MB） |
...
| **周期性待办** | 设为按计划重复的卡片待办（每 N 天/周/月/年）。完成后会生成下一次待办；较远的待办保持「已排程」状态，直到提前期窗口开启，届时会出现在任务页面并通知负责人 |
