...
Se você quer uma recomendação inicial que os donos então validam, o recurso [Cálculos](../admin/calculations.md) pode derivar um valor TIME padrão a partir das duas dimensões integradas de adequação — `functionalSuitability` (faz o que o negócio precisa?) e `technicalSuitability` (a tecnologia subjacente é sólida?). Este é o quadrante TIME canônico do Gartner.

Configure o cálculo em **Admin → Metamodelo → Cálculos** com **Tipo alvo = `Application`**, **Campo alvo = `timeModel`** e a fórmula:
...
O que ela faz — o posicionamento de quatro quadrantes:

| Adequação funcional | Adequação técnica | → TIME |
|---------------------|-------------------|--------|
| Alta | Alta | **Invest** — estratégico, sólido — financie o crescimento |
| Alta | Baixa | **Migrate** — o negócio precisa, mas a tecnologia está apodrecendo — substitua |
| Baixa | Alta | **Tolerate** — funciona bem mas o valor de negócio está desaparecendo — deixe como está |
| Baixa | Baixa | **Eliminate** — nem necessário nem sólido — descomissione |

A fórmula é executada automaticamente toda vez que um card é salvo, e o Turbo EA marca `timeModel` como somente leitura com um selo "calculado" para que os usuários não se desviem acidentalmente da regra. O mesmo exemplo está documentado (e pronto para copiar e colar) em [Admin → Cálculos](../admin/calculations.md#exemplos-de-formulas).
...
O padrão híbrido: mantenha o cálculo ligado enquanto você está construindo o inventário e tem majoritariamente dados de adequação; desligue-o para o workshop de validação; depois deixe-o desligado para que as decisões manuais permaneçam.
