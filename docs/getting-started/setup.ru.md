...
Turbo EA может запуститься пустым (только встроенная метамодель) или с демонстрационным набором данных **NexaTech Industries**, идеальным для оценки, обучения и исследования возможностей. Набор включает логотипы продуктов на приложениях и ИТ-компонентах, три опубликованных веб-портала (каталог приложений, портфель поставки и дом процессов), одну опубликованную диаграмму, сгруппированные и избранные диаграммы, а также карту способностей, начинающуюся с макроуровня.
...
!!! tip "Запуск в Kubernetes?"
    [Helm-чарт](../admin/kubernetes.md) устанавливает Turbo EA в EKS, AKS, GKE или любом другом кластере с именно таким внешним PostgreSQL; на той же странице разобрано каждое облако. Без кластера страница [Управляемые контейнерные сервисы](../admin/managed-containers.md) описывает Azure Container Apps, Cloud Run и ECS Fargate.
