...
Turbo EA 可以以空状态启动（仅内置元模型）或加载 **NexaTech Industries** 演示数据集，非常适合评估、培训和探索功能。该数据集包含应用程序和 IT 组件上的产品徽标、三个已发布的门户网站（应用程序目录、交付组合和流程屋）、一个已发布的图表、已分组和收藏的图表，以及从宏观层级开始的能力地图。
