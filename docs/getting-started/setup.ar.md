...
يمكن أن يبدأ Turbo EA فارغًا (بالنموذج الفوقي المُضمَّن فقط) أو مع مجموعة بيانات العرض التوضيحي **NexaTech Industries**، وهي مثالية للتقييم والتدريب واستكشاف الميزات. تتضمن المجموعة شعارات المنتجات على التطبيقات ومكوّنات تقنية المعلومات، وثلاث بوّابات ويب منشورة (دليل التطبيقات، ومحفظة التسليم، وبيت العمليات)، ومخطّطًا منشورًا، ومخطّطات مُجمّعة ومُفضّلة، وخريطة قدرات تبدأ من المستوى الكلّي.
...
!!! note "تُحمَّل البيانات التجريبية مرة واحدة"
    يعمل كل مُحمِّل **مرة واحدة لكل تثبيت** ويسجّل ذلك. حذف محتوى تجريبي — مخطط
    نموذجي أو استبيان تجريبي — نهائي: لن يعود عند إعادة التشغيل التالية، حتى لو
    بقي `SEED_DEMO=true` مضبوطاً. لاستعادة مجموعة البيانات التجريبية، أعد تعيين
    قاعدة البيانات (راجع *إعادة التعيين وإعادة التحميل* أدناه).
...
## السماح بتضمين المخططات (اختياري)

يمكن تضمين [مخطط منشور](../guide/diagrams.md) في موقع آخر — صفحة Confluence أو بوابة إنترانت — لكن فقط إذا حدّدت ذلك الموقع أولاً. افتراضياً، لا يُسمح لأي موقع خارجي بوضع Turbo EA داخل إطار على الإطلاق.

```dotenv
TURBO_EA_EMBED_ALLOWED_ORIGINS=https://yourcompany.atlassian.net
```

افصل بين عدة أصول بفواصل، ثم أعد تشغيل الحزمة ليسري التغيير.

ينطبق ذلك **فقط** على صفحات المخططات المنشورة. أما التطبيق نفسه — بما في ذلك محرّر المخططات — فيبقى غير قابل للتضمين في كل الأحوال، وتظل الروابط المنشورة تعمل عند فتحها مباشرةً حتى دون ضبط هذا الإعداد.
