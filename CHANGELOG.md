...
- **The Hierarchy link types block on the Relation Types tab looks like the relation list under it.** Each card type is now an outlined row with its link types and a single icon button on the right, matching the relation rows, and the paragraph of explanation above the block is gone — it already appears inside the editor that button opens.
