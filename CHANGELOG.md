...
- **Connections that skip a layer route around cards, not over them.** A dependency spanning non-adjacent layers used to draw one long line straight across everything in between. It now takes a corridor through the gaps between cards — dodging sideways only where a card actually blocks its column and heading straight for its destination otherwise — the way DrawIO's hierarchical layout routes long edges through its ranks. Corridors keep their distance from each other and from other connections sharing the same gap.
