...
- **AI Inventory dashboard** at `/grc?tab=governance&sub=ai`. Architected as a **pure registry**: detection runs in exactly one place — the Compliance Scanner's EU AI Act pass — and the inventory reads from the resulting cache. Aligned with NIST AI RMF (Govern · Map · Measure · Manage), ISO/IEC 42001 (registered AI inventory + impact assessment) and the EU AI Act (Art. 11 / Annex IV obligations): the inventory is the governance artefact; risk treatment is a downstream consumer. Includes:
  - **Header** with a *Last scanned* timestamp, an info banner explaining that detection happens during the compliance scan, a deep-link to the scanner, and a **«Classify a card as AI»** action that force-includes shadow-AI systems the detector can't catch (a curation action, not detection).
  - **Four KPI tiles** — Total / High risk / **Pending review** / Unowned. *Pending review* counts AI cards missing either a risk classification or a documented intended purpose (the actual EU AI Act Annex IV / ISO 42001 6.1.4 gap), replacing the vaguer *Unclassified* tile.
  - **Three filter dropdowns**: risk class, lifecycle, detection method.
  - **Paginated table** of AI systems with chips for risk class (coloured by EU AI Act severity), role, lifecycle, detection method (`subtype` / `semantic` / `override`) and the LLM signal that triggered detection; per-row owner count with explicit "No owner" warning. Card-name cells link to `/cards/:id`.
  - **Risk Register cross-link panel** below the table — surfaces open risks (TOGAF Phase G) linked to AI-bearing cards, with deep-links back into `/grc?tab=risk`. Honours that AI Risk is a slice of the broader Risk Register without burying the inventory under it.
- **Detector emits EU AI Act risk-tier verdict per card.** The `detect_ai_bearing_cards` LLM pass classifies each AI system into `unacceptable / high / limited / minimal` (Art. 5 / Annex III / Art. 50 heuristics in the prompt). The compliance scan writes `aiRiskClass`, `aiSystemRole` and `aiLifecycleStage = "production"` (for ACTIVE cards) back to the metamodel on the card without clobbering admin-set values — so the inventory columns are populated by default instead of staying empty.
- **`POST /grc/ai-inventory/classify/{card_id}`** — force-include a card in the AI Inventory. Sets `aiClassificationOverride="yes"` and routes the upsert through the same persistence helper the scanner uses. Useful for shadow-AI hidden behind generic «Service» cards.
- **`GET /grc/ai-risks`** — returns open risks whose linked cards include at least one AI-bearing system, capped to 10. Powers the cross-link panel.
- **REST endpoints under `/api/v1/grc`**:
  ...
  - `GET /grc/ai-inventory/kpis` — aggregate counts for the dashboard tiles. `last_discovered_at` reflects the latest successful compliance scan that included EU AI Act.
- **Post-scan callout in the Compliance Scanner**: when a scan completes with EU AI Act in scope, an inline link appears under the scan card — *"AI Inventory updated with N classified systems — View inventory →"* — so the contract between the two pages is discoverable.
...
- **Migrations** — `076` grants default GRC role permissions, `077_ai_governance_fields` backfills the AI Governance section onto existing rows (idempotent and admin-customisation-safe), `078` creates the `ai_governance_classifications` cache table, `079` backfills `aiSystemRole` and `aiLifecycleStage` from any pre-existing classification rows (idempotent; doesn't clobber admin-set values).
