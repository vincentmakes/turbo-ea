...
### Added
- **Add several relations in a row without reopening the picker.** The picker used to close after every single relation, so linking one application to nineteen departments meant nineteen rounds of click **+**, search, select ([#918](https://github.com/vincentmakes/turbo-ea/discussions/918)). It now stays open and each pick is saved immediately. What you have just added is listed as removable chips directly above the search box — like entering tags — so it stays visible while you carry on, and a chip's **×** undoes that one add. **Done** closes the row. Relation types that only permit one link still close after the first.
