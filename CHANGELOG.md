...
- **AI Inventory dashboard** at `/grc?tab=governance&sub=ai`. Aligned with NIST AI RMF (Govern · Map · Measure · Manage), ISO/IEC 42001 (registered AI inventory + impact assessment) and the EU AI Act (Art. 11 / Annex IV obligations): the inventory is the governance artefact; risk treatment is a downstream consumer. Includes:
  - **Header** with Last discovered timestamp, a **Run discovery** button (semantic LLM pass), and a **«Classify a card as AI»** action that force-includes shadow-AI systems the detector can't catch.
...
- **Detector emits EU AI Act risk-tier verdict per card.** The `detect_ai_bearing_cards` LLM pass now classifies each AI system into `unacceptable / high / limited / minimal` (Art. 5 / Annex III / Art. 50 heuristics in the prompt). Discovery writes `aiRiskClass`, `aiSystemRole` and `aiLifecycleStage = "production"` (for ACTIVE cards) back to the metamodel on the card without clobbering admin-set values — so the columns are populated by default instead of staying empty.
- **`POST /grc/ai-inventory/classify/{card_id}`** — force-include a card in the AI Inventory. Sets `aiClassificationOverride="yes"` and writes a cache row marked `method="override"`. Useful for shadow-AI hidden behind generic «Service» cards.
