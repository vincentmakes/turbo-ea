...
- **Manage translations now covers hierarchy link types.** The button has moved to the top of the Relations tab, where it governs everything below it, and the dialog has gained a separate **Hierarchy link types** section listing each hierarchical card type's vocabulary alongside the relation verbs. Previously the link types an administrator had just defined were the one metamodel label with no way to translate them.
