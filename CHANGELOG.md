...
- **A call activity now links to the process it calls — picked, never typed.** BPMN's one construct for "this step is another process" is the call activity, and until now Turbo EA drew it and ignored it. Placing one in the process flow editor asks which Business Process it invokes, straight from the inventory; the properties panel shows a **Called process** group with **Open** and **Clear**, the context pad of a call activity carries **Link process**, and the steps table of a published flow (and the pre-link table of a draft) has a **Calls** column with the same picker. Every rendering of the link — the table chip, the badge on the diagram, the chip in the process navigator — drills down into the callee's flow, and publishing creates a new built-in **calls** / *is called by* relation between the two processes, so the call graph shows in the Dependencies view. A diagram imported from another tool keeps that tool's process reference as a hint until the matching process is picked. The demo Order-to-Cash flow now calls its Credit Check process.
