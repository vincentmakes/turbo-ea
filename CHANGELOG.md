...
### Added
- **In-product extension Store tab.** Admin → Extensions now opens on a **Store** tab that browses the vendor's extension catalogue out of the box: each package shows its description and price, **Buy** opens payment in a new browser tab, and **Install**/**Update** downloads the bundle server-side and runs it through the exact same signature verification and dry-run preview as a manual upload. No account, token, store connection, or configuration is involved — the instance only reads public static files from the built-in catalogue URL, and the license still arrives by email and is pasted on the **Installed** tab, which keeps the full file-based flow for bespoke extensions and air-gapped instances (an unreachable store simply degrades to a friendly hint).
- **Automatic license renewal.** Licenses bought through the Store renew themselves: while the subscription is paid, a daily background check fetches the extended license from the store and applies it through the same signature verification as a pasted one — nothing to re-enter. Renewal strictly follows payment (a lapsed or past-due subscription never extends anything) and is extension-only (a fetched license can never shorten or revoke what was already paid for). Manually issued licenses and air-gapped instances are untouched — they renew by pasting the emailed license file, with the existing 30-day grace window as cushion.
- **"See it in action" demo links in the Store.** An extension whose catalogue entry carries a `demo_url` shows a demo link (e.g. a short video) on its Store card and on the storefront website — a lightweight alternative to a trial.
- **Disabling or uninstalling a content pack now visibly deactivates it.** The pack's card types and relation types are soft-hidden from the metamodel (exactly like a metamodel soft-delete) when the extension is disabled or uninstalled, and come back when it is re-enabled or reinstalled. Cards and extension data are still never deleted.
- **Guided, one-click purchase and install flow.** Buying from the Store tab now applies the license automatically once the payment is confirmed (no pasting on connected instances), **Install** asks for the license first when an extension isn't entitled yet — with buy-now or paste-a-license right in the dialog — and **installs in a single click**: a store bundle is downloaded, verified, and applied straight through to *installed* without a second confirmation (the dry-run preview + explicit Apply step is kept for manual `.teax` uploads, and store installs still stop for review if the preview flags any failure). Manual `.teax` upload moved to the Store tab as **Install from file…**; the Installed tab now focuses on managing what's installed, with per-extension license status, **Renew**, and license entry on each row.
...
- **Extensions can unlock advanced field authoring.** Two new metamodel capabilities ship as inert core plumbing that only an installed, licensed extension can switch on (declared as `grants` in its manifest): **collapsible field help text** (guidance shown under a field during data entry, `metamodel.field_help`) and **custom field types** (an extension contributes a namespaced `ext.*` field type — e.g. a configurable rating — through the new UI SDK 1.1 `fieldTypes` extension point, `metamodel.custom_field_types`). The free core never exposes a way to author these. Rendering is always unconditional and degrades to a read-only view of the stored value if the extension is later disabled or unlicensed, so data is never blanked or lost.
- **Extension pages can appear inside the Reports menu.** A UI-plugin route may declare an optional `navGroup: "reports"` placement hint so its nav entry renders under **Reports** alongside the built-in reports (desktop dropdown and mobile drawer), instead of as its own top-level item. Placement is whitelisted, back-compatible (omit for the current top-level behaviour), and still gated by the entitlement-filtered extension registry — an uninstalled or unlicensed extension shows nothing.
