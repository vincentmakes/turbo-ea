...
- **A long card name in the dependency view no longer runs under the logo.** A name is written on one line across the card's full width, so a logo in the corner sat on top of any name long enough to reach it. The logo now has a band of its own above the name, still hard against the card's left edge where a mark belongs — the card is a little taller to hold it, which also gives the lines under a name more room — and the card-type mark has moved along the top edge to sit beside the lifecycle dot.
