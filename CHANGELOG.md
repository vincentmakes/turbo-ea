...
- **A relation's values now have somewhere to live in the inventory's Excel export.** The `Relations` sheet is the only place a relation's own values can be read or filled in, and it listed a relation only when its type *already* carried values. Relation types that carry values also have no column on the card sheet — that cell is a list of card names, with no room for a value — so the moment you gave a relation type its first value, that value had no row anywhere in the workbook, and the relation looked as though it had vanished from the export ([#1089](https://github.com/vincentmakes/turbo-ea/issues/1089)). The sheet now lists **every** relation of the exported cards, so a relation type you have just configured is already there waiting, and relations pointing *at* an exported card are included too — previously an Application export silently dropped every «Organization uses Application» row. Its attribute columns are also limited to the relation types actually in the workbook rather than every one defined in the instance, and a relation's free-text description now survives the round trip whatever its type. Adding a `rel:` column by hand for a relation type that carries values explains why it is ignored instead of reporting an unknown relation type.
