...
### Added
- **Transition Planning** — a manual (no-AI) planning tool in EA Delivery for modelling landscape changes as a single before/after diagram. Optionally name the business objectives the change supports, pick scope cards (an Organization, a Capability, applications), capture a snapshotted baseline of the surrounding landscape, then plan changes: add or remove cards, replace an application with a successor (which inherits its relations), and add or cut relations. The Layered Dependency View renders the merged before/after picture with change indicators — red cross for removals, green plus for additions, blue swap arrows for replacements. Plans live in the Initiative workspace's Deliverables section and can be committed: one click creates the Initiative, the proposed cards and relations, an optional draft Architecture Decision Record documenting the change, and stamps an end-of-life lifecycle date on removed/replaced cards so the roadmap reflects the plan (nothing is archived or deleted). New `transition_plans.view` / `transition_plans.manage` / `transition_plans.commit` permissions; plans transfer with Workspace Transfer. Creating, editing, and committing plans is unlocked by an installed, licensed extension (grant `delivery.transition_planning`); viewing existing plans is always available and a licence lapse never removes data.
- **Transition Planning — holistic insights.** Plans now read as architecture decisions, not just diagrams: **derive scope top-down** from the selected objectives (pulls in the capabilities they drive and the applications supporting them); a **gap analysis** (Added / Removed / Changed / Retained); a **blast-radius** warning when removing or replacing a card (what depends on it, from the card's own impact analysis); **capability-coverage gaps** (flags any capability that loses all its supporting applications); and an **estimated annual cost delta** plus a **risk count on affected cards**. Proposed cards can carry an estimated annual cost that lands on the created card at commit. All of it is surfaced in the editor and the shareable preview, and folded into the committed ADR.
