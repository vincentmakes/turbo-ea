...
- **A logo on a diagram no longer overflows the card it belongs to.** The picture is now built for the size of the cell it lands on, so the card-type mark stays inside the card on a group child, a drilled-down child and a card exported from the dependency view — all of which are smaller or taller than a plain card — and it is rebuilt when you resize a card by hand. The logo itself is drawn smaller and sits higher, on the same line as the card-type mark, with a backing plate that hugs it rather than framing it; the card-type mark has come in off the rounded corner it used to hug.
- **A long card name in the dependency view no longer runs under the logo.** A name is written on one line across the card's full width, so a logo in the corner sat on top of any name long enough to reach it. The logo now has a band of its own above the name — the card is a little taller to hold it, which also gives the lines under a name more room — and the card-type mark has moved along the top edge to sit beside the lifecycle dot.
