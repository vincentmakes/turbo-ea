...
- The catalogue browser now mirrors the visual language of the Business Capability map report: an outer card with 8 px corner radius, a navy header carrying the L1 name and white text, and child rows rendered as small white "mini-cards" with a per-level coloured left-border accent. Capability names render in the BusinessCapability brand navy `#003399` and the accent strip lightens progressively with depth (L1 darkest, L5 lightest) — same discrete-palette idea as the hierarchy chips on regular cards. Names wrap onto multiple lines instead of being truncated.
