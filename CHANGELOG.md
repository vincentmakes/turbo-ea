...
- **The Hierarchy link types block on the Relation Types tab looks like the relation list under it.** Each card type is now an outlined row showing how many link types it has as a single count chip, with one icon button on the right — the same way a relation row summarises its values, and no longer a chip per value, which grew the row without bound once a vocabulary got long. Hover the chip to read the names; open the editor to change them. The paragraph of explanation above the block is gone too — it already appears inside that editor.
