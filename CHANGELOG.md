...
- **Guided purchase and install flow.** Buying from the Store tab now applies the license automatically once the payment is confirmed (no pasting on connected instances), **Install** asks for the license first when an extension isn't entitled yet — with buy-now or paste-a-license right in the dialog — and continues on its own afterwards. Manual `.teax` upload moved to the Store tab as **Install from file…**; the Installed tab now focuses on managing what's installed, with per-extension license status, **Renew**, and license entry on each row.
