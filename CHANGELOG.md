...
### Added
- **Organizations can now be linked at process-flow level** — a new *Organization* column in the flow's step table, right next to the existing Application / Data Object / IT Component linking. Steps in a BPMN swim lane share one organization: linking, changing, or removing the organization on any step applies to the whole lane (and the lane binding can also be set by selecting the lane in the BPMN modeler), following the industry pattern of binding lanes to governed org objects; steps without a lane link an organization individually. The binding is stored once per lane and every step's shown organization is computed from it, so nothing is maintained twice and lane and step can never contradict each other. Each link additively syncs the existing BusinessProcess → Organization relation so it shows up on both cards and in dependency reports. Lane names remain free text — binding is optional and existing diagrams are unaffected. Draft flows support lane pre-linking with the bindings applied on publish, the Process Navigator shows lane/step organization chips, and lane bindings transfer with workspace export/import.
