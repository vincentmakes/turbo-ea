...
- The catalogue browser now mirrors the visual language of the Business Capability map report: an outer card with 8 px corner radius, a navy header carrying the L1 name and white text, and child rows rendered as small mini-cards washed in a very-light BusinessCapability navy tint (`rgba(0, 51, 153, 0.04)`) with the brand `#003399` as the text colour. Names wrap onto multiple lines instead of being truncated. The catalogue page now follows the app-wide theme — under dark mode the L1 cards switch to the `#1e1e1e` paper background, rows pick up a lifted-lavender tint, and checkbox / divider colours move with them, while the L1 navy header keeps its white-on-navy contrast in both modes.
