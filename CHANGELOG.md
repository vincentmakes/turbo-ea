...
- **A logo on a diagram no longer overflows the card it belongs to.** The picture is now built for the size of the cell it lands on, so the card-type mark stays inside the card on a group child, a drilled-down child and a card exported from the dependency view — all of which are smaller or taller than a plain card — and it is rebuilt when you resize a card by hand. The logo itself is drawn a little smaller, and shrinks further on a short card rather than filling it.
