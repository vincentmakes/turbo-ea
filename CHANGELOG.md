...
- The catalogue browser uses a light-blue, level-graded palette for the L1 / L2 / L3 / L4 / L5 header strips (Material Blue 200 → Blue 50 → near-white in light mode; a muted dark-blue family in dark mode). The brand navy `#003399` reads as the foreground on light mode and lifts to a lavender tone in dark, with all rows at 8 px corner radius — matching the Business Capability map report's visual language. Names wrap onto multiple lines instead of being truncated. The page follows the app-wide theme: dark mode swaps the L1 card paper to `#1e1e1e`, the per-level row tints to the dark-blue palette, and the checkbox / ± stepper to lavender so they stay legible.
- Existing-card detection (the green tick + the import re-link logic) now matches on `attributes.catalogueId` first, falling back to a case-insensitive display-name match. So a card previously imported through the catalogue and then renamed by hand is still recognised as "already exists" — and still gets re-parented under a newly-created catalogue parent in the same import.
