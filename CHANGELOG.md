...
- **A field a calculation writes is read-only the moment the calculation exists — on card detail and in the inventory grid.** The list of calculated fields was fetched once per browser session, so a calculation added in Admin → Calculations left its target editable on every card until a hard reload, and a value typed there was silently overwritten by the calculation on save. The admin page now refreshes that list after every change, every card opened re-checks it in the background, and the inventory grid's edit mode and mass edit apply the same lock the card detail does.
- **The percentage editor is a compact control.** It stretched across the whole section as a bare line with the number box detached at the far end. It is now a bounded control: the exact-value box carrying the field label, and beside it a slider with marks at 0, 25, 50, 75 and 100 and a value label, writing straight through so what it shows is always what will be saved.
