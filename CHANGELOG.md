...
### Fixed
- **Demo-seeded instances no longer show an inflated Average Completion.** The demo seed scored cards with an approximation that ignored the relations, tags, and stakeholders buckets, so a fresh demo install displayed a higher average than the canonical scorer (and than any instance the data was exported to). Seeding now finishes with a canonical rescore of every card. (discussion #667)
- **Existing installs are healed automatically.** On the first startup after upgrading, a one-shot background task rescores every card with the canonical scorer (guarded by a settings marker so later boots are a no-op), correcting Average Completion values left behind by the demo seed's approximation or by older workspace imports. (discussion #667)
