...
- Dependencies report: the impact of a retirement is now visible on its dependents — relations into retired cards render as dashed red (the dependencies the transformation severs), and surviving cards that lose one are badged **AT RISK**, even when retired cards themselves are hidden. Arriving cards are also easier to spot: a purple glow, tinted background, and the **PLANNED** badge on the card's top edge.
