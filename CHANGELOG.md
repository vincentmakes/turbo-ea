...
- **Connections no longer lie on top of each other.** Horizontal runs that share the band between two layers used to sit on exactly the same line, reading as one merged wire; each is now placed on its own line, kept clear of card bodies where possible, while lines far enough apart still share a level so a wide landscape doesn't fan out needlessly. Vertical runs that would trace the same column are separated by sliding one connection to the neighbouring exit point — without ever breaking the left-to-right order, and without adding a single extra bend.
- **Connections that skip a layer route around cards, not over them.** A dependency spanning non-adjacent layers used to draw one long line straight across everything in between. It now picks a single corridor that is clear past every row of cards it must cross — the way DrawIO's hierarchical layout routes long edges through its ranks — and may connect through a card's **side** when the corridor runs right beside it, so going around a busy layer costs one or two turns instead of a zigzag. Corridors keep their distance from each other and from other connections sharing the same gap, and connection lines are always exactly horizontal or vertical.
- **Fewer crossings, straighter hubs.** The layout now finishes each pass with the crossing-reduction step from classic hierarchical layouts: neighbouring cards in a row swap places whenever that unties their connections, and the busiest card in a row claims its exact column while sparsely connected cards yield around it — so the most important dependency lines run dead straight.
