...
- **A cross-cutting card health flag has exactly one definition, in `backend/app/services/card_flags.py`.** "Orphaned", "stale" and — since 2.122.0 — "missing EOL" are each read by three surfaces: an inventory filter (`GET /cards?<flag>=true`), a Data Quality KPI tile with its drill-down panel (`GET /reports/data-quality` + `?scope=`), and a deep link from one to the other (`buildInventoryFlagUrl`). If the definitions ever diverged, clicking a count of 37 would land on a grid showing something else, which is why the SQLAlchemy condition and the Python predicate live side by side in that module and every caller imports them. **EOL coverage** is `has_eol_coverage`: an endoflife.date link (`attributes.eol_product` *and* `eol_cycle`) **or** a manual `lifecycle.endOfLife` date — the manual date is the company's own decommission date, which the EOL report has always accepted as a source, so "missing" has to mean neither. `EOL_TYPES` (Application, ITComponent) is the eligible population — EOL is not a metamodel concept, so it cannot be derived from `fields_schema` — and its frontend mirror is `frontend/src/lib/eol.ts`, which also owns the per-status colours the EOL report and the inventory column share. Resolving a *status* (as opposed to coverage) means an outbound call to endoflife.date, so it lives in `backend/app/services/eol_service.py` behind a 30-minute per-product cache, and `GET /eol/card-status` commits its session before calling it. Guard tests: `backend/tests/services/test_eol_coverage.py`, `tests/api/test_reports_eol.py`, `test_cards.py::TestEolMissingFilter`.
