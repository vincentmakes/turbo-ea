...
- **In-product Store tab = read-only proxy over public static files — never a store connection.** `EXTENSION_STORE_URL` points at static vendor hosting (`catalog.json` + public `.teax` bundles); `GET /admin/extensions/store/catalog` proxies + annotates it (installed version, entitlement state, update available) and `POST /admin/extensions/store/install` downloads the bundle server-side into the SAME verify → dry-run preview → apply pipeline as a manual upload (`_persist_upload` + `_verify_and_preview_job`). Bundle downloads are restricted to the configured store origin (SSRF guard). There are NO accounts, tokens, redeem codes, or online activation — bundles are inert without a signed license, payment happens in a new browser tab via the catalogue's `payment_link`, and the license is still pasted as a file on the Installed tab. Leave `EXTENSION_STORE_URL` unset (air-gapped) and the Store tab degrades to a hint; the file-based flow is always fully functional. Anything about how licenses are sold/distributed commercially lives outside this repo, in the vendor's private extensions repo.
...
| `EXTENSION_STORE_URL` | *(empty)* | Base URL of the vendor's static extension catalogue (`catalog.json` + public `.teax` bundles). Powers the Store tab on Admin → Extensions (read-only, no account/token). Leave unset on air-gapped instances — the file-based install flow is unaffected. |
