...
- **In-product Store tab = read-only proxy over public static files — never a store connection.** The catalogue URL is **baked into `config.py` (`DEFAULT_EXTENSION_STORE_URL`) so the store works on every install with zero configuration**; the `EXTENSION_STORE_URL` env var only overrides it (mirror/staging) or disables it (`off`/`none`/`disabled`/`false` → parsed to `""` by `_parse_extension_store_url`). `GET /admin/extensions/store/catalog` proxies + annotates it (installed version, entitlement state, update available) and `POST /admin/extensions/store/install` downloads the bundle server-side into the SAME verify → dry-run preview → apply pipeline as a manual upload (`_persist_upload` + `_verify_and_preview_job`). Bundle downloads are restricted to the configured store origin (SSRF guard). There are NO accounts, tokens, redeem codes, or online activation — bundles are inert without a signed license, payment happens in a new browser tab via the catalogue's `payment_link`, and the license is still pasted as a file on the Installed tab. Air-gapped instances need no config: an unreachable store degrades to a friendly hint and the file-based flow is always fully functional. Anything about how licenses are sold/distributed commercially lives outside this repo, in the vendor's private extensions repo.
...
| `EXTENSION_STORE_URL` | *(built-in default)* | Base URL of the vendor's static extension catalogue (`catalog.json` + public `.teax` bundles). Powers the Store tab on Admin → Extensions (read-only, no account/token). The default is baked into `config.py` (`DEFAULT_EXTENSION_STORE_URL`) so the store works out of the box; set to a URL to point at a mirror, or to `off` to suppress the outbound fetch. Air-gapped instances need not touch it — an unreachable store degrades to a hint and the file-based flow is unaffected. |
