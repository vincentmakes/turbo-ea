...
- **A call activity links to the Business Process it invokes; `calledElement` carries the card UUID.** BPMN's one construct for "this step is another process" is `bpmn:CallActivity`, and its `calledElement` is a free string — a BPMN process id (`Process_1`) collides across diagrams and cannot be resolved, so the modeler writes the **BusinessProcess card UUID** there instead. The parser keeps the raw value on `ProcessElement.called_element` (call activities only), and `sync_process_elements` resolves it into the fifth EA link, `business_process_id`, with a fixed precedence: **the XML wins** when the value is the UUID of an ACTIVE BusinessProcess other than the process itself (`resolve_called_processes`, one query per diagram); a value that does not resolve — a foreign reference from another tool, or nothing — **keeps a link the user made** in the element table, like every other link; and an element that is no longer a call activity **loses** the link. `_apply_draft_link` honours the same order: a stale pre-link cannot overrule a resolved `calledElement`. `validate_called_process` (ACTIVE BusinessProcess → 404, self → 400) is the one validator behind both write paths (`update_element` and `update_draft_element_link`), and the link is refused on any other element type. The relation it mints is the built-in `relProcessCalls` (*calls* / *is called by*, seeded and upserted by key on boot like every relation type, so it needed no migration; migration `150` adds the two columns and backfills from the stored XML), through `ELEMENT_LINK_RELATION_MAP` in `element_relation_sync.py` — the first **self-pair** in that map, which is why `sync_element_relations` now skips `target_id == process_id`. `element_link_ids()` / `ELEMENT_LINK_KEYS` are the one place the write paths collect link ids from, and the legacy `PUT /diagram` save also mints the relation for XML-resolved callees, since there the XML *is* the link. Nobody types an id: the frontend has **three doors to one picker** (`features/bpm/calledProcessModule.ts`, a didi module built around a `bridge` object the React side owns) — a `commandStack.shape.create.postExecuted` listener opens it the moment an unlinked call activity is placed, a properties-panel group `turboea__calledProcess` (provider at priority 500, i.e. after the built-in 1000; the entry is a Preact component in `CalledProcessEntry.ts`, authored with `h()` from `@bpmn-io/properties-panel/preact` and never a `.tsx` file, because `@vitejs/plugin-react` would wrap it in React's Fast Refresh runtime), and a context-pad entry. The pick goes through `modeling.updateProperties` (undoable, trips autosave); names are resolved after import with `fetchCardsByIds` and pushed into the panel with `eventBus.fire("propertiesPanel.providersChanged")`, the one public hook that re-runs every provider. Every rendering of the link — the **Calls** column chip (`ProcessFlowTab`), the viewer badge and popover chip, the navigator step chip — drills down to `/cards/{id}?tab=1` (`calledProcessPath`); the portal publishes `called_process_name` only. The process map's House has **no process↔process bucket**, so `relProcessCalls` shows in the generic Dependencies view, not there. Guard tests: `tests/api/test_bpm_called_process.py`, `test_process_element_sync.py::TestCalledProcess`, `test_element_relation_sync.py`, `frontend/src/features/bpm/calledProcess*.test.ts`, `CalledProcessEntry.test.ts`.
