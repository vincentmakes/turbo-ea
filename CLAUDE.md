...
- **Never hold a database session across work that is not database work.** A session checks a connection out of the pool on its **first `execute()`** and keeps it — inside an open transaction — until it commits or closes. The pool is small: `DB_POOL_SIZE + DB_MAX_OVERFLOW` (**30 by default**, `backend/app/config.py`) on a **single** uvicorn process (`Dockerfile` passes no `--workers`). This has shipped as a bug twice, so it is a rule, not a preference:
  - **A streaming endpoint must not take `Depends(get_db)`.** `get_db` is a yield-dependency — FastAPI unwinds it only when the *response completes*, and an SSE / `StreamingResponse` response completes when the client disconnects, i.e. when the browser tab closes. Querying that session left its connection checked out, `idle in transaction`, for the tab's whole lifetime, so **every open tab permanently consumed one of the 30** ([#901](https://github.com/TurboEA/turbo-ea/discussions/901), fixed in 2.38.0). Do the auth and lookup work in an explicit `async with async_session()` block that closes **before** the response object is constructed, and keep the generator body free of database access. See `event_stream` (`backend/app/api/v1/events.py`) and its guard tests in `backend/tests/api/test_events.py`.
...
The locale code is the ISO 639-1 two-letter code throughout (`da`, not `da-DK`). The recipe touches code in three layers (frontend, backend, docs) plus a one-shot data migration. PR [#623](https://github.com/TurboEA/turbo-ea/pull/623) (Danish) is the canonical worked example.
...
- **db**: pulled from `ghcr.io/TurboEA/turbo-ea/db:${TURBO_EA_TAG}`, persisted in the `postgres_data` volume
- **backend**: pulled from `ghcr.io/TurboEA/turbo-ea/backend:${TURBO_EA_TAG}`, exposed only inside the compose network
- **frontend**: pulled from `ghcr.io/TurboEA/turbo-ea/frontend:${TURBO_EA_TAG}`, exposed only inside the compose network
- **nginx**: pulled from `ghcr.io/TurboEA/turbo-ea/nginx:${TURBO_EA_TAG}`, public entrypoint on `HOST_PORT`, proxies `/api`, `/mcp`, `/drawio`, and `/` to internal services
- **mcp-server**: optional profile `mcp`, pulled from `ghcr.io/TurboEA/turbo-ea/mcp-server:${TURBO_EA_TAG}`
- **ollama**: optional profile `ai`, pulled from `ghcr.io/TurboEA/turbo-ea/ollama:${TURBO_EA_TAG}`, persisted in the `ollama_models` volume
...
- **Workflow**: `.github/workflows/docker-publish.yml` builds and pushes multi-arch (`amd64` + `arm64`) images to `ghcr.io/TurboEA/turbo-ea/{db,backend,frontend,nginx,mcp-server}` on every push to `main`, every `v*.*.*` tag, and on `workflow_dispatch`. The **ollama** image is intentionally excluded from the matrix because it is just a thin non-root patch over `ollama/ollama:latest`; republish it manually with `docker buildx build --platform linux/amd64,linux/arm64 --target ollama -t ghcr.io/TurboEA/turbo-ea/ollama:latest --push .` whenever upstream Ollama changes.
...
- **Image**: `ghcr.io/TurboEA/turbo-ea/ollama:${TURBO_EA_TAG}`, exposes port 11434 internally only
