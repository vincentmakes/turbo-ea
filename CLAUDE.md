...
- **A public read endpoint returns the artefact, not the model behind it.** `GET /diagrams/public/{slug}` returns `{name, xml}` and runs the XML through `sanitise_public_xml` first, stripping `cardId` / `cardType` / `relationId` / `relationType` / `parentGroupCell` (`backend/app/api/v1/diagrams.py`). A published diagram is a picture; without the strip, "no drill-through" would be a UI convention while the raw payload still handed every anonymous visitor the card UUIDs and the relation graph. Any future public surface owes the same treatment. Publishing is gated on its own `diagrams.publish` permission — exposing data outside the instance is a different authority from editing it, and no default role but admin holds it. Publication state is excluded from workspace transfer (`sections.py`) so cloning an instance can never re-expose a link on the target.
