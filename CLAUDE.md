...
- **Multi-key trust**: `DEFAULT_VENDOR_PUBLIC_KEYS` (key_id → base64 key) in `extension_signing.py` is the set of trusted vendor keys; `verify_with_trusted()` selects by the envelope/manifest `key_id` and falls back to trying every trusted key (so pre-`key_id` envelopes still verify). This lets a vendor sign licenses with a key separate from the one used to sign bundles, and lets keys rotate without a format change. `teax pack --key-id` stamps the id into the manifest. Delivery stays purely file-based (upload a `.teax` + paste/upload a signed license); there is no in-product store connection or online activation in the core repo. Anything about how licenses are distributed commercially lives outside this repo, in the vendor's private extensions repo.
