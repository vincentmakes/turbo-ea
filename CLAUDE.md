...
- **Read tools** (31, across eight clusters). **Cards & metamodel**: `search_cards` (exact substring), `semantic_search_cards` (embedding/hybrid meaning-based search — see Semantic Search below), `get_card`, `get_card_relations`, `get_card_hierarchy`, `list_card_types`, `get_relation_types`, `resolve_card_refs` (name-to-UUID pre-check), `analyze_impact` (dependency blast-radius for a proposed change). **Dashboards**: `get_dashboard`, `get_landscape`. **GRC (Risk Register)**: `list_risks`, `get_risk`, `get_risk_metrics`, `get_card_risks`. **GRC (Compliance)**: `list_compliance_findings`, `get_compliance_overview`. **Governance & Delivery**: `list_principles`, `list_adrs`, `get_adr`, `list_soaws`. **Reports**: `get_portfolio_report`, `get_cost_treemap`, `get_capability_heatmap`, `get_data_quality_report`. **Card context**: `get_card_stakeholders`, `get_card_comments`, `get_card_documents`. **Diagrams**: `list_diagrams`, `get_diagram`. (`resolve_card_refs` carries a read annotation — it resolves only, it never writes.)
...
- **Semantic search.** `semantic_search_cards` (MCP) and `GET /cards/semantic-search` (backend) rank cards by embedding similarity fused with the existing substring match (Reciprocal Rank Fusion). Embeddings live in the `card_embeddings` sidecar table (pgvector `vector(768)`, HNSW cosine index) and are generated/kept-fresh by the `_embedding_reconcile_loop` background task in `app.main` (content-hash + model gated; no per-write hook — it covers every write path including bulk/import). The embedding provider is a **separate** `ai.embedding` block in `general_settings` (Ollama `nomic-embed-text` default; OpenAI/Azure also; **never Anthropic — no embeddings API**), configured at `PATCH /settings/ai/embedding`. The search endpoint reuses `_apply_card_scope_filters` + `_cost_redaction_map` so semantic results honour the exact same RBAC/redaction as `list_cards` — a vector match must never surface a card the user couldn't list. Standardise on `EMBEDDING_DIM = 768` (changing it is a schema migration). The `db` image is `pgvector/pgvector:pg18`; the lifespan runs `CREATE EXTENSION IF NOT EXISTS vector` on every startup path (incl. the fresh-DB create_all path that skips migration bodies).
...
| `db` | `pgvector/pgvector:pg18` | Bundled PostgreSQL image (pgvector for semantic search; non-root hardening ported to the Debian base) |
