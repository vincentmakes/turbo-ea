...
- **Online store (optional layer)**: the vendor-hosted `store-server/` package (FastAPI + Stripe, deployed by the vendor only) sells subscriptions, issues signed composite licenses (key id `store-1`, regenerated statelessly from Stripe subscription state), and serves `.teax` downloads. Core connects via `/admin/extensions/store/*` (redeem code → encrypted account token in `app_settings.general_settings.extensionStore`; catalog/install/checkout/refresh proxied by `app/services/extensions/store_client.py`). Store-downloaded bundles run through the SAME verify → preview → apply pipeline as manual uploads. Trust is multi-key: `DEFAULT_VENDOR_PUBLIC_KEYS` (key_id → key) in `extension_signing.py`, `verify_with_trusted()` selects by envelope/manifest `key_id` with try-all fallback. The `store_router` must stay registered BEFORE `extensions.router` in `router.py` — otherwise `/admin/extensions/store` is captured by the parametrized `DELETE /admin/extensions/{key}` uninstall route.
