...
Respond with ONLY a JSON object containing the missing sections."""
    ...
    # ── Call 2: Mermaid diagram ──────────────────────────────────────────
    logger.info("Phase 3 \u2014 Call 2: generating Mermaid diagram...")

    layer_summary = "\n".join(
        f"{ly.get('name', '')}: {', '.join(c.get('name', '') + ' (' + c.get('type', '') + ')' for c in ly.get('components', []))}"  # noqa: E501
        for ly in result.get("layers", [])  # noqa: E501
    )

    integration_summary = "\n".join(
        f"{i.get('from', '')} \u2192 {i.get('to', '')} [{i.get('protocol', 'API')}, {i.get('direction', 'sync')}]"  # noqa: E501
        for i in result.get("integrations", [])
    )

    diagram_prompt = f"""Generate a Mermaid architecture diagram for this solution:

Title: {result.get("title", "")}
Pattern: {result.get("architecturalPattern", "")}
...
- Label arrows with protocol/data
- 15-30 nodes max
- Output ONLY the raw Mermaid code, no JSON wrapping, no markdown fences"""
    ...
    try:
        ...
        logger.info("Diagram generated: %d lines", len(mermaid.split("\n")))
    except Exception as e:
        logger.warning("Diagram generation failed: %s", e)
        result["mermaidDiagram"] = None
