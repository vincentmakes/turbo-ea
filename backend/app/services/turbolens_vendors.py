...
from app.services.turbolens_ai import call_ai, parse_json
...
async def analyse_vendors(db: AsyncSession) -> dict[str, Any]:
    ...
    existing_result = await db.execute(
        select(TurboLensVendorAnalysis.vendor_name, TurboLensVendorAnalysis.id)
    )
    existing_vendor_map: dict[str, TurboLensVendorAnalysis] = {}
    ...
    if existing_ids:
        existing_objs_result = await db.execute(
            select(TurboLensVendorAnalysis).where(
                TurboLensVendorAnalysis.vendor_name.in_(existing_ids.keys())
            )
        )
...
"""
        ...
        for item in parsed or []:
            ...
            if existing:
                ...
                new_obj = TurboLensVendorAnalysis(
...
async def resolve_vendors(db: AsyncSession) -> dict[str, Any]:
    ...
    va_result = await db.execute(select(TurboLensVendorAnalysis.vendor_name))
...
[{{"raw_name":"<exact input>","canonical_name":"<clean name>","vendor_type":"vendor|product|platform|module|unknown","parent_canonical":"<parent or null>","confidence":0.9}}]"""  # noqa: E501
    ...
    va_map: dict[str, TurboLensVendorAnalysis] = {}
    ...
    await db.execute(delete(TurboLensVendorHierarchy))
    await db.flush()
