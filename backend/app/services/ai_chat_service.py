...
Architecture:
  ...
  4. Stream the LLM response back token-by-token via the configured provider
...
async def _fetch_cards_by_type(
    ...
    """Fetch all active cards of a specific type."""
    stmt = (
        select(
            Card.id,
            Card.type,
...
# ---------------------------------------------------------------------------
# Streaming LLM chat — multi-provider support
# ---------------------------------------------------------------------------
...
async def _stream_ollama(
    ...
    """Stream tokens from an Ollama /api/chat endpoint."""
...
async def _stream_anthropic(
    ...
    """Stream tokens from the Anthropic /v1/messages endpoint."""
...
async def stream_chat_response(
    ...
    *,
    ...
    Supports Ollama, OpenAI-compatible, and Anthropic providers.
    ...
    try:
        if provider_type == "openai":
            async for token in _stream_openai_compatible(
                client, provider_url, api_key, model, full_messages
            ):
                yield token
            ...
            async for token in _stream_ollama(client, provider_url, model, full_messages):
                yield token
