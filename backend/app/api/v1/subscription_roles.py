"""Subscription role definition management API routes."""
...
router = APIRouter(tags=["subscription-roles"])
...
def _srd_response(srd: SubscriptionRoleDefinition, sub_count: int | None = None) -> dict:
    data = {
        "id": str(srd.id),
        "fact_sheet_type_key": srd.fact_sheet_type_key,
        "key": srd.key,
        "label": srd.label,
    ...
    if sub_count is not None:
        data["subscription_count"] = sub_count
    return data


async def _ensure_type_exists(db: AsyncSession, type_key: str) -> None:
    result = await db.execute(
        select(FactSheetType).where(FactSheetType.key == type_key)
    )
    if not result.scalar_one_or_none():
        raise HTTPException(404, f"Fact sheet type '{type_key}' not found")


# ---------------------------------------------------------------------------
# Routes — mounted under /metamodel/types/{type_key}/subscription-roles
# ---------------------------------------------------------------------------

@router.get("/metamodel/types/{type_key}/subscription-roles")
async def list_subscription_roles(
    type_key: str,
    include_archived: bool = False,
    db: AsyncSession = Depends(get_db),
    user: User = Depends(get_current_user),
):
    """List subscription roles for a fact sheet type."""
    await _ensure_type_exists(db, type_key)

    query = (
        select(SubscriptionRoleDefinition)
        .where(SubscriptionRoleDefinition.fact_sheet_type_key == type_key)
        .order_by(SubscriptionRoleDefinition.sort_order, SubscriptionRoleDefinition.key)
    )
    if not include_archived:
        query = query.where(SubscriptionRoleDefinition.is_archived == False)  # noqa: E712
...
@router.get("/metamodel/types/{type_key}/subscription-roles/{role_key}")
async def get_subscription_role(
    type_key: str,
    role_key: str,
    db: AsyncSession = Depends(get_db),
    user: User = Depends(get_current_user),
):
    """Get a single subscription role with permissions."""
    await _ensure_type_exists(db, type_key)

    result = await db.execute(
        select(SubscriptionRoleDefinition).where(
            SubscriptionRoleDefinition.fact_sheet_type_key == type_key,
            SubscriptionRoleDefinition.key == role_key,
        )
    )
    srd = result.scalar_one_or_none()
    if not srd:
        raise HTTPException(404, "Subscription role not found")

    # Count active subscriptions
    count_result = await db.execute(
        select(func.count(Subscription.id)).where(Subscription.role == role_key)
    )
    sub_count = count_result.scalar() or 0

    return _srd_response(srd, sub_count=sub_count)


@router.post("/metamodel/types/{type_key}/subscription-roles", status_code=201)
async def create_subscription_role(
    type_key: str,
    body: SubRoleCreate,
    db: AsyncSession = Depends(get_db),
    user: User = Depends(get_current_user),
    ...
    existing = await db.execute(
        select(SubscriptionRoleDefinition).where(
            SubscriptionRoleDefinition.fact_sheet_type_key == type_key,
            SubscriptionRoleDefinition.key == body.key,
        )
    )
    if existing.scalar_one_or_none():
        raise HTTPException(
            409, f"Subscription role '{body.key}' already exists for type '{type_key}'"
        )

    # Calculate next sort_order
    max_order = await db.execute(
        select(func.max(SubscriptionRoleDefinition.sort_order)).where(
            SubscriptionRoleDefinition.fact_sheet_type_key == type_key
        )
    )
    next_order = (max_order.scalar() or 0) + 1

    srd = SubscriptionRoleDefinition(
    ...
    return _srd_response(srd, sub_count=0)


@router.patch("/metamodel/types/{type_key}/subscription-roles/{role_key}")
async def update_subscription_role(
    ...
    if srd.is_archived:
        raise HTTPException(400, "Cannot edit an archived subscription role. Restore it first.")
...
@router.post("/metamodel/types/{type_key}/subscription-roles/{role_key}/archive")
async def archive_subscription_role(
    type_key: str,
    role_key: str,
    db: AsyncSession = Depends(get_db),
    user: User = Depends(get_current_user),
    ...
    if srd.is_archived:
        raise HTTPException(400, "Subscription role is already archived")

    # Check that at least one active role remains for this type
    active_count = await db.execute(
        select(func.count(SubscriptionRoleDefinition.id)).where(
            SubscriptionRoleDefinition.fact_sheet_type_key == type_key,
    ...
    if (active_count.scalar() or 0) == 0:
        raise HTTPException(
            409, "Cannot archive the last active subscription role for this type"
        )
    ...
    resp["affected_subscriptions_count"] = affected_count
    return resp


@router.post("/metamodel/types/{type_key}/subscription-roles/{role_key}/restore")
async def restore_subscription_role(
    type_key: str,
    role_key: str,
    db: AsyncSession = Depends(get_db),
    user: User = Depends(get_current_user),
    ...
    if not srd.is_archived:
        raise HTTPException(400, "Subscription role is not archived")
...
@router.get("/subscription-roles/permissions-schema")
