...
async def create_card(
    ...
    card = await card_write_service.create_card(
        ...
        alias=body.alias,
        strict_attributes=body.strict_attributes,
    )
    await db.commit()
...
async def update_card(
    ...
    # only touch the non-cost keys they were allowed to see. This is
    # permission shaping of the incoming payload, so it stays route-side —
    # the write service applies the merged result.
    ...
    changed = await card_write_service.update_card(
        db,
        card_write_service.WriteActor.from_user(user),
        card,
        updates,
...
async def _resolve_archive_delete_set(
    ...
    Thin wrapper over the shared card write service so the archive and
    delete routes keep their historical call shape.
    """
    return await card_write_service.resolve_archive_delete_set(
        db,
        primary,
        child_strategy=body.child_strategy,
...
async def archive_card(
    ...
    _, affected_children_ids, affected_related_card_ids = await card_write_service.archive_card_set(
        db,
        card_write_service.WriteActor.from_user(user),
        primary,
        child_strategy=body.child_strategy,
