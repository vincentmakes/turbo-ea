...
class TestDataQuality:
    ...
    async def test_data_quality_reports_eol_coverage_per_type(self, client, db, env):
        """The EOL coverage chart splits by where the information came from,
        and counts the same cards the EOL report does (#1065)."""
        admin = env["admin"]
        await create_card(
            db,
            card_type="ITComponent",
        ...
        assert resp.status_code == 200
        data = resp.json()
        itc = next(r for r in data["eol_coverage"] if r["type"] == "ITComponent")
        assert itc == {
            "type": "ITComponent",
            "linked": 1,
            "manual": 1,
        ...
        # Only the EOL-capable types are counted — a Business Capability has
        # no end of life to record.
        assert {r["type"] for r in data["eol_coverage"]} <= {"Application", "ITComponent"}
