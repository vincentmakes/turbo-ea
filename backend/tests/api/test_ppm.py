...
# ── Promotion back-link (GRC bridge) ──────────────────────────────


class TestRiskPromotionBackLink:
    """`GET /initiatives/{id}/risks` resolves the landscape-register
    back-link from the GRC side (source_type="ppm" / source_ref)."""

    async def _mk_risk(self, client, ppm_env, title: str) -> str:
        resp = await client.post(
            f"{BASE}/initiatives/{ppm_env['initiative'].id}/risks",
            json={"title": title, "probability": 3, "impact": 3},
            headers=auth_headers(ppm_env["admin"]),
        )
        assert resp.status_code == 200, resp.text
        return resp.json()["id"]

    async def _list(self, client, ppm_env) -> dict[str, dict]:
        resp = await client.get(
            f"{BASE}/initiatives/{ppm_env['initiative'].id}/risks",
            headers=auth_headers(ppm_env["admin"]),
        )
        assert resp.status_code == 200
        return {r["title"]: r for r in resp.json()}

    async def test_back_link_null_then_populated(self, client, db, ppm_env):
        # Two risks so the map-based (one query per page) enrichment is
        # exercised, not just a single-row special case.
        promoted_id = await self._mk_risk(client, ppm_env, "Promoted")
        await self._mk_risk(client, ppm_env, "Untouched")

        rows = await self._list(client, ppm_env)
        assert rows["Promoted"]["promoted_risk_id"] is None
        assert rows["Untouched"]["promoted_risk_id"] is None

        resp = await client.post(
            f"/api/v1/risks/promote/ppm/{promoted_id}",
            json={},
            headers=auth_headers(ppm_env["admin"]),
        )
        assert resp.status_code == 200, resp.text
        register_risk = resp.json()

        rows = await self._list(client, ppm_env)
        assert rows["Promoted"]["promoted_risk_id"] == register_risk["id"]
        assert rows["Promoted"]["promoted_risk_reference"] == register_risk["reference"]
        assert rows["Untouched"]["promoted_risk_id"] is None

    async def test_back_link_self_heals_when_register_risk_deleted(self, client, db, ppm_env):
        ppm_id = await self._mk_risk(client, ppm_env, "Ephemeral")
        resp = await client.post(
            f"/api/v1/risks/promote/ppm/{ppm_id}",
            json={},
            headers=auth_headers(ppm_env["admin"]),
        )
        assert resp.status_code == 200
        register_id = resp.json()["id"]

        resp = await client.delete(
            f"/api/v1/risks/{register_id}", headers=auth_headers(ppm_env["admin"])
        )
        assert resp.status_code == 200

        rows = await self._list(client, ppm_env)
        # No dangling pointer — the link resolves from the GRC side, so
        # deleting the register risk makes it null again.
        assert rows["Ephemeral"]["promoted_risk_id"] is None
