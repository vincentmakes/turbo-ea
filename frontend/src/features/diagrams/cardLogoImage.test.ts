...
describe("composeCardLogoImage", () => {
  it("returns a data URI carrying no raw ; or = ", async () => {
    ...
    const uri = await composeCardLogoImage("/api/v1/cards/c1/logo?v=1", "apps");
  ...
  it("recomposes when the logo has been replaced", async () => {
    ...
    await composeCardLogoImage("/api/v1/cards/c1/logo?v=1", "apps");
    toDataUrlSpy.mockClear();
    await composeCardLogoImage("/api/v1/cards/c1/logo?v=2", "apps");
  ...
  it("caches the failure too, so a missing logo is not refetched per pass", async () => {
    ...
    expect(await composeCardLogoImage(url, "apps")).toBeNull();
    loadSucceeds = true;
    expect(await composeCardLogoImage(url, "apps")).toBeNull();
  });
});
