...
export default function CommitInitiativeDialog({
  ...
  const [progress, setProgress] = useState<TurboLensCommitProgress | null>(null);
  ...
  const pollProgress = useCallback(
    (runId: string) => {
      pollRef.current = setInterval(async () => {
        try {
          const run = await api.get<TurboLensAnalysisRun>(
            `/turbolens/analysis-runs/${runId}`,
          );
          if (run.results) {
          ...
          if (run.status === "completed") {
            ...
            clearInterval(pollRef.current);
            setError(run.error_message || t("turbolens_commit_error"));
            setSubmitting(false);
          }
        } catch {
  ...
  const progressText = useMemo(() => {
    ...
    switch (progress.step) {
      case "creating_initiative":
        return t("turbolens_commit_progress_initiative");
      case "creating_cards":
        return t("turbolens_commit_progress_cards", {
          current: progress.current,
          total: progress.total,
        });
      case "creating_relations":
        return t("turbolens_commit_progress_relations");
      case "creating_adr":
        return t("turbolens_commit_progress_adr");
      default:
    ...
    >
      <DialogTitle>
        <Stack direction="row" spacing={1} alignItems="center">
          ...
          <span>{t("turbolens_commit_dialog_title")}</span>
        </Stack>
      </DialogTitle>
        ...
        {done ? (
          ...
          <Box sx={{ py: 4 }}>
            ...
            <Typography
              ...
              align="center"
            >
              {progressText || t("turbolens_commit_progress_initiative")}
            </Typography>
            {progress?.detail && (
              <Typography
                variant="caption"
          ...
          <Stack spacing={3} sx={{ mt: 1 }}>
            ...
            <TextField
              label={t("turbolens_commit_initiative_name")}
              value={name}
            ...
            {objectiveIds.length > 0 && (
              <Box>
                <Typography
                  ...
                  sx={{ mb: 0.5 }}
                >
                  {t("turbolens_commit_objectives")} ({objectiveIds.length})
                </Typography>
                ...
                <Typography variant="caption" color="text.secondary">
                  {t("turbolens_commit_objectives_hint")}
                </Typography>
              </Box>
            )}
            ...
            <Box>
              <Typography
                ...
                sx={{ mb: 1 }}
              >
                {t("turbolens_commit_select_cards")} ({selectedCards.size}/
                {newCards.length})
              </Typography>
              <Stack spacing={0.5}>
