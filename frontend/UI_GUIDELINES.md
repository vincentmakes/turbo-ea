...
| Border — proposed | 2 px dashed, type color, plus a green **NEW** badge top-left |
...
- **Aggregate relations** (View options, first row of the Relations group): a *level*, not a switch — **Off / By layer / By card type / By subtype** ([#1117](https://github.com/vincentmakes/turbo-ea/discussions/1117) asks for "card domain, card type, subtype etc."). Above Off, each lane's cards are clustered into **`ldvCluster`** boxes (nesting becomes lane → cluster → card) headed with the group's glyph, label and count, and **two boxes are joined by exactly ONE line**, merged on the unordered pair of endpoints and carrying `data.count`. Four rules are load-bearing:
  ...
  The count rides on `data.count`, never inside `relLabel` — the label's 24-char cap would eat it, and hiding the verbs must still leave the count. **Create diagram** is disabled while aggregating: boxes and merged connectors have no inventory counterpart to link a shape to. Anything resolving a node's absolute position must walk the whole parent chain (`absolutePosition`), not one level.
