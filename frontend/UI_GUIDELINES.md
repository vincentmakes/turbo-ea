...
- **Interaction modes** (bottom-left React Flow `Controls`, mutually exclusive, click again to turn off): **Highlight** (`highlight`) sticky-highlights a card's connections on click; **Expand** (`alt_route`) reveals *all* of a clicked card's neighbours; **Reveal parent** (`arrow_upward`) and **Reveal children** (`arrow_downward`) reveal only the clicked card's hierarchy parent / direct children (targeted, `parent_id`-based). All three delegate to the consumer (which holds the full graph) via `onNodeExpand` / `onNodeReveal`. Turning a Reveal tool off only stops click-to-reveal — the surfaced cards **persist so parent and child reveals can be layered in one view**, and clear only when the view is re-centred (the consumer clears its reveal sets on `center` change). The Reveal tools appear only when the consumer passes `onNodeReveal` — the static TurboLens consumers omit them.
- **Re-center** (bottom-left `Controls`, `location_on` map-pin): the default React Flow fitView button is disabled (`showFitView={false}`) and replaced with a custom `fitView()` button — the stock frame icon read too much like the top-bar **Fullscreen** button.
