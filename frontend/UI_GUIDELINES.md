...
- **Bottom-left `Controls` are split into two groups** by a small vertical gap (`.ldv-controls-gap`): a **view group** (React Flow zoom +/-, then **Re-center**, **Reset view**, **Fullscreen**) and an **exploration group** (Highlight / Expand / Reveal parent / Reveal children). Reset and Fullscreen live here, not in the top bar.
  - **Re-center** (`location_on` map-pin): the default React Flow fitView button is disabled (`showFitView={false}`) and replaced with a custom `fitView()` button — the stock frame icon read too much like the Fullscreen button.
  ...
  - **Fullscreen** (`fullscreen` / `fullscreen_exit`): toggles fullscreen on the view container.
